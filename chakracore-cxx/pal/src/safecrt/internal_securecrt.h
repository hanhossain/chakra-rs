...
#define _SECURECRT__FILL_STRING(_String, _Size, _Offset)                                                               \
    if ((_Size) != (static_cast<size_t>(-1)) && (_Size) != INT_MAX && (static_cast<size_t>(_Offset)) < (_Size))        \
    {                                                                                                                  \
        memset((_String) + (_Offset), _SECURECRT_FILL_BUFFER_PATTERN,                                                  \
...
#define _RETURN_EINVAL _VALIDATE_RETURN(("Invalid parameter" && 0), EINVAL, EINVAL)
