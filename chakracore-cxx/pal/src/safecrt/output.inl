...
#endif /* _UNICODE */
{
    ...
    else if (_Size > (INT_MAX / sizeof(char16_t)))
    {
...
#define FIND_CHAR_CLASS(lookuptbl, c)                                                                                  \
    ((c) < _T(' ') || (c) > _T('x') ? CH_OTHER : static_cast<enum CHARTYPE>(lookuptbl[(c) - _T(' ')] & 0xF))

#define FIND_NEXT_STATE(lookuptbl, class, state) static_cast<enum STATE>(lookuptbl[(class) * NUMSTATES + (state)] >> 4)
...
#ifdef _UNICODE
#ifndef FORMAT_VALIDATIONS
int _woutput(miniFILE *stream,
#else /* FORMAT_VALIDATIONS */
int _woutput_s(miniFILE *stream,
#endif /* FORMAT_VALIDATIONS */
#else /* _UNICODE */
#ifndef FORMAT_VALIDATIONS
int _output(miniFILE *stream,
#else /* FORMAT_VALIDATIONS */
int _output_s(miniFILE *stream,

#endif /* FORMAT_VALIDATIONS */
#endif /* _UNICODE */

             const _TCHAR *format, va_list argptr)
{
    ...
    while ((ch = *format++) != _T('\0') && charsout >= 0)
    {
        ...
        case ST_SIZE:
            ...
            case _T('I'):
                ...
                else if ((*format == _T('d')) || (*format == _T('i')) || (*format == _T('o')) || (*format == _T('u')) ||
            ...
            case _T('a'):
                {
                    ...
                    /* In safecrt, we provide a special version of _cfltcvt which internally calls printf (see
                     * safecrt_output_s.c) */
...
static void write_string(const _TCHAR *string, int len, miniFILE *f, int *pnumwritten)
{
    if ((f->_flag & _IOSTRG) && f->_base == NULL)
    {
        (*pnumwritten) += len;
...
__inline int get_int_arg(va_list *pargptr) { return va_arg(*pargptr, int); }
...
#if !LONG_IS_INT
__inline long get_long_arg(va_list *pargptr) { return va_arg(*pargptr, long); }
#endif /* !LONG_IS_INT */

#if !LONGLONG_IS_INT64
__inline long long get_long_long_arg(va_list *pargptr) { return va_arg(*pargptr, long long); }
#endif /* !LONGLONG_IS_INT64 */

#if _INTEGRAL_MAX_BITS >= 64
__inline long get_int64_arg(va_list *pargptr) { return va_arg(*pargptr, long); }
#endif /* _INTEGRAL_MAX_BITS >= 64    */
