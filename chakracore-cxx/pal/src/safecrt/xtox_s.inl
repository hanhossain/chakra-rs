...
errno_t xtox_s(unsigned long val, TCHAR *buf, size_t sizeInTChars, unsigned radix, int is_neg)
{
...
errno_t _itox_s(int val, TCHAR *buf, size_t sizeInTChars, int radix)
{
    errno_t e = 0;
...
errno_t _ltox_s(long val, TCHAR *buf, size_t sizeInTChars, int radix)
{
    return xtox_s(static_cast<unsigned long>(val), buf, sizeInTChars, radix, (radix == 10 && val < 0));
}

errno_t _ultox_s(unsigned long val, TCHAR *buf, size_t sizeInTChars, int radix)
{
...
errno_t x64tox_s(/* stdcall is faster and smaller... Might as well use it for the helper. */
                 unsigned long val, TCHAR *buf, size_t sizeInTChars, unsigned radix, int is_neg)
{
    TCHAR *p; /* pointer to traverse string */
    TCHAR *firstdig; /* pointer to first digit */
    TCHAR temp; /* temp char */
...
errno_t _i64tox_s(long long val, TCHAR *buf, size_t sizeInTChars, int radix)
{
    return x64tox_s(val, buf, sizeInTChars, radix, (radix == 10 && val < 0));
}

errno_t _ui64tox_s(unsigned long long val, TCHAR *buf, size_t sizeInTChars, int radix)
{
