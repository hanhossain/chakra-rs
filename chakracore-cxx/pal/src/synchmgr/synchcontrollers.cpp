...
namespace CorUnix
{
    ...
    PAL_ERROR CSynchControllerBase::Init(CPalThread *pthrCurrent, ControllerType ctCtrlrType,
                                         ObjectDomain odObjectDomain, CObjectType *potObjectType,
    ...
    void CSynchControllerBase::Release()
    {
...
#ifdef _DEBUG
        ThreadWaitInfo *ptwiWaitInfo = CPalSynchronizationManager::GetThreadWaitInfo(m_pthrOwner);
#endif // _DEBUG

        CPalSynchronizationManager *pSynchManager = CPalSynchronizationManager::GetInstance();
        ...
        if (WaitController == m_ctCtrlrType)
        {
            ...
            pSynchManager->CacheAddWaitCtrlr(m_pthrOwner, static_cast<CSynchWaitController *>(this));
        }
        else
        {
            // The cast here must be static_cast and not reinterpet_cast
            pSynchManager->CacheAddStateCtrlr(m_pthrOwner, static_cast<CSynchStateController *>(this));
        }
    }
    ...
    signaled)
    --*/
    PAL_ERROR CSynchWaitController::CanThreadWaitWithoutBlocking(bool *pfCanWaitWithoutBlocking, bool *pfAbandoned)
    {
        VALIDATEOBJECT(m_psdSynchData);
        ...
        if (!fRetVal && otiProcess == m_psdSynchData->GetObjectTypeId())
        {
            ...
            _ASSERT_MSG(NULL != pProcLocalData, "Process synch data pointer is missing\n");

            if (NULL != pProcLocalData &&
                CPalSynchronizationManager::HasProcessExited(pProcLocalData->dwProcessId, &dwExitCode,
                                                             &fIsActualExitCode))
            {
                TRACE("Process pid=%u exited with %s exitcode=%u\n", pProcLocalData->dwProcessId,
                      fIsActualExitCode ? "actual" : "guessed", dwExitCode);
    ...
    object
    --*/
    PAL_ERROR CSynchWaitController::RegisterWaitingThread(WaitType wtWaitType, uint32_t dwIndex, bool fAlertable)
    {
        VALIDATEOBJECT(m_psdSynchData);
        ...
        ptwiWaitInfo = CPalSynchronizationManager::GetThreadWaitInfo(m_pthrOwner);

        assert(ptwiWaitInfo->pthrOwner == m_pthrOwner);

        pdwWaitState = SharedIDToTypePointer(uint32_t, m_pthrOwner->synchronizationInfo.m_shridWaitAwakened);
        ...
        {
            _ASSERT_MSG(wtWaitType == ptwiWaitInfo->wtWaitType, "Conflicting wait types in wait registration\n");
        ...
        pwtlnNewNode->dwFlags = (MultipleObjectsWaitAll == wtWaitType) ? WTLN_FLAG_WAIT_ALL : 0;
        ...
        if (otiProcess == m_psdSynchData->GetObjectTypeId())
        {
            ...
            palErr = pSynchManager->RegisterProcessForMonitoring(m_pthrOwner, m_psdSynchData, pProcLocalData);
            if (NO_ERROR != palErr)
            {
                goto RWT_exit;
            }
        ...
        if (0 == ptwiWaitInfo->lObjCount)
        {
            ...
            dwWaitState = InterlockedCompareExchange(reinterpret_cast<int32_t *>(pdwWaitState),
                                                     static_cast<int32_t>(dwWaitState), TWS_ACTIVE);
    ...
    RWT_exit:
        if (palErr != NO_ERROR)
        {
            ...
            if (fEarlyDeath)
            {
                ...
                [[maybe_unused]] int32_t lSharedSynchLockCount =
                    CPalSynchronizationManager::ResetSharedSynchLock(m_pthrOwner);
                [[maybe_unused]] int32_t lLocalSynchLockCount =
                    CPalSynchronizationManager::ResetLocalSynchLock(m_pthrOwner);
    ...
    CProcProcessLocalData *CSynchWaitController::GetProcessLocalData()
    {
        ...
        assert(InternalGetCurrentThread() == m_pthrOwner);
        _ASSERT_MSG(NULL != m_pProcLocalData, "Pointer to process local data not yet initialized\n");
    ...
    PAL_ERROR CSynchStateController::GetSignalCount(int32_t *plSignalCount)
    {
        ...
        _ASSERT_MSG(0 <= lCount, "Internal error: negative signal count [signal count=%d]", lCount);
    ...
    waiting threads awakening.
    --*/
    PAL_ERROR CSynchStateController::IncrementSignalCount(int32_t lAmountToIncrement)
    {
        VALIDATEOBJECT(m_psdSynchData);
    ...
    --*/
    PAL_ERROR CSynchStateController::DecrementSignalCount(int32_t lAmountToDecrement)
    {
        VALIDATEOBJECT(m_psdSynchData);
    ...
    PAL_ERROR CSynchStateController::SetOwner(CPalThread *pNewOwningThread)
    {
        ...
        _ASSERT_MSG(CObjectType::OwnershipTracked == m_potObjectType->GetOwnershipSemantics(),
        ...
        palErr = m_psdSynchData->AssignOwnershipToThread(m_pthrOwner, pNewOwningThread);

        _ASSERT_MSG(0 == m_psdSynchData->GetOwnershipCount() || 0 == m_psdSynchData->GetSignalCount(),
                    "Conflicting values for SignalCount [%d] and "
                    "OwnershipCount [%d]\n",
                    m_psdSynchData->GetOwnershipCount(), m_psdSynchData->GetSignalCount());
    ...
    PAL_ERROR CSynchStateController::DecrementOwnershipCount()
    {
        ...
        if ((1 > lOwnershipCount) || (m_psdSynchData->GetOwnerProcessID() != getpid()) ||
        ...
        {
            CPalSynchronizationManager *pSynchManager = CPalSynchronizationManager::GetInstance();
            OwnedObjectsListNode *pooln = m_psdSynchData->GetOwnershipListNode();
    ...
    int32_t CSynchData::Release(CPalThread *pthrCurrent)
    {
        ...
        _ASSERT_MSG(0 <= lCount, "CSynchData %p with negative reference count [%d]\n", this, lCount);
        ...
        if (0 == lCount)
        {
            ...
            TRACE("Disposing %s waitable object with SynchData @ "
                  "{shrid=%p, p=%p}\n",
                  (SharedObject == m_odObjectDomain) ? "shared" : "local", (void *)m_shridThis, this);
            ...
            TRACE("Statistical data for SynchData of otiType=%u @ %p: WaitCount=%d "
                  "ContentionCount=%d\n",
                  m_otiObjectTypeId, this, lStatWaitCount, lStatContentionCount);

            do
            {
                lCount = g_rglStatWaitCount[m_otiObjectTypeId];
                lNewCount = lCount + lStatWaitCount;
                lNewCount = InterlockedCompareExchange(&(g_rglStatWaitCount[m_otiObjectTypeId]), lNewCount, lCount);
            }
            ...
            do
            {
                ...
                lNewCount =
                    InterlockedCompareExchange(&(g_rglStatContentionCount[m_otiObjectTypeId]), lNewCount, lCount);
            }
            while (lCount != lNewCount);
            ...
            TRACE("Total current statistical data for otiType=%u objects: WaitCount=%d "
                  "ContentionCount=%d\n",
                  m_otiObjectTypeId, lStatWaitCount, lStatContentionCount);
#endif // SYNCH_STATISTICS
    ...
    --*/
    PAL_ERROR CSynchData::ReleaseWaiterWithoutBlocking(CPalThread *pthrCurrent, CPalThread *pthrTarget)
    {
        VALIDATEOBJECT(this);
...
#ifdef _DEBUG
        CObjectType::SignalingSemantics ssSignalingSemantics = potObjectType->GetSignalingSemantics();
#endif // _DEBUG
        CObjectType::OwnershipSemantics osOwnershipSemantics = potObjectType->GetOwnershipSemantics();
        CObjectType::ThreadReleaseSemantics trsThreadReleaseSemantics = potObjectType->GetThreadReleaseSemantics();
        bool fReenteringObjWithOwnership = false;

        _ASSERT_MSG(CObjectType::SignalingNotApplicable != ssSignalingSemantics, "Signaling not applicable");
        _ASSERT_MSG(CObjectType::ThreadReleaseNotApplicable != trsThreadReleaseSemantics,
        ...
        _ASSERT_MSG(CObjectType::SingleTransitionObject != ssSignalingSemantics ||
                    ...
                    ssSignalingSemantics, osOwnershipSemantics, trsThreadReleaseSemantics);

        if (CObjectType::OwnershipTracked == osOwnershipSemantics && 0 < GetOwnershipCount())
        {
        ...
        if (!fReenteringObjWithOwnership && CObjectType::ThreadReleaseAltersSignalCount == trsThreadReleaseSemantics)
        {
            _ASSERT_MSG(0 < GetSignalCount(),
                        ...
                        "[signal count=%d]",
                        GetSignalCount());
            _ASSERT_MSG(CObjectType::OwnershipTracked != osOwnershipSemantics || 1 == GetSignalCount(),
    ...
    Note: this method must be called while holding the appropriate
          ...
          object is local, both local and shared one if the object is shared).
    --*/
    bool CSynchData::CanWaiterWaitWithoutBlocking(CPalThread *pWaiterThread, bool *pfAbandoned)
    {
        VALIDATEOBJECT(this);

        bool fRetVal = (0 < GetSignalCount());
        bool fAbandoned = false;
        bool fOwnershipTracked = (CObjectType::OwnershipTracked == GetObjectType()->GetOwnershipSemantics());
        if (fRetVal)
        {
        ...
        if (fOwnershipTracked)
        {
            ...
            if ((GetOwnerProcessID() == getpid()) && (GetOwnerThread() == pWaiterThread))
            {
                fRetVal = true;
                goto CWWWB_exit;
            }
    ...
    Note: this method must be called while holding the appropriate
          ...
          object is local, both local and shared one if the object is shared).
    --*/
    void CSynchData::Signal(CPalThread *pthrCurrent, int32_t lSignalCount, bool fWorkerThread)
    {
        VALIDATEOBJECT(this);
        ...
        bool fReleaseAltersSignalCount =
            (CObjectType::ThreadReleaseAltersSignalCount == GetObjectType()->GetThreadReleaseSemantics());
        ...
        {
            fThreadReleased = ReleaseFirstWaiter(pthrCurrent, &fDelegatedSignaling, fWorkerThread);
            if (!fThreadReleased)
            {
        ...
        _ASSERT_MSG(CObjectType::OwnershipTracked != GetObjectType()->GetOwnershipSemantics() ||
                        0 == GetOwnershipCount() || 0 == GetSignalCount(),
                    "Conflicting values for SignalCount [%d] and "
                    "OwnershipCount [%d]\n",
                    GetOwnershipCount(), GetSignalCount());

        _ASSERT_MSG(otiMutex != m_otiObjectTypeId || m_lSignalCount <= 1, "Mutex with invalid singal count\n");
    ...
    Note: this method must be called while holding the appropriate
          ...
          object is local, both local and shared one if the object is shared).
    --*/
    bool CSynchData::ReleaseFirstWaiter(CPalThread *pthrCurrent, bool *pfDelegated, bool fWorkerThread)
    {
        PAL_ERROR palErr = NO_ERROR;
        ...
        *pfDelegated = false;
        ...
        while (pwtlnItem)
        {
            ...
            pdwWaitState = SharedIDToTypePointer(uint32_t, pwtlnItem->shridWaitingState);

            if (fSharedObject)
            {
                shridNextItem = pwtlnItem->ptrNext.shrid;
                pwtlnNextItem = SharedIDToTypePointer(WaitingThreadsListNode, shridNextItem);
            }
            else
            {
            ...
            if (fWaitAll)
            {
                ...
                if (!fSharedSynchLock && !fSharedObject && LocalWait != pwtlnItem->ptwiWaitInfo->wdWaitDomain)
                {
                ...
                wcsWaitCompletionState = fMarkedForDelegatedObjectSingalingInProgress
                    ? WaitIsNotSatisfied
            ...
            if (WaitIsSatisfied == wcsWaitCompletionState)
            {
                ...
                if (CPalSynchronizationManager::InterlockedAwaken(pdwWaitState, FALSE))
                {
                    ...
                    if (dwPid == pwtlnItem->dwProcessId)
                    {
                        ...
                        if (CObjectType::OwnershipTracked == GetObjectType()->GetOwnershipSemantics())
                        {
                            ...
                            //       CsynchData::Signal
                            palErr = AssignOwnershipToThread(pthrCurrent, ptwiWaitInfo->pthrOwner);
                            if (NO_ERROR != palErr)
                            {
                        ...
                        if (fWaitAll)
                        {
                            ...
                            CPalSynchronizationManager::UnsignalRestOfLocalAwakeningWaitAll(
                                pthrCurrent, ptwiWaitInfo->pthrOwner, pwtlnItem, this);
                        }
                        ...
                        // Unregister the wait
                        pSynchManager->UnRegisterWait(pthrCurrent, ptwiWaitInfo, fSharedObject || fSharedSynchLock);

                        // After UnRegisterWait pwtlnItem is invalid
                        pwtlnItem = NULL;

                        palErr = CPalSynchronizationManager::WakeUpLocalThread(
                            pthrCurrent, ptwiWaitInfo->pthrOwner, fAbandoned ? MutexAbondoned : WaitSucceeded,
                            dwObjIdx);
                    ...
                    else
                    {
                        ...
                        if (NO_ERROR != palErr)
                        {
                            ERROR("Failed to dispatch remote awakening cmd to "
                                  ...
                                  "lost\n",
                                  pwtlnItem->dwProcessId, pwtlnItem->dwThreadId);
                        }
                    }
            ...
            else if (WaitMayBeSatisfied == wcsWaitCompletionState)
            {
                ...
                palErr = CPalSynchronizationManager::DelegateSignalingToRemoteProcess(
                    pthrCurrent, pwtlnItem->dwProcessId, pwtlnItem->ptrOwnerObjSynchData.shrid);
            ...
            if (fWorkerThread && fWaitAll && (dwPid == pwtlnItem->dwProcessId))
            {
                ...
                CPalSynchronizationManager::MarkWaitForDelegatedObjectSignalingInProgress(pthrCurrent, pwtlnItem);
            }
    ...
    Note: this method must be called while holding the appropriate
          ...
          object is local, both local and shared one if the object is shared).
    --*/
    int32_t CSynchData::ReleaseAllLocalWaiters(CPalThread *pthrCurrent)
    {
        PAL_ERROR palErr = NO_ERROR;
        ...
        while (pwtlnItem)
        {
                ...
                (!fWaitAll || WaitIsSatisfied == IsRestOfWaitAllSatisfied(pwtlnItem)))
            {
                ...
                if (CPalSynchronizationManager::InterlockedAwaken(pdwWaitState, FALSE))
                {
                    ...
                    palErr = CPalSynchronizationManager::WakeUpLocalThread(
                        pthrCurrent, ptwiWaitInfo->pthrOwner, fAbandoned ? MutexAbondoned : WaitSucceeded, dwObjIdx);
    ...
    --*/
    WaitCompletionState CSynchData::IsRestOfWaitAllSatisfied(WaitingThreadsListNode *pwtlnNode)
    {
        ...
        for (i = 0; i < iTgtCount; i++)
        {
            ...
            fRetVal = psdSynchDataItem->CanWaiterWaitWithoutBlocking(ptwiWaitInfo->pthrOwner, &fIsAbandoned);
    ...
    Note: this method must be called while holding the appropriate
          ...
          object is local, both local and shared one if the object is shared).
    --*/
    PAL_ERROR CSynchData::AssignOwnershipToThread(CPalThread *pthrCurrent, CPalThread *pthrTarget)
    {
        ...
        _ASSERT_MSG(CObjectType::OwnershipTracked == GetObjectType()->GetOwnershipSemantics(),
                    ...
                    this, GetObjectType()->GetOwnershipSemantics());
        ...
        if (0 < m_lOwnershipCount)
        {
            ...
            //
            _ASSERT_MSG(0 == GetSignalCount(), "Conflicting OwnershipCount and SignalCount values\n");
            ...
            _ASSERT_MSG(pthrTarget == m_pOwnerThread && getpid() == m_dwOwnerPid,
                        ...
                        this, getpid(), pthrTarget->GetThreadId(), m_dwOwnerPid, m_pOwnerThread->GetThreadId());
    ...
    void CSynchData::WaiterEnqueue(WaitingThreadsListNode *pwtlnNewNode)
    {
        ...
        {
            _ASSERT_MSG(NULL == m_ptrWTLHead.ptr, "Corrupted waiting list on local CSynchData @ %p\n", this);
    ...
    void CSynchData::ValidateObject(bool fDestructor)
    {
        ...
        _ASSERT_MSG(HeadSignature == m_dwDebugHeadSignature, "CSynchData header signature corruption [p=%p]", this);
        _ASSERT_MSG(TailSignature == m_dwDebugTailSignature, "CSynchData trailer signature corruption [p=%p]", this);
        _ASSERT_MSG((fDestructor && 0 == m_lRefCount) || (!fDestructor && 0 < m_lRefCount),
    ...
    void CSynchData::ValidateEmptyObject()
    {
        ...
        _ASSERT_MSG(HeadSignature != m_dwDebugHeadSignature, "CSynchData header previously signed [p=%p]", this);
        _ASSERT_MSG(TailSignature != m_dwDebugTailSignature, "CSynchData trailer previously signed [p=%p]", this);
    }
    /*++
    Method:
