...
#define NULLSharedID (static_cast<SHMPTR>(NULL))
...
namespace CorUnix
{
    uint32_t InternalWaitForMultipleObjectsEx(CPalThread *pthrCurrent, uint32_t nCount, const HANDLE *lpHandles,
                                              BOOL bWaitAll, uint32_t dwMilliseconds, BOOL bAlertable);

    PAL_ERROR InternalSleepEx(CPalThread *pthrCurrent, uint32_t dwMilliseconds, BOOL bAlertable);
    ...
    typedef struct _ThreadWaitInfo
    {
        ...
        _ThreadWaitInfo() :
            wtWaitType(SingleObject), wdWaitDomain(LocalWait), lObjCount(0), lSharedObjCount(0), pthrOwner(NULL)
        {
        }
    } ThreadWaitInfo;
    ...
    typedef struct _ThreadNativeWaitData
    {
        ...
        _ThreadNativeWaitData() : iPred(0), dwObjectIndex(0), twrWakeupReason(WaitSucceeded), fInitialized(false) {}
    ...
    public:
        ...
        THREAD_STATE GetThreadState(void) { return m_tsThreadState; };

        void SetThreadState(THREAD_STATE tsThreadState) { m_tsThreadState = tsThreadState; };

        ThreadNativeWaitData *GetNativeData() { return &m_tnwdNativeData; }
    ...
    public:
        CThreadApcInfo() : m_ptainHead(NULL), m_ptainTail(NULL) {}
    };
