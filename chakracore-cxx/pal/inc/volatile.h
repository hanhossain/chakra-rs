...
#if defined(__GNUC__) && !defined(_AMD64_) && !defined(_ARM_) && !defined(_ARM64_)
...
template <typename T>
inline T VolatileLoadWithoutBarrier(T const *pt)
{
    // STATIC_CONTRACT_SUPPORTS_DAC_HOST_ONLY;
...
template <typename T>
inline T VolatileLoad(Volatile<T> const *pt)
{
...
template <typename T>
inline void VolatileStoreWithoutBarrier(T *pt, T val)
{
    // STATIC_CONTRACT_SUPPORTS_DAC_HOST_ONLY;
...
public:
    ...
    // Assignment from T
    //
    inline Volatile<T> &operator=(T val)
    {
        Store(val);
        return *this;
    }
