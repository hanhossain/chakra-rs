...
void WScriptJsrt::SetExceptionIf(JsErrorCode errorCode, const char16_t* errorMessage)
{
    if (errorCode != JsNoError)
    {
    ...
    const char * fileContent, const char * scriptInjectType, bool isSourceModule, JsFinalizeCallback finalizeCallback, bool isFile)
{
    int32_t hr = E_FAIL;
    JsErrorCode errorCode = JsNoError;
    const char16_t* errorMessage = u"Internal error.";
    JsValueRef returnValue = JS_INVALID_REFERENCE;
...
{
    const char16_t* errorMessage = u"invalid call to WScript.monotonicNow";
    JsErrorCode errorCode = JsNoError;
    int32_t hr = S_OK;
    JsValueRef result;
...
{
    const char16_t* errorMessage = u"invalid call to WScript.SetTimeout";
    JsErrorCode errorCode = JsNoError;
    int32_t hr = S_OK;
...
{
    const char16_t* errorMessage = u"invalid call to WScript.ClearTimeout";
    JsErrorCode errorCode = JsNoError;
    int32_t hr = S_OK;
...
{
    const char16_t* errorMessage = u"WScript.Attach requires a function, like WScript.Attach(foo);";
    JsErrorCode errorCode = JsNoError;
    int32_t hr = S_OK;
    JsValueType argumentType = JsUndefined;
    if (argumentCount != 2)
...
{
    const char16_t* errorMessage = u"WScript.Detach requires a function, like WScript.Detach(foo);";
    JsErrorCode errorCode = JsNoError;
    int32_t hr = S_OK;
    JsValueType argumentType = JsUndefined;
    if (argumentCount != 2)
    ...
    if (exception != nullptr)
    {
        if (jsErrorCode == JsErrorCode::JsErrorScriptCompile || jsErrorCode == JsErrorCode::JsErrorScriptException)
        {
            ...
            if (errorMessage.Initialize(exception) != JsNoError)
            {
                ...
                if (jsErrorCode != JsErrorCode::JsErrorScriptCompile)
                {
                    ...
                    if (metaData != JS_INVALID_REFERENCE)
                    {
                        ...
                        std::println(stderr, "        at code ({}{}:{}:{})", shortFileName, ext, line + 1, column + 1);
                    }
                    else
                    {
                        std::println(stderr, "{}", errorMessage.GetString());
                        std::println(stderr, "\tat code ({}{}:\?\?:\?\?)", shortFileName, ext);
                    }
                    return true;
                }
            }
            ...
            if (jsErrorCode == JsErrorCode::JsErrorScriptCompile)
            {
                ...
                _splitpath_s(fileName, nullptr, 0, nullptr, 0, shortFileName, _countof(shortFileName), ext, _countof(ext));
                std::println(stderr, "{}", errorMessage.GetString());
                std::println(stderr, "\tat code ({}{}:{}:{})",
                    shortFileName, ext, (int)line + 1,
                    (int)column + 1);
            }
            else
            {
                ...
                if (errorCode != JsErrorCode::JsNoError || propertyType == JsUndefined)
                {
                    ...
                    std::println(stderr, "thrown at %s%s:\n^", shortFileName, ext);
                    std::println(stderr, "{}", errorMessage.GetString());
                }
                else
                {
