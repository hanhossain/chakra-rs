...
{
    const char16_t* const DebugWriter::hex = u"0123456789abcdef";
...
#define PRINT_DEBUG_WRITER()                                        \
    ...
    void DebugWriter::PrintEscapedChar(const Char c)
    {
        CheckForNewline();
        if (c > 0xff)
            Output::Print(u"\\u%lc%lc%lc%lc", hex[c >> 12], hex[(c >> 8) & 0xf], hex[(c >> 4) & 0xf], hex[c & 0xf]);
        else if (c == '-')
            Output::Print(u"\\x2d");
        else if (c < ' ' || c > '~')
            Output::Print(u"\\x%lc%lc", hex[c >> 4], hex[c & 0xf]);
        else
            Output::Print(u"%lc", c);
    }
