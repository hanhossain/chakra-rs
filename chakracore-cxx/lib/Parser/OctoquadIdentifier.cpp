...
namespace UnifiedRegex
{
    ...
    void TrigramAlphabet::MegaMatch(__in_ecount(inputLen) const char16_t* input,int inputLen) {
        this->input=input;
        ...
        , __in_xcount(regex::TrigramAlphabet::AsciiTableSize) char* alphaBits)
    {
        ...
        for (uint i = 0; i < UnifiedRegex::TrigramAlphabet::AlphaCount; i++) {
            ...
            trigramAlphabet->alphaBits[static_cast<unsigned char>(alpha[i])] = alphaBits[static_cast<unsigned char>(alpha[i])];
        }
