...
static_assert(JSCRIPT_E_CANTEXECUTE != JSERR_CantExecute);
// verify the HR value (as MAKE_HRESULT(SEVERITY_ERROR, FACILITY_CONTROL, 0x1393))
static_assert(JSERR_CantExecute == 0x800A1393);

// verify HR matches between public SDK and private (.h) files
static_assert(JSCRIPT_E_CANTEXECUTE == JSPUBLICERR_CantExecute);
// verify the HR value (as MAKE_HRESULT(SEVERITY_ERROR, FACILITY_JSCRIPT, 0x0001))
static_assert(JSPUBLICERR_CantExecute == static_cast<int32_t>(0x89020001L));

// /PUBLIC ERROR codes

// boundary check - all errNum should be capped to 10,000 (RTERROR_STRINGFORMAT_OFFSET) - except for VBSERR_CantDisplayDate==32812
#define VERIFY_BOUNDARY_ERRNUM(name,errnum) static_assert(name == VBSERR_CantDisplayDate || errnum < RTERROR_STRINGFORMAT_OFFSET);
...
{
    uint32_t id1 = reinterpret_cast<const _ResourceStr*>(a)->id;
    uint32_t id2 = reinterpret_cast<const _ResourceStr*>(b)->id;
    return id1 - id2;
}
