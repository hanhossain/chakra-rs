...
namespace Js
{
    ...
    public:
        ...
        virtual void AsmShuffle(OpCodeAsmJs op, RegSlot R0, RegSlot R1, RegSlot R2, uint8_t indices[]) = 0;
        virtual void AsmSimdTypedArr(OpCodeAsmJs op, RegSlot value, uint32_t slotIndex, uint8_t dataWidth, ArrayBufferView::ViewType viewType, uint32_t offset = 0) = 0;
        virtual void WasmSimdConst(OpCodeAsmJs op, RegSlot R0, int C0, int C1, int C2, int C3) = 0;

        virtual void AsmSlot(OpCodeAsmJs op, RegSlot value, RegSlot instance, uint32_t slotId) = 0;
        ...
        virtual void WasmMemAccess(OpCodeAsmJs op, RegSlot value, uint32_t slotIndex, uint32_t offset, ArrayBufferView::ViewType viewType) = 0;
        virtual uint32_t WasmLoopStart(ByteCodeLabel loopEntrance, __in_ecount(WAsmJs::LIMIT) RegSlot* curRegs) = 0;
