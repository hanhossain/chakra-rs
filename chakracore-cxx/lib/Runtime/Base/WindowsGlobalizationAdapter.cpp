...
namespace Js
{
    ...
    public:
        ...
        IFACEMETHOD(GetRuntimeClassName)(_Out_ HSTRING* runtimeName) sealed
        {
            *runtimeName = nullptr;
            ...
            hr = WindowsCreateString(name, static_cast<uint32_t>(wcslen(name)), runtimeName);
            return hr;
        }
    ...
    HRESULT WindowsGlobalizationAdapter::GetActivationFactory(DelayLoadWindowsGlobalization *delayLoadLibrary, LPCWSTR factoryName, T** instance)
    {
        *instance = nullptr;
        ...
        IfFailedReturn(delayLoadLibrary->WindowsCreateStringReference(factoryName, static_cast<uint32_t>(wcslen(factoryName)), &hStringHdr, &hString));
    ...
    HRESULT WindowsGlobalizationAdapter::CreateLanguage(_In_ ScriptContext* scriptContext, _In_z_ PCWSTR languageTag, ILanguage** language)
    {
        ...
        IfFailedReturn(GetWindowsGlobalizationLibrary(scriptContext)->WindowsCreateStringReference(languageTag, static_cast<uint32_t>(wcslen(languageTag)), &hStringHdr, &hString));
        AnalysisAssert(hString);
    ...
    boolean WindowsGlobalizationAdapter::IsWellFormedLanguageTag(_In_ ScriptContext* scriptContext, _In_z_ PCWSTR languageTag)
    {
        ...
        // OK for languageTag to get truncated as it would pass incomplete languageTag below which
        // will be rejected by globalization dll
        IfFailThrowHr(GetWindowsGlobalizationLibrary(scriptContext)->WindowsCreateStringReference(languageTag, static_cast<uint32_t>(wcslen(languageTag)), &hStringHdr, &hString));
        if (hString == nullptr)
        {
            return 0;
        }
    ...
    boolean WindowsGlobalizationAdapter::ValidateAndCanonicalizeTimeZone(_In_ ScriptContext* scriptContext, _In_z_ PCWSTR timeZoneId, HSTRING *result)
    {
        ...
        // will be rejected by globalization dll
        IfFailThrowHr(GetWindowsGlobalizationLibrary(scriptContext)->WindowsCreateStringReference(timeZoneId, static_cast<uint32_t>(wcslen(timeZoneId)), &timeZoneHeader, &timeZone));
...
#pragma warning(suppress:6387)
    ...
    HRESULT WindowsGlobalizationAdapter::CreateTimeZoneOnCalendar(_In_ DelayLoadWindowsGlobalization *library, __out::ITimeZoneOnCalendar**  result)
    {
        ...
        IfFailedReturn(library->WindowsCreateStringReference(_u("en-US"), static_cast<uint32_t>(wcslen(_u("en-US"))), (headers), (arr)));
...
#undef DetachAndReleaseFactoryObjects
    ...
    HRESULT WindowsGlobalizationAdapter::CreateCurrencyFormatterCode(_In_ ScriptContext* scriptContext, _In_z_ PCWSTR currencyCode, NumberFormatting::ICurrencyFormatter** currencyFormatter)
    {
        ...
        // will be rejected by globalization dll
        IfFailedReturn(GetWindowsGlobalizationLibrary(scriptContext)->WindowsCreateStringReference(currencyCode, static_cast<uint32_t>(wcslen(currencyCode)), &hStringHdr, &hString));
        AnalysisAssert(hString);
    ...
    HRESULT WindowsGlobalizationAdapter::CreateCurrencyFormatter(_In_ ScriptContext* scriptContext, PCWSTR* localeStrings, uint32 numLocaleStrings, _In_z_ PCWSTR currencyCode, NumberFormatting::ICurrencyFormatter** currencyFormatter)
    {
        ...
        for(uint32 i = 0; i< numLocaleStrings; i++)
        {
            ...
            // will be rejected by globalization dll.
            IfFailedReturn(GetWindowsGlobalizationLibrary(scriptContext)->WindowsCreateStringReference(localeStrings[i], static_cast<uint32_t>(wcslen(localeStrings[i])), (headers + i), (arr + i)));
        }
        ...
        uint32 numLocaleStrings, _In_opt_z_ PCWSTR calendar, _In_opt_z_ PCWSTR clock, _Out_ DateTimeFormatting::IDateTimeFormatter** result)
    {
        ...
        // will be rejected by globalization dll.
        IfFailedReturn(GetWindowsGlobalizationLibrary(scriptContext)->WindowsCreateStringReference(formatString, static_cast<uint32_t>(wcslen(formatString)), &fsHStringHdr, &fsHString));
        ...
        else
        {
            ...
            // will be rejected by globalization dll.
            IfFailedReturn(GetWindowsGlobalizationLibrary(scriptContext)->WindowsCreateStringReference(calendar, static_cast<uint32_t>(wcslen(calendar)), &calStringHeader, &calString));
            AnalysisAssert(calString);
            IfFailedReturn(GetWindowsGlobalizationLibrary(scriptContext)->WindowsCreateStringReference(clock, static_cast<uint32_t>(wcslen(clock)), &clockStringHeader, &clockString));
