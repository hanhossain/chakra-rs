...
using namespace Js;
        ...
        : JavascriptNativeArray(type)
    {
        ...
        this->length = length;
        Recycler* recycler = GetRecycler();
        SetHeadAndLastUsedSegment(SparseArraySegment<int32_t>::AllocateSegment(recycler, 0, 0, size, nullptr));
    }
        ...
        : JavascriptNativeArray(type)
    {
        ...
        head->size = size;
        head->CheckLengthvsSize();
        SparseArraySegment<int32_t>::From(head)->FillSegmentBuffer(0, size);
    }
    ...
    bool JavascriptArray::IsMissingItem(uint32_t index)
    {
        ...
        if (isIntArray)
        {
            return IsMissingItemAt<int32_t>(index);
        }
    ...
    {
        return IsMissingHeadSegmentItemImpl<int32_t>(index);
    }
...
#endif
    Var JavascriptArray::OP_NewScIntArray(AuxArray<int32_t> *ints, ScriptContext* scriptContext)
    {
...
#if ENABLE_PROFILE_INFO
    Var JavascriptArray::ProfiledNewScIntArray(AuxArray<int32_t> *ints, ScriptContext* scriptContext, ArrayCallSiteInfo *arrayInfo, RecyclerWeakReference<FunctionBody> *weakFuncRef)
    {
        ...
        if (arrayInfo->IsNativeIntArray())
        {
...
#endif
            {
                ...
                SparseArraySegment<int32_t> *head = SparseArraySegment<int32_t>::From(arr->head);
...
#endif
    {
        ...
        for (i = start; i < length; i++)
        {
            ...
            if (isTaggedInt)
            {
                int32_t iValue = TaggedInt::ToInt32(item);
                isTaggedIntMissingValue = Js::SparseArraySegment<int32_t>::IsMissingItem(&iValue);
            }
            ...
            else if (!isTaggedIntMissingValue && JavascriptNumber::Is_NoTaggedIntCheck(item))
            {
                ...
                int32_t ivalue;
                if (JavascriptNumber::TryGetInt32Value(dvalue, &ivalue) && !Js::SparseArraySegment<int32_t>::IsMissingItem(&ivalue))
                {
            ...
            else
            {
                ...
                if (sizeof(int32_t) < sizeof(Var) && HasInlineHeadSegment(length) && i < this->head->length && !dontCreateNewArray)
                {
                    // Avoid shrinking the number of elements in the head segment. We can still create a new
                    // array here, so go ahead.
    ...
    JavascriptNativeFloatArray *JavascriptNativeIntArray::ToNativeFloatArray(JavascriptNativeIntArray *intArray)
    {
        ...
        if (arrayInfo)
        {
            ...
            if(foundScriptCaller)
            {
                ...
                {
                    Output::Print(u"Conversion: Int array to Float array    ArrayCreationFunctionNumber:%2d    CallSiteNumber:%2d \n", arrayInfo->functionNumber, arrayInfo->callSiteNumber);
                    Output::Flush();
                }
            }
            else
        ...
        for (seg = intArray->head; seg; seg = nextSeg)
        {
            ...
            if (seg == intArray->head || seg->length > (newSegSize >> 1))
            {
                ...
                {
                    ival = ((SparseArraySegment<int32_t>*)seg)->elements[i /*+ seg->length*/];
            ...
            else
            {
                ...
                {
                    ival = ((SparseArraySegment<int32_t>*)seg)->elements[i];
                    if (ival == JavascriptNativeIntArray::MissingItem)
                    {
    ...
    */
    ...
    /*
    *   JavascriptArray::ChangeArrayTypeToNativeArray<int32_t>
    *   -   Converts the Var Array's type to NativeInt.
    *   -   Sets the VirtualTable to "JavascriptNativeIntArray"
    */
    template<>
    void JavascriptArray::ChangeArrayTypeToNativeArray<int32_t>(JavascriptArray * varArray, ScriptContext * scriptContext)
    {
        AssertMsg(!VarIs<JavascriptNativeArray>(varArray), "Ensure that the incoming Array is a Var array");
    ...
    template<>
    int32_t JavascriptArray::GetNativeValue<int32_t>(Js::Var ival, ScriptContext * scriptContext)
    {
    ...
    */
    ...
    JavascriptArray *JavascriptNativeIntArray::ConvertToVarArray(JavascriptNativeIntArray *intArray)
    {
        ...
        for (seg = intArray->head; seg; seg = nextSeg)
        {
            ...
            // Shrink?
            uint32_t growFactor = sizeof(Var) / sizeof(int32_t);
    ...
    JavascriptArray *JavascriptNativeIntArray::ToVarArray(JavascriptNativeIntArray *intArray)
    {
        ...
        if (arrayInfo)
        {
            ...
            if(foundScriptCaller)
            {
                ...
                if(PHASE_TRACE(Js::NativeArrayConversionPhase, caller->GetFunctionBody()))
                {
                    Output::Print(u"Conversion: Int array to Var array    ArrayCreationFunctionNumber:%2d    CallSiteNumber:%2d \n", arrayInfo->functionNumber, arrayInfo->callSiteNumber);
                    Output::Flush();
                }
            }
            else
    ...
    */
    JavascriptArray *JavascriptNativeFloatArray::ConvertToVarArray(JavascriptNativeFloatArray *fArray)
    {
        ...
        for (seg = fArray->head; seg; seg = nextSeg)
        {
            ...
            for (i = 0; i < seg->length; i++)
            {
                ...
                else if (*(unsigned long*)&(((SparseArraySegment<double>*)seg)->elements[i]) == 0ull)
                {
    ...
    JavascriptArray *JavascriptNativeFloatArray::ToVarArray(JavascriptNativeFloatArray *fArray)
    {
        ...
        if (arrayInfo)
        {
            ...
            if(foundScriptCaller)
            {
                ...
                if(PHASE_TRACE(Js::NativeArrayConversionPhase, caller->GetFunctionBody()))
                {
                    Output::Print(u"Conversion: Float array to Var array    ArrayCreationFunctionNumber:%2d    CallSiteNumber:%2d \n", arrayInfo->functionNumber, arrayInfo->callSiteNumber);
                    Output::Flush();
                }
            }
            else
    ...
    uint32_t JavascriptArray::GetNextIndex(uint32_t index) const
    {
        ...
        {
            return this->GetNextIndexHelper<int32_t>(index);
        }
    ...
    BOOL JavascriptArray::SetLength(Var newLength)
    {
        ...
        {
            int32_t lenValue = TaggedInt::ToInt32(newLength);
            if (lenValue < 0)
            {
    ...
    {
        SparseArraySegment<int32_t>::ClearElements(((SparseArraySegment<int32_t>*)seg)->elements + newSegmentLength, seg->length - newSegmentLength);
    }
    ...
    Var JavascriptNativeIntArray::DirectGetItem(uint32_t index)
    {
        ...
        if (index >= seg->left && offset < seg->length)
        {
            if (!SparseArraySegment<int32_t>::IsMissingItem(&seg->elements[offset]))
            {
    ...
    void JavascriptArray::ThrowErrorOnFailure(BOOL succeeded, ScriptContext* scriptContext, BigIndex index)
    {
        if (!succeeded)
        {
            unsigned long i = (unsigned long)(index.IsSmallIndex() ? index.GetSmallIndex() : index.GetBigIndex());
    ...
    unsigned long JavascriptArray::OP_GetLength(Var obj, ScriptContext *scriptContext)
    {
        if (scriptContext->GetConfig()->IsES6ToLengthEnabled())
        {
            // Casting to unsigned long is okay as ToLength will always be >= 0.
            return (unsigned long)JavascriptConversion::ToLength(JavascriptOperators::OP_GetLength(obj, scriptContext), scriptContext);
        }
        else
        {
            return (unsigned long)JavascriptConversion::ToUInt32(JavascriptOperators::OP_GetLength(obj, scriptContext), scriptContext);
        }
    }
    ...
    JavascriptArray* JavascriptArray::ConcatIntArgs(JavascriptNativeIntArray* pDestArray, TypeId *remoteTypeIds, Js::Arguments& args, ScriptContext* scriptContext)
    {
        ...
        for (uint idxArg = 0; idxArg < args.Info.Count; idxArg++)
        {
            ...
            else if (!JavascriptArray::IsAnyArray(aItem) && remoteTypeIds[idxArg] != TypeIds_Array)
            {
                ...
                {
                    int32_t int32Value = TaggedInt::ToInt32(aItem);
                    Assert(!SparseArraySegment<int32_t>::IsMissingItem(&int32Value));
                    pDestArray->DirectSetItemAt(idxDest, int32Value);
                }
                else
                {
                    pDestArray->DirectSetItemAt(idxDest, static_cast<int32_t>(JavascriptNumber::GetValue(aItem)));
                }
                ++idxDest;
            }
            else
    ...
    Var JavascriptArray::EntryConcat(RecyclableObject* function, CallInfo callInfo, ...)
    {
        ...
        {
            JavascriptError::ThrowTypeError(scriptContext, JSERR_This_NullOrUndefined, u"Array.prototype.concat");
        }
            ...
            else // Get remote array or object length
            {
                ...
                else
                {
                    if (isFloat)
                    {
                        ...
                        else if (!TaggedInt::Is(aItem))
                        {
                            ...
                            else if (isInt)
                            {
                                ...
                                if(!JavascriptNumber::TryGetInt32Value(JavascriptNumber::GetValue(aItem), &int32Value) ||
                                    SparseArraySegment<int32_t>::IsMissingItem(&int32Value))
                                {
                                    isInt = false;
                                }
                            }
                        ...
                        {
                            int32_t int32Value = TaggedInt::ToInt32(aItem);
                            if(SparseArraySegment<int32_t>::IsMissingItem(&int32Value))
                            {
                                isInt = false;
                            }
                        }
    ...
    Var JavascriptArray::IndexOfHelper(Arguments const & args, ScriptContext *scriptContext)
    {
        ...
        if (pArr)
        {
            ...
            {
                uint32_t len = length.IsUint32Max() ? MaxArrayLength : length.GetSmallIndex();
                JS_REENTRANT(jsReentLock, int32_t index = pArr->HeadSegmentIndexOfHelper(search, fromIndex, len, includesAlgorithm, scriptContext));
    ...
    template <>
    BOOL JavascriptArray::TemplatedGetItem(RecyclableObject * obj, unsigned long index, Var * element, ScriptContext * scriptContext, bool checkHasItem)
    {
        // Note: Sometime cross site array go down this path to get the marshalling
        Assert(!VirtualTableInfo<JavascriptArray>::HasVirtualTable(obj)
            && !VirtualTableInfo<JavascriptNativeIntArray>::HasVirtualTable(obj)
    ...
    template <>
    BOOL JavascriptArray::TemplatedGetItem(JavascriptArray *pArr, unsigned long index, Var * element, ScriptContext * scriptContext, bool checkHasItem)
    {
    ...
    template <>
    BOOL JavascriptArray::TemplatedGetItem(JavascriptNativeIntArray *pArr, unsigned long index, Var * element, ScriptContext * scriptContext, bool checkHasItem)
    {
        // This should never get called.
        Assert(false);
        Throw::InternalError();
    ...
    template <>
    BOOL JavascriptArray::TemplatedGetItem(JavascriptNativeFloatArray *pArr, unsigned long index, Var * element, ScriptContext * scriptContext, bool checkHasItem)
    {
        // This should never get called.
        Assert(false);
        Throw::InternalError();
    ...
    template <>
    BOOL JavascriptArray::TemplatedGetItem(TypedArrayBase * typedArrayBase, unsigned long index, Var * element, ScriptContext * scriptContext, bool checkHasItem)
    {
        // This should never get called.
        Assert(false);
        Throw::InternalError();
    ...
    int32_t JavascriptArray::HeadSegmentIndexOfHelper(Var search, uint32_t &fromIndex, uint32_t toIndex, bool includesAlgorithm, ScriptContext * scriptContext)
    {
    ...
    void JavascriptArray::CopyValueToSegmentBuferNoCheck(Field(int32_t)* buffer, uint32_t length, int32_t value)
    {
        ...
        {
            memset(buffer, *(byte*)&value, sizeof(int32_t)* length);
        }
        else
        {
            for (uint32_t i = 0; i < length; i++)
    ...
    int32_t JavascriptNativeIntArray::HeadSegmentIndexOfHelper(Var search, uint32_t &fromIndex, uint32_t toIndex, bool includesAlgorithm,  ScriptContext * scriptContext)
    {
        ...
        // except when we can further optimize thanks to the knowledge that all elements in the array are int32_t's. This allows for two additional optimizations:
        // 1. Only tagged ints or JavascriptNumbers that can be represented as int32_t can be strict equal to some element in the array (all int32_t). Thus, if
        // the search value is some other kind of Var, we can return -1 without ever iterating over the elements.
        // 2. If the search value is a number that can be represented as int32_t, then we inspect the elements, but we don't need to perform the full strict equality algorithm.
        ...
        // We need to cast head segment to SparseArraySegment<int32_t> to have access to GetElement (onSparseArraySegment<T>). Because there are separate overloads of this
        ...
        // the other two, so the elements of each segment used here must be int32_t's. Hence, the cast is safe.

        SparseArraySegment<int32_t> * head = static_cast<SparseArraySegment<int32_t>*>(GetHead());
        uint32_t toIndexTrimmed = toIndex <= head->length ? toIndex : head->length;
        for (uint32_t i = fromIndex; i < toIndexTrimmed; i++)
        {
            int32_t element = head->GetElement(i);
    ...
    int32_t JavascriptNativeFloatArray::HeadSegmentIndexOfHelper(Var search, uint32_t &fromIndex, uint32_t toIndex, bool includesAlgorithm, ScriptContext * scriptContext)
    {
        // We proceed largely in the same manner as in JavascriptArray's version of this method (see comments there for more information),
    ...
    Var JavascriptArray::EntryJoin(RecyclableObject* function, CallInfo callInfo, ...)
    {
        ...
        if (args.Info.Count == 0)
        {
            JavascriptError::ThrowTypeError(scriptContext, JSERR_This_NullOrUndefined, u"Array.prototype.join");
        }
...
Case0:
    ...
    {
        JS_REENTRANCY_LOCK(jsReentLock, scriptContext->GetThreadContext());
        SETOBJECT_FOR_MUTATION(jsReentLock, object);
...
Case0:
    ...
    Var JavascriptArray::EntryLastIndexOf(RecyclableObject* function, CallInfo callInfo, ...)
    {
        ...
        JS_REENTRANT(jsReentLock, TryGetArrayAndLength(args[0], scriptContext, u"Array.prototype.lastIndexOf", &pArr, &obj, &length));
    ...
    *   -   Returns int32_t value from the array.
    *   -   Returns missing item when the element is not available in the array object.
    *   -   It doesn't walk up the prototype chain.
    *   -   Length is decremented only if it pops an int32_t element, in all other cases - we bail out from the jitted code.
    *   -   This api cannot cause any implicit call and hence do not need implicit call bailout test around this api
    */
    int32_t JavascriptNativeIntArray::Pop(ScriptContext * scriptContext, Var object)
    {
        ...
        int32_t element = Js::JavascriptOperators::OP_GetNativeIntElementI_UInt32(object, index, scriptContext);

        //If it is a missing item, then don't update the length - Pre-op Bail out will happen.
        if(!SparseArraySegment<int32_t>::IsMissingItem(&element))
        {
            arr->SetLength(index);
        }
    ...
    */
    ...
    Var JavascriptArray::EntryPopNonJavascriptArray(ScriptContext * scriptContext, Var object)
    {
        ...
        {
            JavascriptError::ThrowTypeError(scriptContext, JSERR_This_NullOrUndefined, u"Array.prototype.pop");
        }
        SETOBJECT_FOR_MUTATION(jsReentLock, dynamicObject);

        JS_REENTRANT(jsReentLock, BigIndex length = OP_GetLength(dynamicObject, scriptContext));

        ThrowTypeErrorOnFailureHelper h(scriptContext, u"Array.prototype.pop");
        if (length == 0u)
        {
            // Set length = 0
            JS_REENTRANT(jsReentLock,
    ...
    */
    Var JavascriptArray::EntryPushNonJavascriptArray(ScriptContext * scriptContext, Var * args, uint argCount)
    {
        ...
        {
            JavascriptError::ThrowTypeError(scriptContext, JSERR_This_NullOrUndefined, u"Array.prototype.push");
        }
        ...
        ThrowTypeErrorOnFailureHelper h(scriptContext, u"Array.prototype.push");
        BigIndex n;
        if (scriptContext->GetConfig()->IsES6ToLengthEnabled())
        {
            n = (unsigned long) JavascriptConversion::ToLength(length, scriptContext);
        }
        else
        {
    ...
    */
    ...
    Var JavascriptArray::EntryReverse(RecyclableObject* function, CallInfo callInfo, ...)
    {
        ...
        if (args.Info.Count == 0)
        {
            JavascriptError::ThrowTypeError(scriptContext, JSERR_This_NullOrUndefined, u"Array.prototype.reverse");
        }
        ...
        JS_REENTRANT(jsReentLock, TryGetArrayAndLength(args[0], scriptContext, u"Array.prototype.reverse", &pArr, &obj, &length));
    ...
    Var JavascriptArray::ReverseHelper(JavascriptArray* pArr, Js::TypedArrayBase* typedArrayBase, RecyclableObject* obj, T length, ScriptContext* scriptContext)
    {
        ...
        {
            methodName = u"[TypedArray].prototype.reverse";
        }
        else
        {
            methodName = u"Array.prototype.reverse";
        }
        ...
        if (useNoSideEffectReverse)
        {
            ...
            if (pArr->head && (pArr->head->next || (pArr->head->left + pArr->head->length) < length))
            {
                if (isIntArray)
                {
                    CopyHeadIfInlinedHeadSegment<int32_t>(pArr, recycler);
                    if (pArr->head->next)
                    {
                        ReallocateNonLeafLastSegmentIfLeaf<int32_t>(pArr, recycler);
                    }
                }
                else if (isFloatArray)
                {
            ...
            while (seg)
            {
                ...
                if (seg->length > 0)
                {
                    if (isIntArray)
                    {
                        ((SparseArraySegment<int32_t>*)seg)->ReverseSegment(recycler);
                    }
                    else if (isFloatArray)
                    {
            ...
            if (isIntArray)
            {
                pArr->EnsureHeadStartsFromZero<int32_t>(recycler);
            }
            else if (isFloatArray)
            {
    ...
    Var JavascriptArray::EntryShift(RecyclableObject* function, CallInfo callInfo, ...)
    {
        ...
        if (useNoSideEffectShift)
        {
            ...
            if (pArr->head->length != 0)
            {
                if(isIntArray)
                {
                    int32_t nativeResult = SparseArraySegment<int32_t>::From(pArr->head)->GetElement(0);

                    if(SparseArraySegment<int32_t>::IsMissingItem(&nativeResult))
                    {
                    ...
                    SparseArraySegment<int32_t>::From(pArr->head)->RemoveElement(recycler, 0);
                }
                else if (isFloatArray)
                {
            ...
            if(isIntArray)
            {
                ShiftHelper<int32_t>(pArr, scriptContext);
            }
            else if (isFloatArray)
            {
        ...
        else
        {
            ...
            {
                JavascriptError::ThrowTypeError(scriptContext, JSERR_This_NullOrUndefined, u"Array.prototype.shift");
            }

            ThrowTypeErrorOnFailureHelper h(scriptContext, u"Array.prototype.shift");
            ...
            for (unsigned long i = MaxArrayLength; length > i; i++)
            {
                JS_REENTRANT(jsReentLock, BOOL hasItem = JavascriptOperators::HasItem(dynamicObject, i + 1));
                if (hasItem)
                {
    ...
    Var JavascriptArray::EntrySlice(RecyclableObject* function, CallInfo callInfo, ...)
    {
        ...
        JS_REENTRANT(jsReentLock, TryGetArrayAndLength(args[0], scriptContext, u"Array.prototype.slice", &pArr, &obj, &length));
    ...
    Var JavascriptArray::SliceHelper(JavascriptArray* pArr, Js::TypedArrayBase* typedArrayBase, RecyclableObject* obj, T length, Arguments& args, ScriptContext* scriptContext)
    {
        ...
        {
            JavascriptError::ThrowTypeError(scriptContext, JSERR_DetachedTypedArray, u"Array.prototype.slice");
        }
            ...
            if (newArr && isBuiltinArrayCtor)
            {
                if (JavascriptArray::IsDirectAccessArray(newArr))
                {
                    if (((start + newLen) <= pArr->head->length) && newLen <= newArr->head->size) //Fast Path
                    {
                        if (isIntArray)
                        {
                            JS_REENTRANT(jsReentLock, SliceHelper<int32_t>(pArr, newArr, start, newLen));
                        }
                        else if (isFloatArray)
                        {
    ...
    Var JavascriptArray::SortHelper(Var array, JavascriptArray::CompareVarsInfo* cvInfo)
    {
        ...
        {
            JavascriptError::ThrowTypeError(scriptContext, JSERR_This_NullOrUndefined, u"Array.prototype.sort");
        }
        ...
        BEGIN_TEMP_ALLOCATOR(tempAlloc, scriptContext, u"Runtime")
        {
    ...
    Var JavascriptArray::EntrySort(RecyclableObject* function, CallInfo callInfo, ...)
    {
        ...
        ARGUMENTS(args, callInfo);
        ScriptContext* scriptContext = function->GetScriptContext();
        AUTO_TAG_NATIVE_LIBRARY_ENTRY(function, callInfo, u"Array.prototype.sort");
        ...
        if (args.Info.Count > 1)
        {
            ...
            else
            {
                ...
                {
                    JavascriptError::ThrowTypeError(scriptContext, JSERR_FunctionArgument_NeedFunction, u"Array.prototype.sort");
                }
            }
        }
    ...
    Var JavascriptArray::EntrySplice(RecyclableObject* function, CallInfo callInfo, ...)
    {
        ...
        unsigned long deleteLen = 0u;
        unsigned long length = 0u;

        JS_REENTRANT(jsReentLock, TryGetArrayAndLength(args[0], scriptContext, u"Array.prototype.splice", &pArr, &pObj, &length));
        ...
        {
            JS_REENTRANT_UNLOCK(jsReentLock,
                return ObjectSpliceHelper<unsigned long>(pObj, length, start, deleteLen, (Var*)insertArgs, insertLen, scriptContext, nullptr));
        }
        else
        {
            JS_REENTRANT_UNLOCK(jsReentLock,
        ...
        Var* insertArgs, uint32_t insertLen, ScriptContext *scriptContext)
    {
        ...
        if (newLenOverflow.HasOverflowed())
        {
            ...
            JS_REENTRANT_UNLOCK(jsReentLock, return ObjectSpliceHelper<unsigned long>(pArr, len, start, deleteLen, insertArgs, insertLen, scriptContext, newObj));
        }
        ...
        if (newArr && isBuiltinArrayCtor && len == pArr->length)
        {
            ...
            if (pArr->IsSingleSegmentArray() && pArr->head->HasIndex(start))
            {
                ...
                if (isIntArray)
                {
                    ArraySegmentSpliceHelper<int32_t>(newArr,
                        SparseArraySegment<int32_t>::From(pArr->head),
                        SparseArraySegment<int32_t>::AddressFrom(&pArr->head),
                        start, deleteLen, insertArgs, insertLen, recycler);
                }
                else if (isFloatArray)
                {
            ...
            else
            {
                if (isIntArray)
                {
                    ArraySpliceHelper<int32_t>(newArr, pArr, start, deleteLen, insertArgs, insertLen, scriptContext);
                }
                else if (isFloatArray)
                {
            ...
            {
                pArr->EnsureHeadStartsFromZero<int32_t>(recycler);
                newArr->EnsureHeadStartsFromZero<int32_t>(recycler);
            }
            else if (isFloatArray)
            {
                pArr->EnsureHeadStartsFromZero<double>(recycler);
        ...
        T deleteLen, Var* insertArgs, uint32_t insertLen, ScriptContext *scriptContext, RecyclableObject* pNewObj)
    {
        ...
        ThrowTypeErrorOnFailureHelper h(scriptContext, u"Array.prototype.splice");
    ...
    Var JavascriptArray::EntryToLocaleString(RecyclableObject* function, CallInfo callInfo, ...)
    {
        ...
        if (args.Info.Count == 0)
        {
            JavascriptError::ThrowTypeError(scriptContext, JSERR_This_NeedObject, u"Array.prototype.toLocaleString");
        }
        ...
        else
        {
            ...
            {
                JavascriptError::ThrowTypeError(scriptContext, JSERR_DetachedTypedArray, u"Array.prototype.toLocalString");
            }

            RecyclableObject* obj = nullptr;
            if (FALSE == JavascriptConversion::ToObject(args[0], scriptContext, &obj))
            {
                JavascriptError::ThrowTypeError(scriptContext, JSERR_This_NullOrUndefined, u"Array.prototype.toLocaleString");
            }
    ...
    void JavascriptArray::Unshift(RecyclableObject* obj, const T& toIndex, P start, P end, ScriptContext* scriptContext)
    {
        ...
        ThrowTypeErrorOnFailureHelper h(scriptContext, u"Array.prototype.unshift");
        if (start < end)
        {
    ...
    Var JavascriptArray::UnshiftObjectHelper(Js::Arguments& args, ScriptContext * scriptContext)
    {
        ...
        if (unshiftElements > 0)
        {
            ...
            if (end < length)
            {
                ...
                {
                    JS_REENTRANT(jsReentLock, Unshift<BigIndex, unsigned long>(dynamicObject, MaxArrayLength, (unsigned long)end.GetSmallIndex(), length.GetBigIndex(), scriptContext));
                }
            }
        ...
        if (useNoSideEffectUnshift)
        {
            if (unshiftElements > 0)
            {
                ...
                if (pArr->head->size < pArr->head->length + unshiftElements)
                {
                    if (isIntArray)
                    {
                        GrowArrayHeadHelperForUnshift<int32_t>(pArr, unshiftElements, scriptContext);
                    }
                    else if (isFloatArray)
                    {
                ...
                try
                {
                    if (isIntArray)
                    {
                        UnshiftHelper<int32_t>(pArr, unshiftElements, args.Values);
                    }
                    else if (isFloatArray)
                    {
    ...
    Var JavascriptArray::EntryToString(RecyclableObject* function, CallInfo callInfo, ...)
    {
        ...
        if (FALSE == JavascriptConversion::ToObject(args[0], scriptContext, &obj))
        {
            JavascriptError::ThrowTypeError(scriptContext, JSERR_This_NullOrUndefined, u"Array.prototype.toString");
        }
    ...
    Var JavascriptArray::EntryFind(RecyclableObject* function, CallInfo callInfo, ...)
    {
        ...
        if (args.Info.Count == 0)
        {
            JavascriptError::ThrowTypeError(scriptContext, JSERR_This_NullOrUndefined, u"Array.prototype.find");
        }
        ...
        JS_REENTRANT_UNLOCK(jsReentLock, TryGetArrayAndLength(args[0], scriptContext, u"Array.prototype.find", &pArr, &obj, &length));
    ...
    Var JavascriptArray::FindHelper(JavascriptArray* pArr, Js::TypedArrayBase* typedArrayBase, RecyclableObject* obj, long length, Arguments& args, ScriptContext* scriptContext)
    {
        ...
        if (args.Info.Count < 2 || !JavascriptConversion::IsCallable(args[1]))
        {
            ...
            {
                JavascriptError::ThrowTypeError(scriptContext, JSERR_FunctionArgument_NeedFunction, findIndex ? u"[TypedArray].prototype.findIndex" : u"[TypedArray].prototype.find");
            }
            else
            {
                JavascriptError::ThrowTypeError(scriptContext, JSERR_FunctionArgument_NeedFunction, findIndex ? u"Array.prototype.findIndex" : u"Array.prototype.find");
            }
        }
    ...
    Var JavascriptArray::EntryFindIndex(RecyclableObject* function, CallInfo callInfo, ...)
    {
        ...
        if (args.Info.Count == 0)
        {
            JavascriptError::ThrowTypeError(scriptContext, JSERR_This_NullOrUndefined, u"Array.prototype.findIndex");
        }
        ...
        JS_REENTRANT_UNLOCK(jsReentLock,
            TryGetArrayAndLength(args[0], scriptContext, u"Array.prototype.findIndex", &pArr, &obj, &length));
    ...
    Var JavascriptArray::EntryFindLast(RecyclableObject* function, CallInfo callInfo, ...)
    {
        ...
        if (args.Info.Count == 0)
        {
            JavascriptError::ThrowTypeError(scriptContext, JSERR_This_NullOrUndefined, u"Array.prototype.findLast");
        }
        ...
        JS_REENTRANT_UNLOCK(jsReentLock, TryGetArrayAndLength(args[0], scriptContext, u"Array.prototype.findLast", &pArr, &obj, &length));
    ...
    Var JavascriptArray::EntryFindLastIndex(RecyclableObject* function, CallInfo callInfo, ...)
    {
        ...
        if (args.Info.Count == 0)
        {
            JavascriptError::ThrowTypeError(scriptContext, JSERR_This_NullOrUndefined, u"Array.prototype.findLastIndex");
        }
        ...
        JS_REENTRANT_UNLOCK(jsReentLock,
            TryGetArrayAndLength(args[0], scriptContext, u"Array.prototype.findLastIndex", &pArr, &obj, &length));
    ...
    Var JavascriptArray::EntryEntries(RecyclableObject* function, CallInfo callInfo, ...)
    {
        ...
        if (args.Info.Count == 0)
        {
            JavascriptError::ThrowTypeError(scriptContext, JSERR_This_NullOrUndefined, u"Array.prototype.entries");
        }
    ...
    Var JavascriptArray::EntryKeys(RecyclableObject* function, CallInfo callInfo, ...)
    {
        ...
        if (args.Info.Count == 0)
        {
            JavascriptError::ThrowTypeError(scriptContext, JSERR_This_NullOrUndefined, u"Array.prototype.keys");
        }
    ...
    Var JavascriptArray::EntryValues(RecyclableObject* function, CallInfo callInfo, ...)
    {
        ...
        if (args.Info.Count == 0)
        {
            JavascriptError::ThrowTypeError(scriptContext, JSERR_This_NullOrUndefined, u"Array.prototype.values");
        }
    ...
    Var JavascriptArray::EntryAt(RecyclableObject* function, CallInfo callInfo, ...)
    {
        ...
        ScriptContext* scriptContext = function->GetScriptContext();
        JS_REENTRANCY_LOCK(jsReentLock, scriptContext->GetThreadContext());
        AUTO_TAG_NATIVE_LIBRARY_ENTRY(function, callInfo, u"Array.prototype.at");
        ...
        if (args.Info.Count == 0)
        {
            JavascriptError::ThrowTypeError(scriptContext, JSERR_This_NullOrUndefined, u"Array.prototype.at");
        }
        ...
        JS_REENTRANT(jsReentLock, TryGetArrayAndLength(args[0], scriptContext, u"Array.prototype.at", &pArr, &obj, &length));
    ...
    Var JavascriptArray::EntryEvery(RecyclableObject* function, CallInfo callInfo, ...)
    {
        ...
        ScriptContext* scriptContext = function->GetScriptContext();
        JS_REENTRANCY_LOCK(jsReentLock, scriptContext->GetThreadContext());
        AUTO_TAG_NATIVE_LIBRARY_ENTRY(function, callInfo, u"Array.prototype.every");
        ...
        if (args.Info.Count == 0)
        {
            JavascriptError::ThrowTypeError(scriptContext, JSERR_This_NullOrUndefined, u"Array.prototype.every");
        }
        ...
        JS_REENTRANT(jsReentLock, TryGetArrayAndLength(args[0], scriptContext, u"Array.prototype.every", &pArr, &obj, &length));
    ...
    Var JavascriptArray::EveryHelper(JavascriptArray* pArr, Js::TypedArrayBase* typedArrayBase, RecyclableObject* obj, T length, Arguments& args, ScriptContext* scriptContext)
    {
        ...
        if (args.Info.Count < 2 || !JavascriptConversion::IsCallable(args[1]))
        {
            ...
            if (typedArrayBase != nullptr)
            {
                JavascriptError::ThrowTypeError(scriptContext, JSERR_FunctionArgument_NeedFunction, u"[TypedArray].prototype.every");
            }
            else
            {
                JavascriptError::ThrowTypeError(scriptContext, JSERR_FunctionArgument_NeedFunction, u"Array.prototype.every");
            }
        }
    ...
    Var JavascriptArray::EntrySome(RecyclableObject* function, CallInfo callInfo, ...)
    {
        ...
        AUTO_TAG_NATIVE_LIBRARY_ENTRY(function, callInfo, u"Array.prototype.some");
        ...
        if (args.Info.Count == 0)
        {
            JavascriptError::ThrowTypeError(scriptContext, JSERR_This_NullOrUndefined, u"Array.prototype.some");
        }
        ...
        JS_REENTRANT(jsReentLock, TryGetArrayAndLength(args[0], scriptContext, u"Array.prototype.some", &pArr, &obj, &length));
    ...
    Var JavascriptArray::SomeHelper(JavascriptArray* pArr, Js::TypedArrayBase* typedArrayBase, RecyclableObject* obj, T length, Arguments& args, ScriptContext* scriptContext)
    {
        ...
        if (args.Info.Count < 2 || !JavascriptConversion::IsCallable(args[1]))
        {
            ...
            if (typedArrayBase != nullptr)
            {
                JavascriptError::ThrowTypeError(scriptContext, JSERR_FunctionArgument_NeedFunction, u"[TypedArray].prototype.some");
            }
            else
            {
                JavascriptError::ThrowTypeError(scriptContext, JSERR_FunctionArgument_NeedFunction, u"Array.prototype.some");
            }
        }
    ...
    Var JavascriptArray::EntryForEach(RecyclableObject* function, CallInfo callInfo, ...)
    {
        ...
        AUTO_TAG_NATIVE_LIBRARY_ENTRY(function, callInfo, u"Array.prototype.forEach");
        ...
        if (args.Info.Count == 0)
        {
            JavascriptError::ThrowTypeError(scriptContext, JSERR_This_NullOrUndefined, u"Array.prototype.forEach");
        }
        ...
        JS_REENTRANT(jsReentLock, TryGetArrayAndLength(args[0], scriptContext, u"Array.prototype.forEach", &pArr, &dynamicObject, &length));

        if (args.Info.Count < 2 || !JavascriptConversion::IsCallable(args[1]))
        {
            JavascriptError::ThrowTypeError(scriptContext, JSERR_FunctionArgument_NeedFunction, u"Array.prototype.forEach");
        }
        ...
        auto fn64 = [dynamicObject, callBackFn, flags, thisArg,
            scriptContext](unsigned long k, Var element)
        {
            BEGIN_SAFE_REENTRANT_CALL(scriptContext->GetThreadContext())
            {
                CALL_FUNCTION(scriptContext->GetThreadContext(), callBackFn, CallInfo(flags, 4), thisArg,
    ...
    Var JavascriptArray::EntryCopyWithin(RecyclableObject* function, CallInfo callInfo, ...)
    {
        ...
        JS_REENTRANT_UNLOCK(jsReentLock, TryGetArrayAndLength(args[0], scriptContext, u"Array.prototype.copyWithin", &pArr, &obj, &length));
    ...
    Var JavascriptArray::CopyWithinHelper(JavascriptArray* pArr, Js::TypedArrayBase* typedArrayBase, RecyclableObject* obj, long length, Arguments& args, ScriptContext* scriptContext)
    {
        ...
        if (isTypedArrayEntryPoint)
        {
            ...
            {
                JavascriptError::ThrowTypeError(scriptContext, JSERR_DetachedTypedArray, u"[TypedArray].prototype.copyWithin");
            }
    ...
    Var JavascriptArray::EntryFill(RecyclableObject* function, CallInfo callInfo, ...)
    {
        ...
        JS_REENTRANT_UNLOCK(jsReentLock,
            TryGetArrayAndLength(args[0], scriptContext, u"Array.prototype.fill", &pArr, &obj, &length));
    ...
    Var JavascriptArray::EntryMap(RecyclableObject* function, CallInfo callInfo, ...)
    {
        ...
        ScriptContext* scriptContext = function->GetScriptContext();
        JS_REENTRANCY_LOCK(jsReentLock, scriptContext->GetThreadContext());
        AUTO_TAG_NATIVE_LIBRARY_ENTRY(function, callInfo, u"Array.prototype.map");
        ...
        if (args.Info.Count == 0)
        {
            JavascriptError::ThrowTypeError(scriptContext, JSERR_This_NullOrUndefined, u"Array.prototype.map");
        }
        ...
        JS_REENTRANT(jsReentLock, TryGetArrayAndLength(args[0], scriptContext, u"Array.prototype.map", &pArr, &obj, &length));
    ...
    Var JavascriptArray::MapHelper(JavascriptArray* pArr, Js::TypedArrayBase* typedArrayBase, RecyclableObject* obj, T length, Arguments& args, ScriptContext* scriptContext)
    {
        ...
        if (args.Info.Count < 2 || !JavascriptConversion::IsCallable(args[1]))
        {
            if (isTypedArrayEntryPoint)
            {
                JavascriptError::ThrowTypeError(scriptContext, JSERR_FunctionArgument_NeedFunction, u"[TypedArray].prototype.map");
            }
            else
            {
                JavascriptError::ThrowTypeError(scriptContext, JSERR_FunctionArgument_NeedFunction, u"Array.prototype.map");
            }
        }
    ...
    Var JavascriptArray::EntryFilter(RecyclableObject* function, CallInfo callInfo, ...)
    {
        ...
        AUTO_TAG_NATIVE_LIBRARY_ENTRY(function, callInfo, u"Array.prototype.filter");


        Assert(!(callInfo.Flags & CallFlags_New));
        if (args.Info.Count == 0)
        {
            JavascriptError::ThrowTypeError(scriptContext, JSERR_This_NullOrUndefined, u"Array.prototype.filter");
        }
        ...
        JS_REENTRANT(jsReentLock, TryGetArrayAndLength(args[0], scriptContext, u"Array.prototype.filter", &pArr, &obj, &length));
    ...
    Var JavascriptArray::FilterHelper(JavascriptArray* pArr, RecyclableObject* obj, T length, Arguments& args, ScriptContext* scriptContext)
    {
        ...
        if (args.Info.Count < 2 || !JavascriptConversion::IsCallable(args[1]))
        {
            JavascriptError::ThrowTypeError(scriptContext, JSERR_FunctionArgument_NeedFunction, u"Array.prototype.filter");
        }
    ...
    Var JavascriptArray::EntryReduce(RecyclableObject* function, CallInfo callInfo, ...)
    {
        ...
        if (args.Info.Count == 0)
        {
            JavascriptError::ThrowTypeError(scriptContext, JSERR_This_NullOrUndefined, u"Array.prototype.reduce");
        }

        BigIndex length;
        JavascriptArray * pArr = nullptr;
        RecyclableObject* obj = nullptr;
        JS_REENTRANT(jsReentLock, TryGetArrayAndLength(args[0], scriptContext, u"Array.prototype.reduce", &pArr, &obj, &length));
    ...
    Var JavascriptArray::ReduceHelper(JavascriptArray* pArr, Js::TypedArrayBase* typedArrayBase, RecyclableObject* obj, T length, Arguments& args, ScriptContext* scriptContext)
    {
        ...
        if (args.Info.Count < 2 || !JavascriptConversion::IsCallable(args[1]))
        {
            if (typedArrayBase != nullptr)
            {
                JavascriptError::ThrowTypeError(scriptContext, JSERR_FunctionArgument_NeedFunction, u"[TypedArray].prototype.reduce");
            }
            else
            {
                JavascriptError::ThrowTypeError(scriptContext, JSERR_FunctionArgument_NeedFunction, u"Array.prototype.reduce");
            }
        }
        ...
        else
        {
            if (length == 0)
            {
                if (typedArrayBase)
                {
                    JavascriptError::ThrowTypeError(scriptContext, JSERR_EmptyArrayAndInitValueNotPresent, u"TypedArray.prototype.reduce");
                }
                else
                {
                    JavascriptError::ThrowTypeError(scriptContext, JSERR_EmptyArrayAndInitValueNotPresent, u"Array.prototype.reduce");
                }
            }
    ...
    Var JavascriptArray::EntryReduceRight(RecyclableObject* function, CallInfo callInfo, ...)
    {
        ...
        AUTO_TAG_NATIVE_LIBRARY_ENTRY(function, callInfo, u"Array.prototype.reduceRight");
        ...
        if (args.Info.Count == 0)
        {
            JavascriptError::ThrowTypeError(scriptContext, JSERR_This_NullOrUndefined, u"Array.prototype.reduceRight");
        }

        BigIndex length;
        JavascriptArray * pArr = nullptr;
        RecyclableObject* obj = nullptr;
        JS_REENTRANT(jsReentLock, TryGetArrayAndLength(args[0], scriptContext, u"Array.prototype.reduceRight", &pArr, &obj, &length));
    ...
    Var JavascriptArray::ReduceRightHelper(JavascriptArray* pArr, Js::TypedArrayBase* typedArrayBase, RecyclableObject* obj, T length, Arguments& args, ScriptContext* scriptContext)
    {
        ...
        if (args.Info.Count < 2 || !JavascriptConversion::IsCallable(args[1]))
        {
            if (typedArrayBase != nullptr)
            {
                JavascriptError::ThrowTypeError(scriptContext, JSERR_FunctionArgument_NeedFunction, u"[TypedArray].prototype.reduceRight");
            }
            else
            {
                JavascriptError::ThrowTypeError(scriptContext, JSERR_FunctionArgument_NeedFunction, u"Array.prototype.reduceRight");
            }
        }
    ...
    Var JavascriptArray::EntryFrom(RecyclableObject* function, CallInfo callInfo, ...)
    {
        ...
        AUTO_TAG_NATIVE_LIBRARY_ENTRY(function, callInfo, u"Array.from");
        ...
        {
            JavascriptError::ThrowTypeError(scriptContext, JSERR_FunctionArgument_NeedObject, u"Array.from");
        }
        ...
        {
#if ENABLE_COPYONACCESS_ARRAY
            JavascriptLibrary::CheckAndConvertCopyOnAccessNativeIntArray<Var>(itemsArr);
#endif
        }
        ...
        if (args.Info.Count >= 3 && !JavascriptOperators::IsUndefinedObject(args[2]))
        {
            ...
            {
                JavascriptError::ThrowTypeError(scriptContext, JSERR_FunctionArgument_NeedFunction, u"Array.from");
            }
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    JavascriptError::ThrowRangeError(scriptContext, JSERR_ArrayLengthAssignIncorrect, u"Array.from");
                }
    ...
    Var JavascriptArray::EntryOf(RecyclableObject* function, CallInfo callInfo, ...)
    {
        ...
        if (args.Info.Count == 0)
        {
            JavascriptError::ThrowTypeError(scriptContext, JSERR_This_NullOrUndefined, u"Array.of");
        }
    ...
    Var JavascriptArray::OfHelper(bool isTypedArrayEntryPoint, Arguments& args, ScriptContext* scriptContext)
    {
        ...
        else
        {
            ...
            if (isTypedArrayEntryPoint)
            {
                JavascriptError::ThrowTypeError(scriptContext, JSERR_This_NeedFunction, u"[TypedArray].of");
            }
    ...
    */
    ...
    void JavascriptArray::DoTypeMutation()
    {
        ...
        uint32_t random1 = static_cast<uint32_t>(PAL_rand());
        if (random1 % 2 == 0)
        {
            if (VarIs<JavascriptNativeIntArray>(this))
            {
                uint32_t random2 = static_cast<uint32_t>(PAL_rand());
                if (random2 % 2 == 0)
                {
    ...
    void JavascriptArray::ForEachOwnMissingArrayIndexOfObject(JavascriptArray *baseArray, JavascriptArray *destArray, RecyclableObject* obj, uint32_t startIndex, uint32_t limitIndex, uint32_t destIndex, Fn fn)
    {
        ...
        SET_SECOND_OBJECT_FOR_MUTATION(jsReentLock, destArray);
    ...
    //
    // Construct a BigIndex initialized to a given unsigned long (large or small index).
    //
    JavascriptArray::BigIndex::BigIndex(unsigned long initIndex)
    ...
    unsigned long JavascriptArray::BigIndex::GetBigIndex() const
    {
        Assert(!IsSmallIndex());
        return bigIndex;
    }
    ...
    JavascriptArray::BigIndex JavascriptArray::BigIndex::operator+(uint32_t delta) const
    {
        ...
        {
            return bigIndex + static_cast<unsigned long>(delta);
        }
    }
    ...
    bool JavascriptArray::BigIndex::operator==(const BigIndex& rhs) const
    {
        ...
        else if (rhs.IsSmallIndex() && !this->IsSmallIndex())
        {
            ...
            return this->GetBigIndex() == (unsigned long) rhs.GetSmallIndex();
        }
        else if (!rhs.IsSmallIndex() && this->IsSmallIndex())
        {
            // if rhs is big promote lhs
            return ((unsigned long)this->GetSmallIndex()) == rhs.GetBigIndex();
        }
    ...
    bool JavascriptArray::CopyNativeIntArrayElements(JavascriptNativeIntArray* dstArray, uint32_t dstIndex, JavascriptNativeIntArray* srcArray, uint32_t start, uint32_t end)
    {
        ...
        // iterate on the array itself
        ArrayElementEnumerator e(srcArray, start, end);
        while(e.MoveNext<int32_t>())
        {
            uint n = dstIndex + (e.GetIndex() - start);
            dstArray->DirectSetItemAt(n, e.GetItem<int32_t>());
            count++;
        }
    ...
    bool JavascriptArray::CopyNativeIntArrayElementsToFloat(JavascriptNativeFloatArray* dstArray, uint32_t dstIndex, JavascriptNativeIntArray* srcArray, uint32_t start, uint32_t end)
    {
        ...
        {
            uint n = dstIndex + (e.GetIndex() - start);
            dstArray->DirectSetItemAt(n, (double)e.GetItem<int32_t>());
            count++;
        }
    ...
    void JavascriptArray::InternalCopyNativeIntArrayElements(JavascriptArray* dstArray, uint32_t dstIndex, JavascriptNativeIntArray* srcArray, uint32_t start, uint32_t end)
    {
        ...
        {
            uint32_t n = dstIndex + (e.GetIndex() - start);
            dstArray->DirectSetItemAt(n, JavascriptNumber::ToVar(e.GetItem<int32_t>(), scriptContext));
            count++;
        }
    ...
    void JavascriptNativeIntArray::ValidateArray()
    {
        ...
        ValidateArrayCommon();
        // Detailed segments validation
        JavascriptArray::ValidateSegment<int32_t>(SparseArraySegment<int32_t>::From(head));
    }
    ...
    {
        TTD::NSSnapObjects::SnapArrayInfo<int32_t>* sai = TTD::NSSnapObjects::ExtractArrayValues<int32_t>(this, alloc);
        TTD::NSSnapObjects::StdExtractSetKindSpecificInfo<TTD::NSSnapObjects::SnapArrayInfo<int32_t>*, TTD::NSSnapObjects::SnapObjectType::SnapNativeIntArrayObject>(objData, sai);
    }
...
#endif
        int32_t value;
        return JavascriptConversion::BooleanToPropertyQueryFlags(this->DirectGetItemAt<int32_t>(index, &value));
    }
    ...
    {
        int32_t intvalue;
        if (!this->DirectGetItemAt<int32_t>(index, &intvalue))
        {
            return FALSE;
        }
    ...
    TypeId JavascriptNativeIntArray::TrySetNativeIntArrayItem(Var value, int32_t *iValue, double *dValue)
    {
        if (TaggedInt::Is(value))
        {
            int32_t i = TaggedInt::ToInt32(value);
    ...
    BOOL JavascriptNativeIntArray::SetItem(uint32_t index, int32_t iValue)
    {
    ...
    {
        return this->DirectDeleteItemAt<int32_t>(index);
    }
    ...
    {
        stringBuilder->AppendCppLiteral(u"Object, (Array)");
        return TRUE;
    }
    ...
    template void Js::JavascriptArray::TypedArraySort<int32_t>(int32_t*, uint32_t, JavascriptArray::CompareVarsInfo*, ArenaAllocator*);
