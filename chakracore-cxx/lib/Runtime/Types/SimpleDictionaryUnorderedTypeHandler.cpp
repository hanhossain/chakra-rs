...
namespace Js
{
    ...
    SimpleDictionaryUnorderedTypeHandler<TPropertyIndex, TMapKey, IsNotExtensibleSupported>::
        ...
        SimpleDictionaryTypeHandlerBase<TPropertyIndex, TMapKey, IsNotExtensibleSupported>(
            recycler, slotCapacity, inlineSlotCapacity, offsetOfInlineSlots),
    ...
    SimpleDictionaryUnorderedTypeHandler<TPropertyIndex, TMapKey, IsNotExtensibleSupported>::
        ...
        SimpleDictionaryTypeHandlerBase<TPropertyIndex, TMapKey, IsNotExtensibleSupported>(
            scriptContext, propertyDescriptors, propertyCount, slotCapacity, inlineSlotCapacity, offsetOfInlineSlots),
        deletedPropertyIndex(PropertyIndexRanges<TPropertyIndex>::NoSlots)
    {
        this->isUnordered = true;
    }
    ...
    SimpleDictionaryUnorderedTypeHandler<TPropertyIndex, TMapKey, IsNotExtensibleSupported>::
        ...
        SimpleDictionaryTypeHandlerBase<TPropertyIndex, TMapKey, IsNotExtensibleSupported>(
            recycler, slotCapacity, propertyCapacity, inlineSlotCapacity, offsetOfInlineSlots),
        deletedPropertyIndex(PropertyIndexRanges<TPropertyIndex>::NoSlots)
    {
        this->isUnordered = true;
    }
    ...
    template <class TPropertyIndex, class TMapKey, bool IsNotExtensibleSupported>
    bool
    SimpleDictionaryUnorderedTypeHandler<TPropertyIndex, TMapKey, IsNotExtensibleSupported>::IsReusablePropertyIndex(
        const TPropertyIndex propertyIndex)
    {
        // When reusing a deleted property index, we will treat the property index as the dictionary index, so they must
        // be the same. Also, property indexes stored in the object's slot corresponding to a deleted property are
        // tagged so that they don't look like pointers. If the property index is too large, it will not be free-listed.
        return static_cast<int>(propertyIndex) >= 0 && static_cast<int>(propertyIndex) < this->propertyMap->Count() &&
            this->propertyMap->GetValueAt(propertyIndex).propertyIndex == propertyIndex &&
            !TaggedInt::IsOverflow(propertyIndex);
    }

    template <class TPropertyIndex, class TMapKey, bool IsNotExtensibleSupported>
    bool SimpleDictionaryUnorderedTypeHandler<TPropertyIndex, TMapKey, IsNotExtensibleSupported>::
        TryRegisterDeletedPropertyIndex(DynamicObject *const object, const TPropertyIndex propertyIndex)
    {
        Assert(object);
        ...
        Assert(!TaggedInt::IsOverflow(
            PropertyIndexRanges<TPropertyIndex>::NoSlots)); // the last deleted property's slot in the chain is going to
                                                            // store NoSlots as a tagged int
    ...
    template <class TPropertyIndex, class TMapKey, bool IsNotExtensibleSupported>
    bool SimpleDictionaryUnorderedTypeHandler<TPropertyIndex, TMapKey, IsNotExtensibleSupported>::
        TryReuseDeletedPropertyIndex(DynamicObject *const object, TPropertyIndex *const propertyIndex)
    {
        Assert(object);
    ...
    bool SimpleDictionaryUnorderedTypeHandler<TPropertyIndex, TMapKey, IsNotExtensibleSupported>::TryUndeleteProperty(
        DynamicObject *const object, const TPropertyIndex existingPropertyIndex, TPropertyIndex *const propertyIndex)
    {
        Assert(object);
        Assert(propertyIndex);
        ...
        if (!reused)
        {
            Assert(0); // at least one property index must have been free-listed since we're adding an existing deleted
                       // property
            return false;
        }
        ...
        // We're trying to add a deleted property index that is currently somewhere in the middle of the free-list
        // chain. To avoid rebuilding the free-list, swap the property descriptor with the one for the first deleted
        // property index in the free-list. Since we also need to make sure that each descriptor's property index is the
        // same as its dictionary entry index, we need to remove them from the dictionary and add them back in the same
        // order, which actually adds them in reverse order. This relies on the fact that BaseDictionary first reuses
        // the last-deleted entry index in its free-listing strategy. Should remove this dependence in the future.

        TMapKey propertyKeyToPreserve = this->propertyMap->GetKeyAt(*propertyIndex);
        SimpleDictionaryPropertyDescriptor<TPropertyIndex> descriptorToPreserve =
            this->propertyMap->GetValueAt(*propertyIndex);
        ...
        SimpleDictionaryPropertyDescriptor<TPropertyIndex> descriptorToReuse =
            this->propertyMap->GetValueAt(existingPropertyIndex);
