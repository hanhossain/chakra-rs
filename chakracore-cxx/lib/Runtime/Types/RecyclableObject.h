...
namespace Js
{
    ...
    private:
        ...
        PropertyIndex
            m_propertyIndex; // Slot index on m_instance for the property, or NoSlot to indicate the object can't cache
        ...
        PropertyRecordUsageCache
            *propertyRecordUsageCache; // Usage cache for the Symbol or PropertyString `prop` (interior pointer).

        uint inlineCacheIndex;
        bool isFunctionPIC;
        bool allowResizingPolymorphicInlineCache;

        void Set(RecyclableObject *instance, PropertyIndex propertyIndex, PropertyAttributes attributes,
                 InlineCacheFlags flags)
        {
            m_instance = instance;
    ...
    public:
        ...
        static void Set(PropertyValueInfo *info, RecyclableObject *instance, PropertyIndex propertyIndex,
                        PropertyAttributes attributes = PropertyWritable, InlineCacheFlags flags = InlineCacheNoFlags)
        {
            if (info)
            {
        ...
        static void SetCacheInfo(PropertyValueInfo *info, FunctionBody *const functionBody,
                                 InlineCache *const inlineCache, const InlineCacheIndex inlineCacheIndex,
        ...
        static void SetCacheInfo(_Out_ PropertyValueInfo *info, _In_opt_ TProperty *prop,
                                 _In_ PolymorphicInlineCache *const polymorphicInlineCache, bool allowResizing)
        {
            SetCacheInfo(info, prop, prop->GetPropertyRecordUsageCache(), polymorphicInlineCache, allowResizing);
        }
        static void SetCacheInfo(_Out_ PropertyValueInfo *info, _In_opt_ RecyclableObject *prop,
                                 _In_opt_ PropertyRecordUsageCache *const propertyRecordUsageCache,
                                 _In_ PolymorphicInlineCache *const polymorphicInlineCache, bool allowResizing);
        static void SetCacheInfo(_Out_ PropertyValueInfo *info,
                                 _In_ PolymorphicInlineCache *const polymorphicInlineCache, bool allowResizing);
        static void ClearCacheInfo(PropertyValueInfo *info);

        InlineCache *GetInlineCache() const { return this->inlineCache; }

        PolymorphicInlineCache *GetPolymorphicInlineCache() const { return this->polymorphicInlineCache; }

        FunctionBody *GetFunctionBody() const { return this->functionBody; }

        PropertyRecordUsageCache *GetPropertyRecordUsageCache() const { return this->propertyRecordUsageCache; }

        RecyclableObject *GetProperty() const { return this->prop; }

        uint GetInlineCacheIndex() const { return this->inlineCacheIndex; }

        bool AllowResizingPolymorphicInlineCache() const { return allowResizingPolymorphicInlineCache; }
        ...
        bool IsStoreFieldCacheEnabled() const { return IsStoreFieldCacheEnabled(this); }
    };
    ...
    enum PropertyOperationFlags : int32_t
    {
        ...
        // In particular, used by SetProperty/WithAttributes to throw, rather than return false, when then instance
        // object is not extensible.
        PropertyOperation_ThrowIfNotExtensible = 0x04,

        // Intent: avoid any checks and force the operation.
        // In particular, used by SetProperty/WithAttributes to force adding a property when an object is not
        // extensible.
        ...
        // No need to check for undeclared let/const in case of console scope (as this operation is initializing the
        // let/const)
        PropertyOperation_AllowUndeclInConsoleScope = 0x100,

        PropertyOperation_ThrowIfNonWritable = 0x200,

        // This will be passed during delete operation. This will make the delete operation throw when the property not
        // configurable.
    ...
    public:
        ...
        BOOL GetProperty(Var originalInstance, PropertyId propertyId, Var *value, PropertyValueInfo *info,
                         ScriptContext *requestContext);
        BOOL GetProperty(Var originalInstance, JavascriptString *propertyNameString, Var *value,
                         PropertyValueInfo *info, ScriptContext *requestContext);
        BOOL GetPropertyReference(Var originalInstance, PropertyId propertyId, Var *value, PropertyValueInfo *info,
                                  ScriptContext *requestContext);
        ...
        virtual BOOL HasOwnPropertyCheckNoRedecl(PropertyId propertyId)
        {
            Assert(FALSE);
            return FALSE;
        }
        virtual BOOL UseDynamicObjectForNoHostObjectAccess() { return FALSE; }
        virtual DescriptorFlags GetSetter(PropertyId propertyId, Var *setterValue, PropertyValueInfo *info,
                                          ScriptContext *requestContext)
        {
            return None;
        }
        virtual DescriptorFlags GetSetter(JavascriptString *propertyNameString, Var *setterValue,
                                          PropertyValueInfo *info, ScriptContext *requestContext)
        {
            return None;
        }
        ...
        virtual BOOL GetInternalProperty(Var instance, PropertyId internalPropertyId, Var *value,
                                         PropertyValueInfo *info, ScriptContext *requestContext);
        _Check_return_ _Success_(return) virtual BOOL
            GetAccessors(PropertyId propertyId, _Outptr_result_maybenull_ Var *getter,
                         _Outptr_result_maybenull_ Var *setter, ScriptContext *requestContext);
        ...
        virtual BOOL SetProperty(JavascriptString *propertyNameString, Var value, PropertyOperationFlags flags,
                                 PropertyValueInfo *info);
        virtual BOOL SetInternalProperty(PropertyId internalPropertyId, Var value, PropertyOperationFlags flags,
                                         PropertyValueInfo *info);
        ...
        virtual BOOL InitPropertyInEval(PropertyId propertyId, Var value,
                                        PropertyOperationFlags flags = PropertyOperation_None,
                                        PropertyValueInfo *info = NULL);
        virtual BOOL EnsureProperty(PropertyId propertyId);
        virtual BOOL EnsureNoRedeclProperty(PropertyId propertyId);
        virtual BOOL SetPropertyWithAttributes(PropertyId propertyId, Var value, PropertyAttributes attributes,
                                               PropertyValueInfo *info,
                                               PropertyOperationFlags flags = PropertyOperation_None,
                                               SideEffects possibleSideEffects = SideEffects_Any);
        ...
        virtual PropertyQueryFlags GetItemQuery(Var originalInstance, uint32_t index, Var *value,
                                                ScriptContext *requestContext);
        virtual PropertyQueryFlags GetItemReferenceQuery(Var originalInstance, uint32_t index, Var *value,
                                                         ScriptContext *requestContext);
        virtual DescriptorFlags GetItemSetter(uint32_t index, Var *setterValue, ScriptContext *requestContext)
        {
            return None;
        }
        ...
        virtual BOOL GetEnumerator(JavascriptStaticEnumerator *enumerator, EnumeratorFlags flags,
                                   ScriptContext *requestContext, EnumeratorCache *enumeratorCache = nullptr);
        virtual BOOL ToPrimitive(JavascriptHint hint, Var *value, ScriptContext *requestContext);
        virtual BOOL SetAccessors(PropertyId propertyId, Var getter, Var setter,
                                  PropertyOperationFlags flags = PropertyOperation_None);
        ...
        {
            return false;
        } // Freezes the instance, no additional property can be added or deleted or written
        ...
        virtual BOOL GetSpecialPropertyName(uint32_t index, JavascriptString **propertyName,
                                            ScriptContext *requestContext)
        {
            return false;
        }
        ...
        virtual RecyclableObject *GetThisAndUnwrappedInstance(
            Var *thisVar) const; // Due to the withScope object there are times we need to unwrap
...
#if DBG
        virtual bool CanStorePropertyValueDirectly(PropertyId propertyId, bool allowLetConst)
        {
            Assert(false);
            return false;
        };
        ...
        virtual void RemoveFromPrototype(ScriptContext *requestContext, bool *allProtoCachesInvalidated)
        {
            AssertMsg(false, "Shouldn't call this implementation.");
        }
        virtual void AddToPrototype(ScriptContext *requestContext, bool *allProtoCachesInvalidated)
        {
            AssertMsg(false, "Shouldn't call this implementation.");
        }
        virtual void SetPrototype(RecyclableObject *newPrototype)
        {
            AssertMsg(false, "Shouldn't call this implementation.");
        }
        virtual bool ClearProtoCachesWereInvalidated()
        {
            AssertMsg(false, "Shouldn't call this implementation.");
            return false;
        }

        virtual BOOL ToString(Js::Var *value, Js::ScriptContext *scriptContext)
        {
            AssertMsg(FALSE, "Do not use this function.");
            return false;
        }

        // don't need cross-site: in HostDispatch it's IDispatchEx based; in CustomExternalObject we have marshalling
        // code explicitly.
        ...
        virtual BOOL InvokeBuiltInOperationRemotely(JavascriptMethod entryPoint, Arguments args, Var *result)
        {
            return FALSE;
        };
    ...
    template <>
    inline bool VarIsImpl<RecyclableObject>(RecyclableObject *obj)
    {
        return true;
    }
    ...
    template <typename T>
    bool VarIsCorrectType(WriteBarrierPtr<T> obj)
    {
        return VarIsImpl<T>(obj);
    }
