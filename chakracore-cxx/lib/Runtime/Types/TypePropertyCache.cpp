...
namespace Js
{
    ...
    PropertyId TypePropertyCacheElement::Id() const { return id; }

    PropertyIndex TypePropertyCacheElement::Index() const { return index; }

    bool TypePropertyCacheElement::IsInlineSlot() const { return isInlineSlot; }

    bool TypePropertyCacheElement::IsSetPropertyAllowed() const { return isSetPropertyAllowed; }

    bool TypePropertyCacheElement::IsMissing() const { return isMissing; }

    DynamicObject *TypePropertyCacheElement::PrototypeObjectWithProperty() const { return prototypeObjectWithProperty; }

    void TypePropertyCacheElement::Cache(const PropertyId id, const PropertyIndex index, const bool isInlineSlot,
    ...
    void TypePropertyCacheElement::Cache(const PropertyId id, const PropertyIndex index, const bool isInlineSlot,
                                         const bool isSetPropertyAllowed, const bool isMissing,
                                         DynamicObject *const prototypeObjectWithProperty, Type *const myParentType)
    {
        Assert(id != Constants::NoProperty);
        Assert(index != Constants::NoSlot);
        ...
        Assert(this->isMissing ==
               (uint16)(this->prototypeObjectWithProperty ==
                        this->prototypeObjectWithProperty->GetLibrary()->GetMissingPropertyHolder()));
    }

    void TypePropertyCacheElement::Clear() { id = Constants::NoProperty; }
    ...
    inline bool TypePropertyCache::TryGetIndexForLoad(const bool checkMissing, const PropertyId id,
                                                      PropertyIndex *const index, bool *const isInlineSlot,
    ...
    inline bool TypePropertyCache::TryGetIndexForStore(const PropertyId id, PropertyIndex *const index,
                                                       bool *const isInlineSlot) const
    {
        ...
        const TypePropertyCacheElement &element = elements[ElementIndex(id)];
        if (element.Id() != id || !element.IsSetPropertyAllowed() || element.PrototypeObjectWithProperty())
        {
            return false;
        }
        ...
        PropertyValueInfo *const propertyValueInfo)
    {
        ...
        if (!TryGetIndexForLoad(checkMissing, propertyId, &propertyIndex, &isInlineSlot, &isMissing,
                                &prototypeObjectWithProperty))
        {
#if DBG_DUMP
            if (PHASE_TRACE1(TypePropertyCachePhase))
            {
                CacheOperators::TraceCache(static_cast<InlineCache *>(nullptr), u"TypePropertyCache get miss",
                                           propertyId, requestContext, propertyObject);
            }
#endif
            return false;
        }
        ...
        if (!prototypeObjectWithProperty)
        {
            ...
            if (PHASE_TRACE1(TypePropertyCachePhase))
            {
                CacheOperators::TraceCache(static_cast<InlineCache *>(nullptr), u"TypePropertyCache get hit",
                                           propertyId, requestContext, propertyObject);
            }
#endif
            ...
            {
                *propertyValue = isInlineSlot ? VarTo<DynamicObject>(propertyObject)->GetInlineSlot(propertyIndex)
            ...
            if (propertyObject->GetScriptContext() == requestContext)
            {
                if (!OutputExistence)
                {
                    DebugOnly(Var getPropertyValue =
                                  JavascriptOperators::GetProperty(propertyObject, propertyId, requestContext));
                    Assert(*propertyValue == getPropertyValue ||
                           (getPropertyValue == requestContext->GetLibrary()->GetNull() &&
                            requestContext->GetThreadContext()->IsDisableImplicitCall() &&
                            propertyObject->GetType()->IsExternal()));
                }

                CacheOperators::Cache<false, true, false>(false, VarTo<DynamicObject>(propertyObject), false,
                                                          propertyObject->GetType(), nullptr, propertyId, propertyIndex,
                                                          isInlineSlot, false, 0, propertyValueInfo, requestContext);
                return true;
            }
        ...
        if (PHASE_TRACE1(TypePropertyCachePhase))
        {
            CacheOperators::TraceCache(static_cast<InlineCache *>(nullptr), u"TypePropertyCache get hit prototype",
                                       propertyId, requestContext, propertyObject);
        }
#endif
...
#if DBG
        const PropertyIndex typeHandlerPropertyIndex =
            prototypeObjectWithProperty->GetDynamicType()->GetTypeHandler()->InlineOrAuxSlotIndexToPropertyIndex(
                propertyIndex, isInlineSlot);
        ...
        {
            *propertyValue = isInlineSlot ? prototypeObjectWithProperty->GetInlineSlot(propertyIndex)
        ...
        if (prototypeObjectWithProperty->GetScriptContext() == requestContext)
        {
            if (!OutputExistence)
            {
                DebugOnly(Var getPropertyValue =
                              JavascriptOperators::GetProperty(propertyObject, propertyId, requestContext));
                Assert(*propertyValue == getPropertyValue ||
                       // In some cases, such as CustomExternalObject, if implicit calls are disabled GetPropertyQuery
                       // may return null. See CustomExternalObject::GetPropertyQuery for an example.
                       (getPropertyValue == requestContext->GetMissingPropertyResult() &&
                        requestContext->GetThreadContext()->IsDisableImplicitCall() &&
                        propertyObject->GetType()->IsJsrtExternal()));
            }
            ...
            CacheOperators::Cache<false, true, false>(true, prototypeObjectWithProperty, false,
                                                      propertyObject->GetType(), nullptr, propertyId, propertyIndex,
                                                      isInlineSlot, isMissing, 0, propertyValueInfo, requestContext);
            return true;
        }
        else if (!OutputExistence)
        {
    ...
    template bool TypePropertyCache::TryGetProperty<false>(const bool checkMissing,
                                                           RecyclableObject *const propertyObject,
                                                           const PropertyId propertyId, Var *const propertyValue,
                                                           ScriptContext *const requestContext,
                                                           PropertyCacheOperationInfo *const operationInfo,
                                                           PropertyValueInfo *const propertyValueInfo);
    template bool TypePropertyCache::TryGetProperty<true>(const bool checkMissing,
                                                          RecyclableObject *const propertyObject,
                                                          const PropertyId propertyId, Var *const propertyValue,
                                                          ScriptContext *const requestContext,
                                                          PropertyCacheOperationInfo *const operationInfo,
    ...
    bool TypePropertyCache::TrySetProperty(RecyclableObject *const object, const PropertyId propertyId,
                                           Var propertyValue, ScriptContext *const requestContext,
                                           PropertyCacheOperationInfo *const operationInfo,
                                           PropertyValueInfo *const propertyValueInfo)
    {
        Assert(propertyValueInfo);
        ...
        if (!TryGetIndexForStore(propertyId, &propertyIndex, &isInlineSlot))
        {
            ...
            if (PHASE_TRACE1(TypePropertyCachePhase))
            {
                CacheOperators::TraceCache(static_cast<InlineCache *>(nullptr), u"TypePropertyCache set miss",
                                           propertyId, requestContext, object);
            }
#endif
            return false;
        }
        ...
        if (PHASE_TRACE1(TypePropertyCachePhase))
        {
            CacheOperators::TraceCache(static_cast<InlineCache *>(nullptr), u"TypePropertyCache set hit", propertyId,
...
#endif
        Assert((VarTo<DynamicObject>(object)->GetDynamicType()->GetTypeHandler()->InlineOrAuxSlotIndexToPropertyIndex(
                   propertyIndex, isInlineSlot)) == object->GetPropertyIndex(propertyId));
        ...
        {
            CacheOperators::Cache<false, false, false>(false, VarTo<DynamicObject>(object), false, object->GetType(),
                                                       nullptr, propertyId, propertyIndex, isInlineSlot, false, 0,
    ...
    void TypePropertyCache::Cache(const PropertyId id, const PropertyIndex index, const bool isInlineSlot,
                                  const bool isSetPropertyAllowed)
    {
                                  ...
                                  DynamicObject *const prototypeObjectWithProperty, Type *const myParentType)
    {
        ...
        elements[ElementIndex(id)].Cache(id, index, isInlineSlot, isSetPropertyAllowed, isMissing,
                                         prototypeObjectWithProperty, myParentType);
    }
