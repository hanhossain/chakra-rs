...
namespace Js
{
    ...
    typedef bool (*DeferredTypeInitializer)(DynamicObject *instance, DeferredTypeHandlerBase *typeHandler,
    ...
    class DynamicTypeHandler
    {
        ...
        template <DeferredTypeInitializer initializer, typename DeferredTypeFilter, bool isPrototypeTemplate,
                  uint16 _inlineSlotCapacity, uint16 _offsetOfInlineSlots>
    ...
    private:
        ...
        // * PropertyTypesWritableDataOnly - when true, the type being handled is known to have only writable data
        // properties
        // * PropertyTypesWritableDataOnlyDetection - set each time SetHasOnlyWritableDataProperties is called.

        // PropertyTypesReserved (0x1) is always on so that the uint32_t formed with the following boolean doesn't look
        // like a pointer.
        ...
        typename WriteBarrierFieldTypeTraits<uint16>::Type
            unusedBytes; // This always has it's lowest bit set to avoid false references
    ...
    public:
        ...
        DynamicTypeHandler(DynamicTypeHandler *typeHandler) :
            flags(typeHandler->flags), propertyTypes(typeHandler->propertyTypes),
            slotCapacity(typeHandler->slotCapacity), offsetOfInlineSlots(typeHandler->offsetOfInlineSlots),
            unusedBytes(typeHandler->unusedBytes), protoCachesWereInvalidated(false),
    ...
    public:
        ...
        DynamicTypeHandler(int slotCapacity, uint16 inlineSlotCapacity = 0, uint16 offsetOfInlineSlots = 0,
                           uint8_t flags = DefaultFlags);
        ...
        static void SetInstanceTypeHandler(DynamicObject *instance, DynamicTypeHandler *typeHandler,
    ...
    public:
        ...
        uint16 GetUnusedBytesValue() const { return unusedBytes >> 1; }
    ...
    public:
        ...
        static const uint8_t IsSealedOnceFlag =
            0x40; // Set state means the object is sealed, clear state means nothing (can be sealed, or not).
        static const uint8_t IsFrozenOnceFlag =
            0x80; // Set state means the object is frozen, clear state means nothing (can be frozen, or not).
    ...
    public:
        ...
        void EnsureSlots(DynamicObject *instance, int oldCount, int newCount, ScriptContext *scriptContext,
                         DynamicTypeHandler *newTypeHandler = nullptr);
...
#if ENABLE_FIXED_FIELDS
        void TraceUseFixedProperty(PropertyRecord const *propertyRecord, Var *pProperty, bool result,
                                   const char16_t *typeHandlerName, ScriptContext *requestContext);
        ...
        static bool CheckHeuristicsForFixedDataProps(DynamicObject *instance, const PropertyRecord *propertyRecord,
                                                     PropertyId propertyId, Var value);
        static bool CheckHeuristicsForFixedDataProps(DynamicObject *instance, const PropertyRecord *propertyRecord,
                                                     Var value);
    ...
    public:
        ...
        inline PropertyIndex AdjustValidSlotIndexForInlineSlots(PropertyIndex slotIndex)
        {
            ...
            return slotIndex < inlineSlotCapacity ? slotIndex + (offsetOfInlineSlots / sizeof(Var))
                                                  : slotIndex - (PropertyIndex)inlineSlotCapacity;
        }

        inline void PropertyIndexToInlineOrAuxSlotIndex(PropertyIndex propertyIndex,
                                                        PropertyIndex *inlineOrAuxSlotIndex, bool *isInlineSlot) const
        {
    ...
    protected:
        void SetFlags(uint8_t values)
        {
            ...
            Assert((this->flags & MayBecomeSharedFlag) != 0 || (values & MayBecomeSharedFlag) != 0 ||
                   (values & IsSharedFlag) == 0);

            // It's ok to set up a shared prototype type handler through a constructor (see NullTypeHandler and
            // DeferredTypeHandler), but it's not ok to change these after the fact.

            // If we isolate prototypes, don't set a prototype flag on a type handler that is shared or may become
            // shared
            Assert((this->flags & IsPrototypeFlag) != 0 || !IsolatePrototypes() ||
                   (this->flags & (MayBecomeSharedFlag | IsSharedFlag)) == 0 || (values & IsPrototypeFlag) == 0);
            // If we isolate prototypes, don't set a shared or may become shared flag on a prototype type handler.
            Assert((this->flags & IsSharedFlag) != 0 || !IsolatePrototypes() || (this->flags & IsPrototypeFlag) == 0 ||
                   (values & (MayBecomeSharedFlag | IsSharedFlag)) == 0);
        ...
        void SetFlags(uint8_t selector, uint8_t values) { SetFlags(selector & values); }
        ...
        void ChangeFlags(uint8_t selector, uint8_t values)
        {
            ...
            Assert((this->flags & IsPrototypeFlag) == 0 || (selector & IsPrototypeFlag) == 0 ||
                   (values & IsPrototypeFlag) != 0);

            // Don't set a shared flag if the type handler isn't locked.
            Assert((this->flags & IsLockedFlag) != 0 || ((selector & values) & IsLockedFlag) != 0 ||
                   ((selector & values) & IsSharedFlag) == 0);
            ...
            Assert((this->flags & IsPrototypeFlag) != 0 || !IsolatePrototypes() ||
                   (this->flags & (MayBecomeSharedFlag | IsSharedFlag)) == 0 ||
                   ((selector & values) & IsPrototypeFlag) == 0);
            // If we isolate prototypes, don't set a shared flag on a prototype type handler.
            Assert((this->flags & IsSharedFlag) != 0 || !IsolatePrototypes() || (this->flags & IsPrototypeFlag) == 0 ||
                   ((selector & values) & (MayBecomeSharedFlag | IsSharedFlag)) == 0);
    ...
    public:
        ...
        Given below is the list of  all the actions where different type handlers explicitly check for the type handler
        being locked. In most cases, when a type is evolving, if the type handler is locked a new type handler is
        created and all existing properties are copied over before evolving the type.
        1. SimpleTypeHandler
        ...
        */
        ...
        bool GetIsInlineSlotCapacityLocked() const
        {
            return (this->propertyTypes & PropertyTypesInlineSlotCapacityLocked) != 0;
        }

        void LockTypeHandler()
        {
            Assert(IsLockable());
            SetFlags(IsLockedFlag);
        }
        ...
        void SetMayBecomeShared() { SetFlags(MayBecomeSharedFlag); }

        void SetHasKnownSlot0() { SetFlags(HasKnownSlot0Flag); }

        void ClearHasKnownSlot0() { ClearFlags(HasKnownSlot0Flag); }
        ...
        bool GetHasOnlyWritableDataProperties()
        {
            return (GetPropertyTypes() & PropertyTypesWritableDataOnly) == PropertyTypesWritableDataOnly;
        }
        bool GetHasSpecialProperties()
        {
            return (GetPropertyTypes() & PropertyTypesHasSpecialProperties) == PropertyTypesHasSpecialProperties;
        }
    ...
    public:
        ...
        {
            return (GetPropertyTypes() & PropertyTypesWritableDataOnlyDetection) ==
                PropertyTypesWritableDataOnlyDetection;
        }

        BOOL Freeze(DynamicObject *instance, bool isConvertedType = false)
        {
        ...
        virtual BOOL FindNextProperty(ScriptContext *scriptContext, PropertyIndex &index,
                                      ...
                                      EnumeratorFlags flags, DynamicObject *instance, PropertyValueInfo *info) = 0;
        virtual BOOL FindNextProperty(ScriptContext *scriptContext, BigPropertyIndex &index,
                                      JavascriptString **propertyString, PropertyId *propertyId,
                                      PropertyAttributes *attributes, Type *type, DynamicType *typeToEnumerate,
        ...
        virtual bool GetPropertyEquivalenceInfo(PropertyRecord const *propertyRecord,
                                                PropertyEquivalenceInfo &info) = 0;
        virtual bool IsObjTypeSpecEquivalent(const Type *type, const Js::TypeEquivalenceRecord &record,
                                             uint &failedPropertyIndex) = 0;
        ...
        virtual BOOL HasProperty(DynamicObject *instance, PropertyId propertyId, __out_opt bool *pNoRedecl = nullptr,
                                 _Inout_opt_ PropertyValueInfo *info = nullptr) = 0;
        ...
        virtual BOOL GetProperty(DynamicObject *instance, Var originalInstance, PropertyId propertyId, Var *value,
                                 PropertyValueInfo *info, ScriptContext *requestContext) = 0;
        virtual BOOL GetProperty(DynamicObject *instance, Var originalInstance, JavascriptString *propertyNameString,
                                 Var *value, PropertyValueInfo *info, ScriptContext *requestContext) = 0;
        virtual BOOL GetInternalProperty(DynamicObject *instance, Var originalInstance, PropertyId propertyId,
                                         Var *value);
        ...
        virtual BOOL SetProperty(DynamicObject *instance, PropertyId propertyId, Var value,
                                 PropertyOperationFlags flags, PropertyValueInfo *info) = 0;
        virtual BOOL SetProperty(DynamicObject *instance, JavascriptString *propertyNameString, Var value,
                                 PropertyOperationFlags flags, PropertyValueInfo *info) = 0;
        virtual BOOL SetInternalProperty(DynamicObject *instance, PropertyId propertyId, Var value,
        ...
        virtual BOOL DeleteProperty(DynamicObject *instance, JavascriptString *propertyNameString,
                                    PropertyOperationFlags flags);
        // ===================================================================================================================
        // Special versions of the various *Property methods that recognize PropertyLetConstGlobal properties.
        // Only used for GlobalObject and ModuleRoot and so only recognized by SimpleDictionary and Dictionary type
        // handlers.
        //
        ...
        //
        virtual PropertyIndex GetRootPropertyIndex(PropertyRecord const *propertyRecord)
        {
            Throw::FatalInternalError();
        }

        virtual BOOL HasRootProperty(DynamicObject *instance, PropertyId propertyId,
                                     __out_opt bool *pNoRedecl = nullptr, __out_opt bool *pDeclaredProperty = nullptr,
                                     __out_opt bool *pNonconfigurableProperty = nullptr)
        {
            Throw::FatalInternalError();
        }
        virtual BOOL GetRootProperty(DynamicObject *instance, Var originalInstance, PropertyId propertyId, Var *value,
                                     PropertyValueInfo *info, ScriptContext *requestContext)
        {
            Throw::FatalInternalError();
        }
        virtual BOOL SetRootProperty(DynamicObject *instance, PropertyId propertyId, Var value,
                                     PropertyOperationFlags flags, PropertyValueInfo *info)
        {
            Throw::FatalInternalError();
        }
        virtual DescriptorFlags GetRootSetter(DynamicObject *instance, PropertyId propertyId, Var *setterValue,
                                              PropertyValueInfo *info, ScriptContext *requestContext)
        {
            Throw::FatalInternalError();
        }
        virtual BOOL DeleteRootProperty(DynamicObject *instance, PropertyId propertyId, PropertyOperationFlags flags)
        {
            Throw::FatalInternalError();
        }
        ...
        // accept lambdas.
        virtual bool NextLetConstGlobal(int &index, RootObjectBase *instance, const PropertyRecord **propertyRecord,
        ...
        virtual BOOL SetAccessors(DynamicObject *instance, PropertyId propertyId, Var getter, Var setter,
                                  PropertyOperationFlags flags = PropertyOperation_None) = 0;
        ...
        virtual BOOL SetPropertyWithAttributes(DynamicObject *instance, PropertyId propertyId, Var value,
                                               ...
                                               SideEffects possibleSideEffects = SideEffects_Any) = 0;
        ...
        virtual BOOL GetAttributesWithPropertyIndex(DynamicObject *instance, PropertyId propertyId,
                                                    BigPropertyIndex index, PropertyAttributes *attributes) = 0;
        ...
        virtual bool IsLengthWritable() const
        {
            Assert(false);
            return false;
        }
        virtual uint32_t SetLength(ES5Array *arr, uint32_t newLen, PropertyOperationFlags propertyOperationFlags)
        {
            Assert(false);
            return 0;
        }
        virtual BOOL IsObjectArrayFrozen(ES5Array *arr)
        {
            Assert(false);
            return FALSE;
        }
        virtual BOOL IsItemEnumerable(ES5Array *arr, uint32_t index)
        {
            Assert(false);
            return FALSE;
        }
        virtual BOOL IsValidDescriptorToken(void *descriptorValidationToken) const
        {
            Assert(false);
            return FALSE;
        }
        ...
        virtual BOOL GetDescriptor(uint32_t index, IndexPropertyDescriptor **ppDescriptor)
        {
            Assert(false);
            return FALSE;
        }
    ...
    public:
        ...
        static bool ShouldFixAnyProperties()
        {
...
#endif
    public:
        static void AdjustSlots_Jit(DynamicObject *const object, const PropertyIndex newInlineSlotCapacity,
                                    const int newAuxSlotCapacity);
        static void AdjustSlots(DynamicObject *const object, const PropertyIndex newInlineSlotCapacity,
                                const int newAuxSlotCapacity);
    ...
    protected:
        void SetPropertyUpdateSideEffect(DynamicObject *instance, PropertyId propertyId, Var value,
                                         SideEffects possibleSideEffects);
        void SetPropertyUpdateSideEffect(DynamicObject *instance, JsUtil::CharacterBuffer<char16_t> const &propertyName,
                                         Var value, SideEffects possibleSideEffects);
        bool VerifyIsExtensible(ScriptContext *scriptContext, bool alwaysThrow);

        void SetOffsetOfInlineSlots(const uint16 offsetOfInlineSlots)
        {
