...
#define DEFINE_MARSHAL_OBJECT_TO_SCRIPT_CONTEXT(T)                                                                     \
    ...
    {                                                                                                                  \
        ...
        AssertMsg(VirtualTableInfo<T>::HasVirtualTable(this),                                                          \
...
#define SetSlotArgumentsRoot(propertyId, allowLetConst, slotIndex, value) slotIndex, value
    ...
    {
        None = 0u,
        ObjectArrayFlagsTag = 1u << 0, // Tag bit used to indicate the objectArrayOrFlags field is used as flags as
                                       // opposed to object array pointer.
        HasSegmentMap = 1u << 1,
    ...
    private:
        ...
        // can improve performance for typical objects. To indicate the bitfield usage we set the least significant bit
        // to 1. Object array pointer always trumps the flags, such that when the first numeric property is added to an
        // object, its flags will be wiped out.  Hence flags can only be used as a form of cache to improve performance.
        // For functional correctness, some other fallback mechanism must exist to convey the information contained in
        // flags. This fields always starts off initialized to null.  Currently, only JavascriptArray overrides it to
        // store flags, the bits it uses are DynamicObjectFlags::AllArrayFlags. Regarding c) above, inline slots can be
        // stored within the allocation of sizeof(DynamicObject) (#3) or after sizeof(DynamicObject) (#2). This is
        // indicated by GetTypeHandler()->IsObjectHeaderInlinedTypeHandler(); when true, the inline slots are within the
        // object, and thus the union members *and* auxSlots actually contain inline slot data.
    ...
    public:
        ...
        void EnsureSlots(int oldCount, int newCount, ScriptContext *scriptContext,
    ...
    public:
        ...
        bool UsesObjectArrayOrFlagsAsFlags() const { return !!(arrayFlags & DynamicObjectFlags::ObjectArrayFlagsTag); }

        ArrayObject *GetObjectArray() const { return HasObjectArray() ? GetObjectArrayOrFlagsAsArray() : nullptr; }

        bool HasObjectArray() const
        {
            // Only JavascriptArray uses the objectArrayOrFlags as flags.
            Assert(DynamicObject::IsAnyArray((Var)this) || !UsesObjectArrayOrFlagsAsFlags() ||
                   IsObjectHeaderInlinedTypeHandler());
            return ((objectArray != nullptr) && !UsesObjectArrayOrFlagsAsFlags() &&
                    !IsObjectHeaderInlinedTypeHandler());
        }
        ...
        {
            return ((objectArray != nullptr) && !UsesObjectArrayOrFlagsAsFlags() &&
                    !IsObjectHeaderInlinedTypeHandlerUnchecked());
        }
        ...
        virtual BOOL InitProperty(PropertyId propertyId, Var value,
                                  ...
                                  PropertyValueInfo *info = nullptr) override;
        ...
        virtual BOOL GetEnumerator(JavascriptStaticEnumerator *enumerator, EnumeratorFlags flags,
                                   ScriptContext *scriptContext, EnumeratorCache *enumeratorCache = nullptr) override;
        ...
        _Check_return_ _Success_(return) virtual BOOL
            GetAccessors(PropertyId propertyId, _Outptr_result_maybenull_ Var *getter,
                         _Outptr_result_maybenull_ Var *setter, ScriptContext *requestContext) override;
        ...
        static bool IsTypeHandlerCompatibleForObjectHeaderInlining(DynamicTypeHandler *oldTypeHandler,
        ...
        BOOL FindNextProperty(BigPropertyIndex &index, JavascriptString **propertyString, PropertyId *propertyId,
                              PropertyAttributes *attributes, DynamicType *typeToEnumerate, EnumeratorFlags flags,
        ...
        BOOL GetAttributesWithPropertyIndex(PropertyId propertyId, BigPropertyIndex index,
    ...
    protected:
        BOOL GetEnumeratorWithPrefix(JavascriptEnumerator *prefixEnumerator, JavascriptStaticEnumerator *enumerator,
    ...
    private:
        ...
        BOOL CallToPrimitiveFunction(Var toPrimitiveFunction, PropertyId propertyId, Var *result,
