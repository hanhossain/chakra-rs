...
namespace Js
{
    ...
    PathTypeSuccessorKey::PathTypeSuccessorKey(const PropertyId propertyId, const ObjectSlotAttributes attributes) :
        propertyId(propertyId), attributes(attributes)
    {
    }

    bool PathTypeSuccessorKey::HasInfo() const { return propertyId != Constants::NoProperty; }

    void PathTypeSuccessorKey::Clear() { propertyId = Constants::NoProperty; }

    PropertyId PathTypeSuccessorKey::GetPropertyId() const { return propertyId; }

    ObjectSlotAttributes PathTypeSuccessorKey::GetAttributes() const { return attributes; }
    ...
    bool PathTypeSuccessorKey::operator!=(const PathTypeSuccessorKey &other) const { return !(*this == other); }

    hash_t PathTypeSuccessorKey::GetHashCode() const
    {
        return static_cast<hash_t>((propertyId << ObjectSlotAttr_BitSize) |
                                   static_cast<ObjectSlotAttr_TSize>(attributes));
    }

    bool PathTypeSuccessorInfo::GetSuccessor(const PathTypeSuccessorKey successorKey,
                                             RecyclerWeakReference<DynamicType> **typeWeakRef) const
    {
    ...
    void PathTypeSuccessorInfo::SetSuccessor(DynamicType *type, const PathTypeSuccessorKey successorKey,
                                             RecyclerWeakReference<DynamicType> *typeWeakRef,
                                             ScriptContext *scriptContext)
    {
        if (IsSingleSuccessor())
        {
            static_cast<PathTypeSingleSuccessorInfo *>(this)->SetSuccessor(type, successorKey, typeWeakRef,
                                                                           scriptContext);
        }
        else
        {
            static_cast<PathTypeMultiSuccessorInfo *>(this)->SetSuccessor(type, successorKey, typeWeakRef,
                                                                          scriptContext);
        }
    }

    void PathTypeSuccessorInfo::ReplaceSuccessor(DynamicType *type, PathTypeSuccessorKey successorKey,
                                                 RecyclerWeakReference<DynamicType> *typeWeakRef)
    {
        if (IsSingleSuccessor())
        {
    ...
    PathTypeSingleSuccessorInfo::PathTypeSingleSuccessorInfo(const PathTypeSuccessorKey key,
                                                             RecyclerWeakReference<DynamicType> *typeWeakRef) :
    ...
    PathTypeSingleSuccessorInfo *PathTypeSingleSuccessorInfo::New(const PathTypeSuccessorKey successorKey,
                                                                  RecyclerWeakReference<DynamicType> *typeWeakRef,
                                                                  ScriptContext *scriptContext)
    {
    ...
    bool PathTypeSingleSuccessorInfo::GetSuccessor(const PathTypeSuccessorKey key,
                                                   RecyclerWeakReference<DynamicType> **typeWeakRef) const
    {
    ...
    void PathTypeSingleSuccessorInfo::SetSuccessor(DynamicType *type, const PathTypeSuccessorKey key,
                                                   RecyclerWeakReference<DynamicType> *typeWeakRef,
                                                   ScriptContext *scriptContext)
    {
        ...
        PathTypeMultiSuccessorInfo *newInfo =
            PathTypeMultiSuccessorInfo::New(this->successorKey, this->successorTypeWeakRef, scriptContext);
    ...
    void PathTypeSingleSuccessorInfo::ReplaceSuccessor(DynamicType *type, const PathTypeSuccessorKey key,
                                                       RecyclerWeakReference<DynamicType> *typeWeakRef)
    {
    ...
    PathTypeMultiSuccessorInfo::PathTypeMultiSuccessorInfo(Recycler *recycler, const PathTypeSuccessorKey key,
                                                           RecyclerWeakReference<DynamicType> *typeWeakRef)
    {
    ...
    PathTypeMultiSuccessorInfo *PathTypeMultiSuccessorInfo::New(const PathTypeSuccessorKey key,
                                                                RecyclerWeakReference<DynamicType> *typeWeakRef,
                                                                ScriptContext *scriptContext)
    {
        return RecyclerNew(scriptContext->GetRecycler(), PathTypeMultiSuccessorInfo, scriptContext->GetRecycler(), key,
                           typeWeakRef);
    }

    bool PathTypeMultiSuccessorInfo::GetSuccessor(const PathTypeSuccessorKey key,
                                                  RecyclerWeakReference<DynamicType> **typeWeakRef) const
    {
    ...
    void PathTypeMultiSuccessorInfo::SetSuccessor(DynamicType *type, const PathTypeSuccessorKey key,
                                                  RecyclerWeakReference<DynamicType> *typeWeakRef,
                                                  ScriptContext *scriptContext)
    {
        Assert(this->propertySuccessors);
    ...
    void PathTypeMultiSuccessorInfo::ReplaceSuccessor(DynamicType *type, const PathTypeSuccessorKey key,
                                                      RecyclerWeakReference<DynamicType> *typeWeakRef)
    {
        Assert(this->propertySuccessors);
    ...
    PathTypeHandlerBase::PathTypeHandlerBase(TypePath *typePath, uint16 pathLength, const PropertyIndex slotCapacity,
                                             uint16 inlineSlotCapacity, uint16 offsetOfInlineSlots, bool isLocked,
                                             bool isShared, DynamicType *predecessorType) :
        DynamicTypeHandler(slotCapacity, inlineSlotCapacity, offsetOfInlineSlots,
                           DefaultFlags | (isLocked ? IsLockedFlag : 0) |
                               (isShared ? (MayBecomeSharedFlag | IsSharedFlag) : 0)),
        typePath(typePath), predecessorType(predecessorType), successorInfo(nullptr), hasUserDefinedCtor(false),
    ...
    int PathTypeHandlerBase::GetPropertyCount() { return GetPathLength(); }
    ...
    bool PathTypeHandlerBase::GetSuccessor(const PathTypeSuccessorKey successorKey,
                                           RecyclerWeakReference<DynamicType> **typeWeakRef) const
    {
    ...
    void PathTypeHandlerBase::SetSuccessor(DynamicType *type, const PathTypeSuccessorKey successorKey,
                                           RecyclerWeakReference<DynamicType> *typeWeakRef,
                                           ScriptContext *scriptContext)
    {
#if DBG
        ...
        AssertMsg(!successorType || !successorType->GetTypeHandler()->IsPathTypeHandler() ||
                      PathTypeHandlerBase::FromTypeHandler(successorType->GetTypeHandler())->GetPredecessorType() ==
                          type,
    ...
    BOOL PathTypeHandlerBase::FindNextPropertyHelper(ScriptContext *scriptContext, ObjectSlotAttributes *objectAttrs,
                                                     PropertyIndex &index, JavascriptString **propertyStringName,
                                                     ...
                                                     DynamicObject *instance, PropertyValueInfo *info)
    {
            ...
            for (; index < GetPathLength(); ++index)
            {
                    ...
                    (!!(flags & EnumeratorFlags::EnumNonEnumerable) || (attr & ObjectSlotAttr_Enumerable)))
                {
                    ...
                    *propertyStringName = propertyString;
                    ...
                    {
                        PropertyValueInfo::SetCacheInfo(info, propertyString, propertyString->GetLdElemInlineCache(),
                                                        false);
        ...
        // Need to enumerate a different type than the current one. This is because type snapshot enumerate is enabled
        // and the object's type changed since enumeration began, so need to enumerate properties of the initial type.
        ...
        {
            return typeHandlerToEnumerate->FindNextProperty(scriptContext, index, propertyStringName, propertyId,
                                                            attributes, type, typeToEnumerate, flags, instance, info);
        }

        PathTypeHandlerBase *pathTypeToEnumerate = (PathTypeHandlerBase *)typeHandlerToEnumerate;

        BOOL found =
            pathTypeToEnumerate->FindNextProperty(scriptContext, index, propertyStringName, propertyId, attributes,
                                                  typeToEnumerate, typeToEnumerate, flags, instance, info);
    ...
    BOOL PathTypeHandlerBase::SetAttributesHelper(DynamicObject *instance, PropertyId propertyId,
                                                  PropertyIndex propertyIndex, ObjectSlotAttributes *instanceAttributes,
                                                  ObjectSlotAttributes propertyAttributes, bool setAllAttributes)
    {
        if (instanceAttributes)
        {
            if (!setAllAttributes)
            {
                ...
                propertyAttributes = (ObjectSlotAttributes)(propertyAttributes |
                                                            (instanceAttributes[propertyIndex] &
                                                             ~ObjectSlotAttr_PropertyAttributesMask));
            }
...
#endif
                // This can happen if object header inlining is deoptimized, and we haven't built a full path from the
                // root. For now, just punt this case.

                if (setAllAttributes)
                {
                    // We could be trying to convert an accessor to a data property, or something similar, so do the
                    // type handler conversion here and let the caller handle setting the attributes.
                    TryConvertToSimpleDictionaryType(instance, GetPathLength());
                    return false;
                }
                else
                {
                    return TryConvertToSimpleDictionaryType(instance, GetPathLength())
                        ->SetAttributes(instance, propertyId,
                                        ObjectSlotAttributesToPropertyAttributes(propertyAttributes));
                }
            }
            predTypeHandler = PathTypeHandlerBase::FromTypeHandler(currentType->GetTypeHandler());
        }
        Assert(predTypeHandler);
        Assert(predTypeHandler->GetTypePath()->LookupInline(propertyId, predTypeHandler->GetPathLength()) ==
               Constants::NoSlot);
        ...
        while (true)
        {
            ...
            PropertyIndex index;
            currentType = currentTypeHandler->PromoteType<false>(
                currentType, PathTypeSuccessorKey(currentPropertyRecord->GetPropertyId(), currentAttributes), false,
                scriptContext, instance, &index);
            ...
            currentTypeHandler->InitializePath(
                instance, currentSlotIndex, currentTypeHandler->GetPathLength(), scriptContext, [=, this]()
                { return typePath->GetIsFixedFieldAt(currentSlotIndex, currentTypeHandler->GetPathLength()); });
#endif
#endif
            if (currentAttributes == ObjectSlotAttr_Setter)
            {
                PropertyIndex getterIndex =
                    currentTypeHandler->GetPropertyIndex(currentPropertyRecord->GetPropertyId());
...
#if ENABLE_FIXED_FIELDS
#ifdef SUPPORT_FIXED_FIELDS_ON_PATH_TYPES
    void PathTypeHandlerBase::InitializeExistingPath(const PropertyIndex slotIndex, const PropertyIndex objectSlotCount,
                                                     ScriptContext *const scriptContext)
    {
        ...
        if (typePath->GetIsUsedFixedFieldAt(slotIndex, objectSlotCount))
        {
            ...
            // field we must clear the bit. If the value was hard coded in the JIT-ed code, we must invalidate the
            // guards.
            ...
            scriptContext->GetThreadContext()->InvalidatePropertyGuards(
                typePath->GetPropertyIdUnchecked(slotIndex)->GetPropertyId());
        }
        ...
        {
            // We have now reached the most advanced instance along this path.  If this instance is not the singleton
            // instance, then the former singleton instance (if any) is no longer a singleton.  This instance could be
            // the singleton instance, if we just happen to set (overwrite) its last property.

            // This is perhaps the most fragile point of fixed fields on path types.  If we cleared the singleton
            // instance while some fields remained fixed, the instance would be collectible, and yet some code would
            // expect to see values and call methods on it.  Clearly, a recipe for disaster.  We rely on the fact that
            // we always add properties to (pre-initialized) type handlers in the order they appear on the type path. By
            // the time we reach the singleton instance, all fixed fields will have been invalidated.  Otherwise, some
            // fields could remain fixed (or even uninitialized) and we would have to spin off a loop here to invalidate
            // any remaining fixed fields - a rather unfortunate overhead.
    ...
    bool PathTypeHandlerBase::GetPropertyEquivalenceInfo(PropertyRecord const *propertyRecord,
    ...
    bool PathTypeHandlerBase::IsObjTypeSpecEquivalent(const Type *type, const TypeEquivalenceRecord &record,
                                                      uint &failedPropertyIndex)
    {
        return IsObjTypeSpecEquivalentHelper(type, nullptr, record, failedPropertyIndex);
    }

    bool PathTypeHandlerBase::IsObjTypeSpecEquivalentHelper(const Type *type, const ObjectSlotAttributes *attributes,
                                                            const TypeEquivalenceRecord &record,
                                                            uint &failedPropertyIndex)
    {
        ...
        if (absSlotIndex != Constants::NoSlot)
        {
            ...
            if (entry->mustBeWritable)
            {
                ...
                if (FixPropsOnPathTypes() &&
                    (absSlotIndex >= maxInitializedLength ||
                     this->GetTypePath()->GetIsFixedFieldAt(absSlotIndex, this->GetPathLength())))
                {
                    return false;
                }
#endif
    ...
    void PathTypeHandlerBase::SetPropertyValueInfo(PropertyValueInfo *info, RecyclableObject *instance,
                                                   PropertyIndex index, ObjectSlotAttributes attributes)
    {
        ...
        if (FixPropsOnPathTypes() &&
            (index >= this->GetTypePath()->GetMaxInitializedLength() ||
             this->GetTypePath()->GetIsFixedFieldAt(index, GetPathLength())))
        {
    ...
    BOOL PathTypeHandlerBase::GetProperty(DynamicObject *instance, Var originalInstance, PropertyId propertyId,
    ...
    BOOL PathTypeHandlerBase::GetProperty(DynamicObject *instance, Var originalInstance,
                                          ...
                                          ScriptContext *requestContext)
    {
        ...
        return PathTypeHandlerBase::GetProperty(instance, originalInstance, propertyRecord->GetPropertyId(), value,
                                                info, requestContext);
    }

    BOOL PathTypeHandlerBase::InitProperty(DynamicObject *instance, PropertyId propertyId, Var value,
                                           PropertyOperationFlags flags, PropertyValueInfo *info)
    {
        PropertyIndex index = PathTypeHandlerBase::GetPropertyIndex(propertyId);
        return SetPropertyInternal<false>(instance, propertyId, index, value, ObjectSlotAttr_Default, info, flags,
                                          SideEffects_Any, true /* IsInit */);
    }

    BOOL PathTypeHandlerBase::SetProperty(DynamicObject *instance, PropertyId propertyId, Var value,
                                          PropertyOperationFlags flags, PropertyValueInfo *info)
    {
        PropertyIndex index = PathTypeHandlerBase::GetPropertyIndex(propertyId);
        return SetPropertyInternal<false>(instance, propertyId, index, value, ObjectSlotAttr_Default, info, flags,
    ...
    BOOL PathTypeHandlerBase::SetProperty(DynamicObject *instance, JavascriptString *propertyNameString, Var value,
                                          PropertyOperationFlags flags, PropertyValueInfo *info)
    {
        // Consider: Implement actual string hash lookup
        PropertyRecord const *propertyRecord;
    ...
    void PathTypeHandlerBase::SetSlotAndCache(DynamicObject *instance, PropertyId propertyId,
                                              PropertyRecord const *propertyRecord, PropertyIndex index, Var value,
                                              ...
                                              SideEffects possibleSideEffects)
    {
        ...
        // reach this handler without us noticing and we could fail to release the old singleton instance, which may
        // later become collectible (not referenced by anything other than this handler), thus we would leak the old
        // singleton instance.
        ...
        {
#if ENABLE_FIXED_FIELDS
            Assert((instance->GetDynamicType()->GetIsShared()) ||
                   (FixPropsOnPathTypes() && instance->GetDynamicType()->GetTypeHandler()->GetIsOrMayBecomeShared()));
#endif
            // Can't assert the following.  With NewScObject we can jump to the type handler at the tip (where the
            // singleton is), even though we haven't yet initialized the properties all the way to the tip, and we don't
            // want to kill the singleton in that case yet.  It's basically a transient inconsistent state, but we have
            // to live with it. The user's code will never see the object in this state.
    ...
    BOOL PathTypeHandlerBase::SetPropertyInternal(DynamicObject *instance, PropertyId propertyId, PropertyIndex index,
                                                  Var value, ObjectSlotAttributes attr, PropertyValueInfo *info,
                                                  ...
                                                  bool isInit)
    {
        ...
        if (index != Constants::NoSlot)
        {
            // If type is shared then the handler must be shared as well.  This is a weaker invariant than in
            // AddPropertyInternal, because the type coming in here may be the result of DynamicObject::ChangeType(). In
            // that case the handler may have already been shared, but the newly created type isn't - and likely never
            // will be - shared (is typically unreachable). In CacheOperators::CachePropertyWrite we ensure that we
            // never cache property adds for types that aren't shared.
            ...
            if (setAttributes)
            {
                ...
                if (!setAttrDone)
                {
                    return instance->GetTypeHandler()->SetPropertyWithAttributes(instance, propertyId, value, attr,
            ...
            else if (isInit)
            {
                ...
                {
                    setAttrDone = this->SetAttributesHelper(
                        instance, propertyId, index, attributes,
                        (ObjectSlotAttributes)(attributes[index] & ~ObjectSlotAttr_Accessor), true);
                    if (!setAttrDone)
                    {
                        return instance->GetTypeHandler()->InitProperty(instance, propertyId, value, flags, info);
                    }
                    // We're changing an accessor into a data property at object init time. Don't cache this transition
                    // from setter to non-setter, as it behaves differently from a normal set property.
                    PropertyValueInfo::SetNoCache(info, instance);
            ...
            newTypeHandler = PathTypeHandlerBase::FromTypeHandler(instance->GetDynamicType()->GetTypeHandler());
            newTypeHandler->SetSlotAndCache(instance, propertyId, nullptr, index, value, info, flags,
                                            possibleSideEffects);
            return true;
        }
        ...
        if (scriptContext->IsNumericPropertyId(propertyId, &indexVal))
        {
            if (setAttributes)
            {
                ...
                {
                    return this->ConvertToTypeWithItemAttributes(instance)->SetItemWithAttributes(
                        instance, indexVal, value, ObjectSlotAttributesToPropertyAttributes(attr));
                }
            }
            return PathTypeHandlerBase::SetItem(instance, indexVal, value, PropertyOperation_None);
        }

        return PathTypeHandlerBase::AddPropertyInternal(instance, propertyId, value, attr, info, flags,
                                                        possibleSideEffects);
    }
    ...
    void PathTypeHandlerBase::MoveAuxSlotsToObjectHeader(DynamicObject *const object)
    {
        ...
        // When transition from ObjectHeaderInlined to auxSlot happend 2 properties where moved from inlineSlot to
        // auxSlot as we have to have space for auxSlot and objectArray (see DynamicTypeHandler::AdjustSlots). And then
        // the new property was added at 3rd index in auxSlot
        AssertMsg(this->GetUnusedBytesValue() - this->GetInlineSlotCapacity() == 3,
        ...
        Var *const oldInlineSlots =
            reinterpret_cast<Var *>(reinterpret_cast<uintptr_t>(object) + this->GetOffsetOfInlineSlots());
        typename WriteBarrierFieldTypeTraits<Var>::Type *const newInlineSlots =
            reinterpret_cast<typename WriteBarrierFieldTypeTraits<Var>::Type *>(
                reinterpret_cast<uintptr_t>(object) + this->GetOffsetOfObjectHeaderInlineSlots());

        PHASE_PRINT_TRACE1(
            ObjectHeaderInliningPhase,
            u"ObjectHeaderInlining: Re-optimizing the object. Moving auxSlots properties to ObjectHeader.\n");
        ...
        Assert(propertyIndex ==
               ((PathTypeHandlerBase *)this->GetPredecessorType()->GetTypeHandler())->GetInlineSlotCapacity());
    }
    ...
    BOOL PathTypeHandlerBase::DeleteLastProperty(DynamicObject *const object)
    {
        ...
        AssertMsg((isCurrentTypeOHI ^ isPredecessorTypeOHI) ||
                  ...
                  "When both types are ObjectHeaderInlined (or not ObjectHeaderInlined), InlineSlotCapacity of types "
                  "should match");
    ...
    BOOL PathTypeHandlerBase::DeleteProperty(DynamicObject *instance, PropertyId propertyId,
                                             PropertyOperationFlags flags)
    {
        ...
        if ((index + 1) == pathLength && this->GetPredecessorType() != nullptr && this->DeleteLastProperty(instance))
        {
            return TRUE;
        }
...
#endif
        BOOL deleteResult =
            TryConvertToSimpleDictionaryType(instance, pathLength)->DeleteProperty(instance, propertyId, flags);

        AssertMsg(deleteResult,
                  "PathType delete property can return false, this should be handled in DeleteLastProperty as well.");

        return deleteResult;
    }

    BOOL PathTypeHandlerBase::IsEnumerable(DynamicObject *instance, PropertyId propertyId) { return true; }

    BOOL PathTypeHandlerBase::IsWritable(DynamicObject *instance, PropertyId propertyId) { return true; }

    BOOL PathTypeHandlerBase::IsConfigurable(DynamicObject *instance, PropertyId propertyId) { return true; }
    ...
    BOOL PathTypeHandlerBase::SetConfigurable(DynamicObject *instance, PropertyId propertyId, BOOL value)
    {
        ...
        return SetAttributesHelper(instance, propertyId, propertyIndex, nullptr,
                                   (ObjectSlotAttributes)(ObjectSlotAttr_Default & ~ObjectSlotAttr_Configurable));
    }
    ...
    BOOL PathTypeHandlerBase::SetEnumerable(DynamicObject *instance, PropertyId propertyId, BOOL value)
    {
        ...
        return SetAttributesHelper(instance, propertyId, propertyIndex, nullptr,
                                   (ObjectSlotAttributes)(ObjectSlotAttr_Default & ~ObjectSlotAttr_Enumerable));
    }
    ...
    BOOL PathTypeHandlerBase::SetWritable(DynamicObject *instance, PropertyId propertyId, BOOL value)
    {
        ...
        return SetAttributesHelper(instance, propertyId, propertyIndex, nullptr,
                                   (ObjectSlotAttributes)(ObjectSlotAttr_Default & ~ObjectSlotAttr_Writable));
    }

    BOOL PathTypeHandlerBase::SetAccessors(DynamicObject *instance, PropertyId propertyId, Var getter, Var setter,
                                           PropertyOperationFlags flags)
    {
        return SetAccessorsHelper(instance, propertyId, nullptr, nullptr, getter, setter, flags);
    }

    BOOL PathTypeHandlerBase::SetAccessorsHelper(DynamicObject *instance, PropertyId propertyId,
                                                 ObjectSlotAttributes *attributes, PathTypeSetterSlotIndex *setters,
                                                 Var getter, Var setter, PropertyOperationFlags flags)
    {
        if (instance->GetType()->IsExternal() || instance->GetScriptContext()->IsScriptContextInDebugMode() ||
            PHASE_OFF1(ShareAccessorTypesPhase))
        {
#ifdef PROFILE_TYPES
        ...
        if (propertyIndex == Constants::NoSlot)
        {
...
#endif
                return ConvertToDictionaryType(instance)->SetItemAccessors(instance, propertyRecord->GetNumericValue(),
                                                                           getter, setter);
            }
            ...
            AddPropertyInternal(instance, propertyId, getter,
                                (ObjectSlotAttributes)(ObjectSlotAttr_Default | ObjectSlotAttr_Accessor), nullptr,
                                flags, SideEffects_None);
            newTypeHandler = PathTypeHandlerBase::FromTypeHandler(instance->GetTypeHandler());
            Assert(propertyIndex ==
                   newTypeHandler->GetTypePath()->LookupInline(propertyId, newTypeHandler->GetPathLength()));
        }
        else
        {
            ...
            if (!(attr & ObjectSlotAttr_Accessor))
            {
                ...
                if (!setters || setters[propertyIndex] == NoSetterSlot)
                {
                    ...
                    if (GetPathLength() + 1 > TypePath::MaxPathTypeHandlerLength)
                    {
                        ...
                        instance->GetScriptContext()->convertPathToDictionaryAccessorsCount++;
#endif
                        return ConvertToDictionaryType(instance)->SetAccessors(instance, propertyId, getter, setter,
                                                                               flags);
                    }
                }

                SetAttributesHelper(instance, propertyId, propertyIndex, attributes,
                                    (ObjectSlotAttributes)(attr | ObjectSlotAttr_Accessor));
                // SetAttributesHelper can convert to dictionary in corner cases, e.g., if we haven't got a full path
                // from the root. Remove this check when that's fixed.
        ...
        // The type has been changed, if necessary, and the getter value has been set. Now see if the setter index needs
        // to be set up.
        ...
        {
            // We may already have a valid setter slot, if the property has gone from accessor to data and now back to
            // accessor. Re-use the setter slot if we can.
            setters =
                PathTypeHandlerWithAttr::FromPathTypeHandler(newTypeHandler)->PathTypeHandlerWithAttr::GetSetterSlots();
            if (setters != nullptr)
            {
                setterSlot = setters[propertyIndex];
                Assert(setterSlot == NoSetterSlot || setterSlot >= GetPathLength() ||
                       attributes[setterSlot] == ObjectSlotAttr_Setter);
            }
        }
        ...
        if (setterSlot == NoSetterSlot || setterSlot >= GetPathLength())
        {
            ...
            DynamicType *newType = newTypeHandler->PromoteType<false>(
                instance->GetDynamicType(), PathTypeSuccessorKey(propertyId, ObjectSlotAttr_Setter), false,
                scriptContext, instance, &index);

            if (newType != instance->GetType())
            {
                instance->EnsureSlots(newTypeHandler->GetSlotCapacity(), newType->GetTypeHandler()->GetSlotCapacity(),
                                      scriptContext, newType->GetTypeHandler());
...
#if ENABLE_FIXED_FIELDS
#ifdef SUPPORT_FIXED_FIELDS_ON_PATH_TYPES
            newTypeHandler->InitializePath(instance, setterSlot, newTypeHandler->GetPathLength(), scriptContext,
                                           [=]()
                                           {
                                               return newTypeHandler->GetTypePath()->GetIsFixedFieldAt(
                                                   setterSlot, newTypeHandler->GetPathLength());
                                           });
#endif
#endif
        }
    ...
    BOOL PathTypeHandlerBase::PreventExtensions(DynamicObject *instance)
    {
        ...
        BOOL tempResult = this->ConvertToSharedNonExtensibleTypeIfNeededAndCallOperation(
            instance, InternalPropertyRecords::NonExtensibleType,
    ...
    BOOL PathTypeHandlerBase::Seal(DynamicObject *instance)
    {
        ...
        // Note that 2 instances can have same PathTypehandler but still different objectArray items, e.g. {x:0, 0:0}
        // and {x:0, 1:0}. Technically we could change SimpleDictionaryTypehandler to override *Item* methods, similar
        // to DictionaryTypeHandler, but objects with numeric properties are currently seen as low priority, so just
        // don't share the type.
        ...
        {
            return this->ConvertToSharedNonExtensibleTypeIfNeededAndCallOperation(
                instance, InternalPropertyRecords::SealedType,
                [&](SimpleDictionaryTypeHandlerWithNonExtensibleSupport *newTypeHandler)
    ...
    BOOL PathTypeHandlerBase::FreezeImpl(DynamicObject *instance, bool isConvertedType)
    {
        ...
        {
            return this->ConvertToSharedNonExtensibleTypeIfNeededAndCallOperation(
                instance, InternalPropertyRecords::FrozenType,
                [&](SimpleDictionaryTypeHandlerWithNonExtensibleSupport *newTypeHandler)
                {
                    return newTypeHandler->FreezeInternal(
                        instance, true); // true: we don't want to change type in FreezeInternal.
                });
        }
    }

    // Checks whether conversion to shared type is needed and performs it, then calls actual operation on the shared
    // type. Template method used for PreventExtensions, Seal, Freeze. Parameters:
    ...
    // - FType: functor/lambda to perform actual forced operation (such as PreventExtensionsInternal) on the shared
    // type.
    template <typename FType>
    BOOL PathTypeHandlerBase::ConvertToSharedNonExtensibleTypeIfNeededAndCallOperation(
        DynamicObject *instance, const PropertyRecord *operationInternalPropertyRecord, FType operation)
    {
        ...
        if (!GetSuccessor(key, &newTypeWeakRef) || newTypeWeakRef->Get() == nullptr)
        {
            ...
            SimpleDictionaryTypeHandlerWithNonExtensibleSupport *newTypeHandler =
                ConvertToSimpleDictionaryType<SimpleDictionaryTypeHandlerWithNonExtensibleSupport>(
                    instance, this->GetPathLength(), true);
    ...
    DynamicType *PathTypeHandlerBase::PromoteType(DynamicObject *instance, const PathTypeSuccessorKey key,
                                                  PropertyIndex *propertyIndex)
    {
        ...
        DynamicType *nextType =
            this->PromoteType<false>(currentType, key, false, scriptContext, instance, propertyIndex);
        PathTypeHandlerBase *nextPath = (PathTypeHandlerBase *)nextType->GetTypeHandler();

        instance->EnsureSlots(this->GetSlotCapacity(), nextPath->GetSlotCapacity(), scriptContext,
                              nextType->GetTypeHandler());
    ...
    T *PathTypeHandlerBase::ConvertToTypeHandler(DynamicObject *instance)
    {
        ...
        T *newTypeHandler =
            RecyclerNew(recycler, T, recycler, oldTypeHandler->GetSlotCapacity(),
                        oldTypeHandler->GetInlineSlotCapacity(), oldTypeHandler->GetOffsetOfInlineSlots());
        ...
        oldTypeHandler->TraceFixedFieldsBeforeTypeHandlerChange(u"converting", u"PathTypeHandler",
                                                                u"DictionaryTypeHandler", instance, oldTypeHandler,
                                                                oldType, oldSingletonInstance);
        ...
        if (canBeSingletonInstance)
        {
            ...
            {
                RecyclerWeakReference<DynamicObject> *curSingletonInstance =
                    oldTypeHandler->GetTypePath()->GetSingletonInstance();
        ...
        // If we are a prototype or may become a prototype we must transfer used as fixed bits.  See point 4 in
        // ConvertToSimpleDictionaryType.
        Assert(!DynamicTypeHandler::IsolatePrototypes() || ((oldTypeHandler->GetFlags() & IsPrototypeFlag) == 0));
        bool transferUsedAsFixed =
            ((oldTypeHandler->GetFlags() & IsPrototypeFlag) != 0 ||
             (oldTypeHandler->GetIsOrMayBecomeShared() && !DynamicTypeHandler::IsolatePrototypes()));
#endif
        ...
        for (PropertyIndex i = 0; i < oldTypeHandler->GetPathLength(); i++)
        {
            ...
            {
                // Adding a setter. Don't add another descriptor. Find the getter and convert its descriptor, which will
                // cause the setter to get the next free slot.
                DictionaryPropertyDescriptor<PropertyIndex> *descriptor;
                [[maybe_unused]] bool result =
                    newTypeHandler->propertyMap->TryGetReference(propertyRecord, &descriptor);
                Assert(result);
                ...
                Assert(oldTypeHandler->GetSetterSlotIndex(descriptor->GetDataPropertyIndex<false>()) ==
                       newTypeHandler->nextPropertyIndex);
            ...
            {
#if ENABLE_FIXED_FIELDS
                // Consider: As noted in point 2 in ConvertToSimpleDictionaryType, when converting to non-shared handler
                // we could be more aggressive and mark every field as fixed, because we will always take a type
                // transition. We have to remember to respect the switches as to which kinds of properties we should
                // fix, and for that we need the values from the instance. Even if the type handler says the property is
                // initialized, the current instance may not have a value for it. Check for value != null.
                if (PathTypeHandlerBase::FixPropsOnPathTypes())
                {
                    newTypeHandler->Add(
                        propertyRecord, ObjectSlotAttributesToPropertyAttributes(attr),
...
#endif
                {
                    newTypeHandler->Add(propertyRecord, ObjectSlotAttributesToPropertyAttributes(attr), true, false,
                                        false, scriptContext);
                }
            }
        }
        ...
        Assert(!DynamicTypeHandler::IsolatePrototypes() || ((oldTypeHandler->GetFlags() & IsPrototypeFlag) == 0));
        AssertMsg(!newTypeHandler->GetIsPrototype(),
        ...
        // Any new type handler we expect to see here should have inline slot capacity locked.  If this were to change,
        // we would need to update our shrinking logic (see ShrinkSlotAndInlineSlotCapacity).
        ...
        newTypeHandler->SetPropertyTypes(PropertyTypesWritableDataOnly | PropertyTypesWritableDataOnlyDetection |
                                         ...
                                         oldTypeHandler->GetPropertyTypes());
        newTypeHandler->SetInstanceTypeHandler(instance);

#if ENABLE_FIXED_FIELDS
        Assert(!newTypeHandler->HasSingletonInstance() || !instance->HasSharedType());

        PathTypeHandlerBase::TraceFixedFieldsAfterTypeHandlerChange(instance, oldTypeHandler, newTypeHandler, oldType,
                                                                    instance->GetDynamicType(), oldSingletonInstance);
#endif
    ...
    template <typename T>
    DynamicTypeHandler *PathTypeHandlerBase::TryConvertToSimpleDictionaryType(DynamicObject *instance,
    ...
    template <typename T>
    T *PathTypeHandlerBase::ConvertToSimpleDictionaryType(DynamicObject *instance, int propertyCapacity,
                                                          bool mayBecomeShared)
    {
        Assert(CanConvertToSimpleDictionaryType());
        Assert(instance);
...
#endif
        T *newTypeHandler =
            RecyclerNew(recycler, T, recycler, oldTypeHandler->GetSlotCapacity(), propertyCapacity,
                        oldTypeHandler->GetInlineSlotCapacity(), oldTypeHandler->GetOffsetOfInlineSlots());
        // We expect the new type handler to start off marked as having only writable data properties.
        Assert(newTypeHandler->GetHasOnlyWritableDataProperties());

        // Care must be taken to correctly set up fixed field bits whenever a type's handler is changed.  Exactly what
        // needs to be done depends on whether the current handler is shared, whether the new handler is shared, whether
        // the current handler has the prototype flag set, and even whether we take a type transition as part of the
        // process.
        //
        // 1. Can we set fixed bits on new handler for the fields that are marked as fixed on current handler?
        //
        //    Yes, if the new type handler is not shared.  If the handler is not shared, we know that only this instance
        //    will ever use it.  Otherwise, a different instance could transition to the same type handler, but have
        //    different values for fields marked as fixed.
        //
        // 2. Can we set fixed bits on new handler even for the fields that are not marked as fixed on current handler?
        //
        //    Yes, if the new type handler is not shared and we take a type transition during conversion.  The first
        //    condition is required for the same reason as in point 1 above.  The type transition is needed to ensure
        //    that any store field fast paths for this instance get invalidated.  If they didn't, then the newly fixed
        //    field could get overwritten on the fast path without triggering necessary invalidation.
        //
        //    Note that it's desirable to mark additional fields as fixed (particularly when the instance becomes a
        //    prototype) to counteract the effect of false type sharing, which may unnecessarily turn off some fixed
        //    field bits.
        //
        // 3. Do we need to clear any fixed field bits on the old or new type handler?
        //
        //    Yes, we must clear fixed fields bits for properties that aren't also used as fixed, but only if both type
        //    handlers are shared and we don't isolate prototypes.  This is rather tricky and results from us
        //    pre-creating certain handlers even before any instances actually have values for all represented
        //    properties.  We must avoid the situation, in which one instance switched to a new type handler with some
        //    fixed field not yet used as fixed, and later the second instance follows the same handler evolution with
        //    the same field used as fixed.  Upon switching to the new handler the second instance would "forget" that
        //    the field was used as fixed and fail to invalidate when overwritten.
        //
        //    Example: Instance A with TH1 has a fixed method FOO, which has not been used as fixed yet.  Then instance
        //    B gets pre-created and lands on TH1 (and so far assumes FOO is fixed).  As B's pre-creation continues, it
        //    moves to TH2, but thus far FOO has not been used as fixed.  Now instance A becomes a prototype, and its
        //    method FOO is used in a hard-coded JIT sequence, thus marking it as used as fixed.  Instance A then
        //    transitions to TH2 and we lose track of FOO being used as fixed.  If FOO is then overwritten on A, the
        //    hard-coded JIT sequence does not get invalidated and continues to call the old method FOO.
        //
        // 4. Can we avoid setting used as fixed bits on new handler for fields marked as used as fixed on current
        // handler?
        //
        //    Yes, if the current type handler doesn't have the prototype flag and current handler is not shared or new
        //    handler is not shared or we isolate prototypes, and we take a type transition as part of the conversion.
        //
        //    Type transition ensures that any field loads from the instance are invalidated (including
        //    any that may have hard-coded the fixed field's value).  Hence, if the fixed field on this instance were to
        //    be later overwritten it will not cause any functional issues.  On the other hand, field loads from
        //    prototype are not affected by the prototype object's type change.  Therefore, if this instance is a
        //    prototype we must carry the used as fixed bits forward to ensure that if we overwrite any fixed field we
        //    explicitly trigger invalidation.
        //
        //    Review: Actually, the comment below is overly conservative.  If the second instance that became a
        //    prototype followed the same type evolution path, it would have to have invalidated all fixed fields, so
        //    there should be no need to transfer used as fixed bits, unless the current instance is already a
        //    prototype. In addition, if current handler is shared and the new handler is shared, a different instance
        //    with the current handler may later become a prototype (if we don't isolate prototypes) and follow the same
        //    conversion to the new handler, even if the current instance is not a prototype.  Hence, the new type
        //    handler must retain the used as fixed bits, so that proper invalidation can be triggered later, if
        //    overwritten.
        //
        //    Note that this may lead to the new type handler with some fields not marked as fixed, but marked as used
        //    as fixed.
        //
        //    Note also that if we isolate prototypes, we guarantee that no prototype instance will share a type handler
        //    with any other instance.  Hence, the problem sequence above could not take place.
        //
        // 5. Do we need to invalidate JIT-ed code for any fields marked as used as fixed on current handler?
        //
        //    No.  With the rules above any necessary invalidation will be triggered when the value actually gets
        //    overwritten.
        //
        ...
        RecyclerWeakReference<DynamicObject> *oldSingletonInstance = oldTypeHandler->GetSingletonInstance();
        oldTypeHandler->TraceFixedFieldsBeforeTypeHandlerChange(u"converting", u"PathTypeHandler",
                                                                u"SimpleDictionaryTypeHandler", instance,
                                                                oldTypeHandler, oldType, oldSingletonInstance);

        bool const canBeSingletonInstance = DynamicTypeHandler::CanBeSingletonInstance(instance);
        // If this type had been installed on a stack instance it shouldn't have a singleton Instance
        Assert(canBeSingletonInstance || !oldTypeHandler->HasSingletonInstance());

        // Consider: It looks like we're delaying sharing of these type handlers until the second instance arrives, so
        // we could set the singleton here and zap it later.
        ...
        // It would be nice to transfer fixed fields if the new type handler may become fixed later (but isn't yet).
        // This would allow singleton instances to retain fixed fields.  It would require that when we do actually share
        // the target type (when the second instance arrives), we clear (and invalidate, if necessary) any fixed fields.
        ...
        Assert(!DynamicTypeHandler::IsolatePrototypes() || ((oldTypeHandler->GetFlags() & IsPrototypeFlag) == 0));
        // For the global object we don't emit a type check before a hard-coded use of a fixed field.  Therefore a type
        // transition isn't sufficient to invalidate any used fixed fields, and we must continue tracking them on the
        // new type handler.  The global object should never have a path type handler.
        Assert(instance->GetTypeId() != TypeIds_GlobalObject);
        // If the type isn't locked, we may not change the type of the instance, and we must also track the used fixed
        // fields on the new handler.
        bool transferUsedAsFixed = !instance->GetDynamicType()->GetIsLocked() ||
            ((oldTypeHandler->GetFlags() & IsPrototypeFlag) != 0 ||
             (oldTypeHandler->GetIsOrMayBecomeShared() && !DynamicTypeHandler::IsolatePrototypes()));
#endif

        // Consider: As noted in point 2 above, when converting to non-shared SimpleDictionaryTypeHandler we could be
        // more aggressive and mark every field as fixed, because we will always take a type transition.  We have to
        // remember to respect the switches as to which kinds of properties we should fix, and for that we need the
        // values from the instance.  Even if the type handler says the property is initialized, the current instance
        // may not have a value for it.  Check for value != null.
        ...
        for (PropertyIndex i = 0; i < oldTypeHandler->GetPathLength(); i++)
        {
            ...
            if (PathTypeHandlerBase::FixPropsOnPathTypes())
            {
                ...
                newTypeHandler->Add(
                    typePath->GetPropertyId(i),
                    attributes ? ObjectSlotAttributesToPropertyAttributes(attributes[i]) : PropertyDynamicTypeDefaults,
                    i < typePath->GetMaxInitializedLength(),
                    transferIsFixed && typePath->GetIsFixedFieldAt(i, GetPathLength()),
                    transferUsedAsFixed && typePath->GetIsUsedFixedFieldAt(i, GetPathLength()), scriptContext);
            }
            else
#endif
            {
                newTypeHandler->Add(oldTypeHandler->GetTypePath()->GetPropertyId(i),
                                    attributes ? ObjectSlotAttributesToPropertyAttributes(attributes[i])
                                               : PropertyDynamicTypeDefaults,
                                    true, false, false, scriptContext);
            }

            // No need to clear fixed fields not used as fixed, because we never convert during pre-creation of type
            // handlers and we always add properties in order they appear on the type path.  Hence, any existing fixed
            // fields will be turned off by any other instance following this type path.  See point 3 above.
        }

#ifdef SUPPORT_FIXED_FIELDS_ON_PATH_TYPES
        // Clear the singleton from this handler regardless of mayBecomeShared, because this instance no longer uses
        // this handler.
        if (PathTypeHandlerBase::FixPropsOnPathTypes())
        {
            Assert(oldTypeHandler->HasSingletonInstanceOnlyIfNeeded());
        ...
        Assert(!DynamicTypeHandler::IsolatePrototypes() || !oldTypeHandler->GetIsOrMayBecomeShared() ||
               ((oldTypeHandler->GetFlags() & IsPrototypeFlag) == 0));
        AssertMsg((newTypeHandler->GetFlags() & IsPrototypeFlag) == 0,
                  "Why did we create a brand new type handler with a prototype flag set?");
        newTypeHandler->SetFlags(IsPrototypeFlag, oldTypeHandler->GetFlags());
...
#if ENABLE_FIXED_FIELDS
        Assert(!newTypeHandler->HasSingletonInstance() || !instance->HasSharedType());
        // We assumed that we don't need to transfer used as fixed bits unless we are a prototype, which is only valid
        // if we also changed the type.
    ...
    BOOL PathTypeHandlerBase::SetPropertyWithAttributes(DynamicObject *instance, PropertyId propertyId, Var value,
                                                        ...
                                                        PropertyOperationFlags flags, SideEffects possibleSideEffects)
    {
        if (!ObjectSlotAttributesContains(attributes) || PHASE_OFF1(ShareTypesWithAttributesPhase))
        {
            ...
            return TryConvertToSimpleDictionaryType(instance, GetPathLength() + 1)
                ->SetPropertyWithAttributes(instance, propertyId, value, attributes, info, flags, possibleSideEffects);
        }

        PropertyIndex index = PathTypeHandlerBase::GetPropertyIndex(propertyId);
        return PathTypeHandlerBase::SetPropertyInternal<true>(instance, propertyId, index, value,
                                                              PropertyAttributesToObjectSlotAttributes(attributes),
                                                              info, flags, possibleSideEffects);
    }

    BOOL PathTypeHandlerBase::SetAttributes(DynamicObject *instance, PropertyId propertyId,
                                            PropertyAttributes attributes)
    {
        if (!ObjectSlotAttributesContains(attributes) || PHASE_OFF1(ShareTypesWithAttributesPhase))
        {
            ...
            return TryConvertToSimpleDictionaryType(instance, GetPathLength())
                ->SetAttributes(instance, propertyId, attributes);
        }
        ...
        if (propertyIndex == Constants::NoSlot)
        {
            if (instance->HasObjectArray() && attributes != PropertyDynamicTypeDefaults)
            {
                ...
                if (propertyRecord->IsNumeric())
                {
                    this->ConvertToTypeWithItemAttributes(instance)->SetItemAttributes(
                        instance, propertyRecord->GetNumericValue(), attributes);
                }
            }
            return true;
        }
    ...
    BOOL PathTypeHandlerBase::SetAttributesAtIndex(DynamicObject *instance, PropertyId propertyId, PropertyIndex index,
                                                   PropertyAttributes attributes)
    {
        ...
        return SetAttributesHelper(instance, propertyId, index, GetAttributeArray(),
                                   PropertyAttributesToObjectSlotAttributes(attributes));
    }

    BOOL PathTypeHandlerBase::GetAttributesWithPropertyIndex(DynamicObject *instance, PropertyId propertyId,
    ...
    bool PathTypeHandlerBase::UsePathTypeHandlerForObjectLiteral(const PropertyIdArray *const propIds,
    ...
    DynamicType *PathTypeHandlerBase::CreateTypeForNewScObject(ScriptContext *scriptContext, DynamicType *type,
                                                               const Js::PropertyIdArray *propIds, bool shareType)
    {
        Assert(scriptContext);
        ...
        if (UsePathTypeHandlerForObjectLiteral(propIds, &check__proto__))
        {
            ...
            for (uint i = 0; i < count; i++)
            {
...
#ifdef PROFILE_OBJECT_LITERALS
                {
                    ...
                    if (!pathHandler->GetSuccessor(PathTypeSuccessorKey(propertyId, ObjectSlotAttr_Default),
                                                   &nextTypeWeakRef) ||
                        nextTypeWeakRef->Get() == nullptr)
                    {
                        scriptContext->objectLiteralPathCount++;
                    }
                }
#endif
                type = pathHandler->PromoteType<true>(type, PathTypeSuccessorKey(propertyId, ObjectSlotAttr_Default),
                                                      shareType, scriptContext, nullptr, &propertyIndex);
            }
        }
        else if (count <= static_cast<uint>(SimpleDictionaryTypeHandler::MaxPropertyIndexSize))
        {
            type = SimpleDictionaryTypeHandler::CreateTypeForNewScObject(scriptContext, type, propIds, shareType,
                                                                         check__proto__);
        }
        else if (count <= static_cast<uint>(BigSimpleDictionaryTypeHandler::MaxPropertyIndexSize))
        {
            type = BigSimpleDictionaryTypeHandler::CreateTypeForNewScObject(scriptContext, type, propIds, shareType,
                                                                            check__proto__);
        }
        else
        {
    ...
    DynamicType *PathTypeHandlerBase::CreateNewScopeObject(ScriptContext *scriptContext, DynamicType *type,
                                                           const PropertyIdArray *propIds,
                                                           ...
                                                           uint extraAttributesSlotCount)
    {
        ...
        for (uint i = 0; i < count; i++)
        {
            ...
            const PropertyRecord *propertyRecord =
                propertyId == Constants::NoProperty ? NULL : scriptContext->GetPropertyName(propertyId);
            // This will add the property as initialized and non-fixed.  That's fine because we will populate the
            // property values on the scope object right after this (see JavascriptOperators::OP_InitCachedScope).  We
            // will not treat these properties as fixed.
        ...
        AssertMsg((typeHandler->GetFlags() & IsPrototypeFlag) == 0,
    ...
    template DynamicType *PathTypeHandlerBase::CreateNewScopeObject<true>(ScriptContext *scriptContext,
                                                                          DynamicType *type,
                                                                          const PropertyIdArray *propIds,
                                                                          PropertyAttributes extraAttributes,
                                                                          uint extraAttributesSlotCount);
    template DynamicType *PathTypeHandlerBase::CreateNewScopeObject<false>(ScriptContext *scriptContext,
                                                                           DynamicType *type,
                                                                           const PropertyIdArray *propIds,
                                                                           PropertyAttributes extraAttributes,
                                                                           uint extraAttributesSlotCount);
    ...
    DynamicType *PathTypeHandlerBase::PromoteType(DynamicType *predecessorType, const PathTypeSuccessorKey key,
                                                  ...
                                                  PropertyIndex *propertyIndex)
    {
        ...
        if (!GetSuccessor(key, &nextTypeWeakRef) || (nextType = nextTypeWeakRef->Get()) == nullptr)
        {
            ...
            TraceFixedFieldsBeforeTypeHandlerChange(branching ? u"branching" : u"advancing", u"PathTypeHandler",
                                                    u"PathTypeHandler", instance, this, oldType, oldSingletonInstance);
#endif
            ...
            if (branching)
            {
                ...
                if (oldSetters)
                {
                    newTypePath = GetTypePath()->Branch<true>(
                        recycler, GetPathLength(), GetIsOrMayBecomeShared() && !IsolatePrototypes(), oldAttributes);
                }
                else
                {
                    newTypePath = GetTypePath()->Branch<false>(recycler, GetPathLength(),
                ...
                {
                    newAttributes =
                        this->UpdateAttributes(recycler, oldAttributes, oldPathSize, newTypePath->GetPathSize());
                }
            ...
            if (key.GetAttributes() == ObjectSlotAttr_Default && oldAttributes == nullptr)
            {
                nextPath = PathTypeHandlerNoAttr::New(scriptContext, newTypePath, newPropertyCount, newSlotCapacity,
                                                      newInlineSlotCapacity, newOffsetOfInlineSlots, true,
                                                      markTypeAsShared, predecessorType);
            }
            else
            {
                ...
                nextPath =
                    PathTypeHandlerWithAttr::New(scriptContext, newTypePath, newAttributes, newSetters, newSetterCount,
                                                 newPropertyCount, newSlotCapacity, newInlineSlotCapacity,
                                                 newOffsetOfInlineSlots, true, markTypeAsShared, predecessorType);
            }
            if (!markTypeAsShared)
                nextPath->SetMayBecomeShared();
            Assert(nextPath->GetHasOnlyWritableDataProperties());
            nextPath->CopyPropertyTypes(PropertyTypesWritableDataOnly | PropertyTypesWritableDataOnlyDetection |
                                            PropertyTypesHasSpecialProperties,
                                        GetPropertyTypes());
            ...
            if (isObjectLiteral)
            {
                ...
                nextType = RecyclerNew(recycler, DynamicType, predecessorType, nextPath, /* isLocked = */ true,
                                       /* isShared = */ markTypeAsShared);
            }
            else
            {
                // The new type isn't shared yet.  We will make it shared when the second instance attains it.
            ...
            SetSuccessor(predecessorType, key, recycler->CreateWeakReferenceHandle<DynamicType>(nextType),
                         scriptContext);
...
#if ENABLE_FIXED_FIELDS
            Assert(!FixPropsOnPathTypes() || shareType ||
                   nextPath->GetPathLength() > newTypePath->GetMaxInitializedLength());
...
#if ENABLE_FIXED_FIELDS
            Assert((FixPropsOnPathTypes() && nextPath->GetMayBecomeShared()) ||
                   (nextPath->GetIsShared() && nextType->GetIsShared()));
        ...
        Assert(this->GetHasOnlyWritableDataProperties() == nextPath->GetHasOnlyWritableDataProperties() ||
               !(key.GetAttributes() & ObjectSlotAttr_Writable) || (key.GetAttributes() & ObjectSlotAttr_Accessor));
        Assert(this->GetIsInlineSlotCapacityLocked() == nextPath->GetIsInlineSlotCapacityLocked());
        nextPath->SetPropertyTypes(PropertyTypesWritableDataOnlyDetection | PropertyTypesHasSpecialProperties,
        ...
        PropertyAttributes isWritableAttribute =
            ...
            ? PropertyWritable
            : PropertyNone;
        library->GetTypesWithOnlyWritablePropertyProtoChainCache()->ProcessProperty(nextPath, isWritableAttribute,
                                                                                    propertyRecord, scriptContext);
        library->GetTypesWithNoSpecialPropertyProtoChainCache()->ProcessProperty(nextPath, isWritableAttribute,
                                                                                 propertyRecord, scriptContext);

        (*propertyIndex) = index;

#if DBG
        AssertMsg(nextPath->GetPredecessorType()->GetTypeHandler() == this,
    ...
    ObjectSlotAttributes *PathTypeHandlerBase::UpdateAttributes(Recycler *recycler, ObjectSlotAttributes *oldAttributes,
                                                                uint8_t oldPathSize, uint8_t newTypePathSize)
    {
    ...
    PathTypeSetterSlotIndex *PathTypeHandlerBase::UpdateSetterSlots(Recycler *recycler,
    ...
    void PathTypeHandlerBase::ResetTypeHandler(DynamicObject *instance)
    {
#ifdef PROFILE_TYPES
    ...
    void PathTypeHandlerBase::SetAllPropertiesToUndefined(DynamicObject *instance, bool invalidateFixedFields)
    {
        ...
        // Before using for other purposes, make sure the assumptions made here make sense in the new context.  In
        // particular, the invalidateFixedFields == false is only correct if a) the object is known not to have any, or
        // b) the type of the object has changed and/or property guards have already been invalidated through some other
        // means.
    ...
    void PathTypeHandlerBase::MarshalAllPropertiesToScriptContext(DynamicObject *instance,
                                                                  ...
                                                                  bool invalidateFixedFields)
    {
        ...
        {
            SetSlotUnchecked(instance, slotIndex,
                             CrossSite::MarshalVar(targetScriptContext, GetSlot(instance, slotIndex)));
        }
    }

    BOOL PathTypeHandlerBase::AddProperty(DynamicObject *instance, PropertyId propertyId, Js::Var value,
                                          PropertyAttributes attributes, PropertyValueInfo *info,
                                          PropertyOperationFlags flags, SideEffects possibleSideEffects)
    {
        ...
        return AddPropertyInternal(instance, propertyId, value, PropertyAttributesToObjectSlotAttributes(attributes),
                                   info, flags, possibleSideEffects);
    }

    BOOL PathTypeHandlerBase::AddPropertyInternal(DynamicObject *instance, PropertyId propertyId, Js::Var value,
                                                  ObjectSlotAttributes attr, PropertyValueInfo *info,
                                                  PropertyOperationFlags flags, SideEffects possibleSideEffects)
    {
        ScriptContext *scriptContext = instance->GetScriptContext();
...
#endif
            return TryConvertToSimpleDictionaryType(instance, GetPathLength() + 1)
                ->SetPropertyWithAttributes(instance, propertyId, value, ObjectSlotAttributesToPropertyAttributes(attr),
                                            info, PropertyOperation_None, possibleSideEffects);
        }
        ...
        newTypeHandler->SetSlotAndCache(instance, propertyId, propertyRecord, index, value, info, flags,
                                        possibleSideEffects);
    ...
    bool PathTypeHandlerBase::GetMaxPathLength(uint16 *maxPathLength)
    {
        ...
        bool result = true;
        this->MapSuccessorsUntil(
            [&result, maxPathLength](PathTypeSuccessorKey,
                                     RecyclerWeakReference<DynamicType> *typeWeakReference) -> bool
            {
    ...
    void PathTypeHandlerBase::ShrinkSlotAndInlineSlotCapacity()
    {
        if (!GetIsInlineSlotCapacityLocked())
        {
            ...
            {
                uint16 newInlineSlotCapacity = IsObjectHeaderInlinedTypeHandler()
    ...
    void PathTypeHandlerBase::ShrinkSlotAndInlineSlotCapacity(uint16 newInlineSlotCapacity)
    {
        ...
        this->MapSuccessors(
            [newInlineSlotCapacity](PathTypeSuccessorKey, RecyclerWeakReference<DynamicType> *typeWeakReference)
            {
                DynamicType *type = typeWeakReference->Get();
                if (type)
                {
                    PathTypeHandlerBase::FromTypeHandler(type->GetTypeHandler())
                        ->ShrinkSlotAndInlineSlotCapacity(newInlineSlotCapacity);
                }
            });
    }
    ...
    void PathTypeHandlerBase::LockInlineSlotCapacity()
    {
        ...
        this->MapSuccessors(
            [](const PathTypeSuccessorKey, RecyclerWeakReference<DynamicType> *typeWeakReference)
            {
                DynamicType *type = typeWeakReference->Get();
                if (!type)
                {
            ...
            this->MapSuccessors(
                [](const PathTypeSuccessorKey, RecyclerWeakReference<DynamicType> *typeWeakReference)
                {
                    ...
                    successorTypeHandler->IsPathTypeHandler()
                        ? PathTypeHandlerBase::FromTypeHandler(successorTypeHandler)
                              ->EnsureInlineSlotCapacityIsLocked(false)
                        : successorTypeHandler->EnsureInlineSlotCapacityIsLocked();
                });
        }
    }

    void PathTypeHandlerBase::VerifyInlineSlotCapacityIsLocked() { VerifyInlineSlotCapacityIsLocked(true); }
    ...
    void PathTypeHandlerBase::VerifyInlineSlotCapacityIsLocked(bool startFromRoot)
    {
        ...
        this->MapSuccessors(
            [](const PathTypeSuccessorKey, RecyclerWeakReference<DynamicType> *typeWeakReference)
            {
                ...
                DynamicTypeHandler *successorTypeHandler = type->GetTypeHandler();
                successorTypeHandler->IsPathTypeHandler() ? PathTypeHandlerBase::FromTypeHandler(successorTypeHandler)
                                                                ->VerifyInlineSlotCapacityIsLocked(false)
    ...
    PathTypeHandlerBase *PathTypeHandlerBase::DeoptimizeObjectHeaderInlining(JavascriptLibrary *const library)
    {
        ...
        // We don't copy the fixed fields, as we will be sharing this type anyways later and the fixed fields vector has
        // to be invalidated.
        PathTypeHandlerBase *clonedTypeHandler;
        if (attributes == nullptr)
        {
            clonedTypeHandler = PathTypeHandlerNoAttr::New(
                library->GetScriptContext(), clonedPath, GetPathLength(), static_cast<PropertyIndex>(GetSlotCapacity()),
                GetInlineSlotCapacity() - GetObjectHeaderInlinableSlotCapacity(), sizeof(DynamicObject), false, false);
        }
        else
        {
            ...
            clonedTypeHandler = PathTypeHandlerWithAttr::New(
                library->GetScriptContext(), clonedPath, newAttributes, newSetters, GetSetterCount(), GetPathLength(),
                static_cast<PropertyIndex>(GetSlotCapacity()),
                GetInlineSlotCapacity() - GetObjectHeaderInlinableSlotCapacity(), sizeof(DynamicObject), false, false);
        }
        clonedTypeHandler->SetMayBecomeShared();
        clonedTypeHandler->CopyPropertyTypes(PropertyTypesWritableDataOnly | PropertyTypesWritableDataOnlyDetection |
                                                 PropertyTypesHasSpecialProperties,
                                             this->GetPropertyTypes());
    ...
    PathTypeHandlerBase *PathTypeHandlerBase::BuildPathTypeFromNewRoot(DynamicObject *instance, DynamicType **typeRef)
    {
        Assert(typeRef);
        DynamicType *type = *typeRef;
        ScriptContext *scriptContext = type->GetScriptContext();

        PathTypeHandlerBase *newTypeHandler = PathTypeHandlerNoAttr::New(
            scriptContext, scriptContext->GetLibrary()->GetRootPath(), 0,
            static_cast<PropertyIndex>(this->GetSlotCapacity()), this->GetInlineSlotCapacity(),
            this->GetOffsetOfInlineSlots(), GetIsLocked(), GetIsShared());
        ...
        for (PropertyIndex propertyIndex = 0; propertyIndex < GetPathLength(); propertyIndex++)
        {
            ...
            ObjectSlotAttributes attr = attributes ? attributes[propertyIndex] : ObjectSlotAttr_Default;
            type = newTypeHandler->PromoteType<false>(type, PathTypeSuccessorKey(propertyId, attr), true, scriptContext,
            ...
            if (attr == ObjectSlotAttr_Setter)
            {
                ...
                {
                    newTypeHandler->SetSetterSlot(getterIndex,
                                                  (PathTypeSetterSlotIndex)(newTypeHandler->GetPathLength() - 1));
                }
            }
        }
    ...
    void PathTypeHandlerBase::SetPrototype(DynamicObject *instance, RecyclableObject *newPrototype)
    {
        ...
        if (useCache)
        {
            hasMap = newPrototype->GetInternalProperty(newPrototype,
                                                       Js::InternalPropertyIds::TypeOfPrototypeObjectDictionary,
                                                       (Js::Var *)&oldTypeToPromotedTypeMap, nullptr, scriptContext);
        }

        PathTypeHandlerBase *_this = PathTypeHandlerBase::FromTypeHandler(instance->GetTypeHandler());
        _this->SetPrototypeHelper(instance, newPrototype, hasMap ? oldTypeToPromotedTypeMap : nullptr, useCache,
                                  scriptContext);
    }

    void PathTypeHandlerBase::SetPrototypeHelper(DynamicObject *instance, RecyclableObject *newPrototype,
                                                 TypeTransitionMap *oldTypeToPromotedTypeMap, bool useCache,
                                                 ScriptContext *scriptContext)
    {
        ...
        uint16 roundedInlineSlotCapacity =
            ...
            if (oldTypeToPromotedTypeMap->TryGetValue(oldType, &cachedDynamicType))
            {
...
#if DBG
                    swprintf_s(reason, 1024, u"OffsetOfInlineSlot mismatch. Required = %d, Cached = %d",
                               this->GetOffsetOfInlineSlots(), cachedDynamicTypeHandler->GetOffsetOfInlineSlots());
#endif
                }
                else if (cachedDynamicTypeHandler->GetInlineSlotCapacity() != roundedInlineSlotCapacity)
                {
                    ...
                    if (cachedDynamicTypeHandler->GetInlineSlotCapacity() != roundedInlineSlotCapacity)
                    {
                        cachedDynamicType = nullptr;
#if DBG
                        swprintf_s(reason, 1024,
                                   u"InlineSlotCapacity mismatch after shrinking. Required = %d, Cached = %d",
                                   roundedInlineSlotCapacity, cachedDynamicTypeHandler->GetInlineSlotCapacity());
#endif
                    }
                }
            }
        ...
        {
            Assert(!oldTypeToPromotedTypeMap ||
                   (Js::Var)oldTypeToPromotedTypeMap == scriptContext->GetLibrary()->GetUndefined());
        ...
        if (cachedDynamicType == nullptr)
        {
            ...
            if (useCache)
            {
                ...
                if (oldTypeToPromotedTypeMap == nullptr)
                {
                    oldTypeToPromotedTypeMap =
                        RecyclerNew(instance->GetRecycler(), TypeTransitionMap, instance->GetRecycler(), 2);
                    newPrototype->SetInternalProperty(Js::InternalPropertyIds::TypeOfPrototypeObjectDictionary,
                                                      (Var)oldTypeToPromotedTypeMap,
                ...
                if (PHASE_TRACE1(TypeShareForChangePrototypePhase))
                {
                    ...
                    {
                        Output::Print(u"TypeSharing: Updating prototype [0x%p] object's DictionarySlot in __proto__. "
                                      u"Adding (key = 0x%p, value = 0x%p) in map = 0x%p. Reason = %s\n",
            ...
            {
                if (PHASE_TRACE1(TypeShareForChangePrototypePhase) ||
                    PHASE_VERBOSE_TRACE1(TypeShareForChangePrototypePhase))
                {
                    Output::Print(u"TypeSharing: No Typesharing because instance and newPrototype are from different "
                                  u"scriptContext.\n");
                    Output::Flush();
                }
            }
        ...
        else
        {
            ...
            if (PHASE_TRACE1(TypeShareForChangePrototypePhase))
            {
                ...
                if (PHASE_VERBOSE_TRACE1(TypeShareForChangePrototypePhase))
                {
                    ...
                    Output::Print(u"TypeSharing: Reusing prototype [0x%p] object's DictionarySlot (key = 0x%p, value = "
                                  u"0x%p) from map = 0x%p in __proto__.\n",
        ...
        Assert(
            DynamicObject::IsTypeHandlerCompatibleForObjectHeaderInlining(this, cachedDynamicType->GetTypeHandler()));
    ...
    void PathTypeHandlerBase::SetIsPrototype(DynamicObject *instance)
    {
        ...
        {
            // The type coming in may not be shared or even locked (i.e. might have been created via
            // DynamicObject::ChangeType()). In that case the type handler change below won't change the type on the
            // object, so we have to force it.
        ...
        else
        {
            ...
            if (ChangeTypeOnProto())
            {
                ...
                {
                    // We're about to split out the type.  If the original type was shared the handler better be shared
                    // as well. Otherwise, the handler would lose track of being shared between different types and
                    // instances.
...
#if ENABLE_FIXED_FIELDS
            TraceFixedFieldsAfterSetIsProto(instance, this, typeHandler, oldTypeDebug, instance->GetDynamicType(),
...
#if DBG
    bool PathTypeHandlerBase::CanStorePropertyValueDirectly(const DynamicObject *instance, PropertyId propertyId,
                                                            bool allowLetConst)
    {
        ...
        if (index != Constants::NoSlot)
        {
            ...
            if (FixPropsOnPathTypes())
            {
                return index < this->GetTypePath()->GetMaxInitializedLength() &&
                    !this->GetTypePath()->GetIsFixedFieldAt(index, this->GetPathLength());
            }
            else
#endif
            {
    ...
    bool PathTypeHandlerBase::HasSingletonInstance() const
    {
        ...
        return this->GetTypePath()->HasSingletonInstance() &&
    ...
    {
        Assert((GetFlags() & (IsLockedFlag | MayBecomeSharedFlag | IsSharedFlag)) ==
               (IsLockedFlag | MayBecomeSharedFlag));
        Assert(!IsolatePrototypes() || !GetIsOrMayBecomeShared() || !GetIsPrototype());
        ...
        if (FixPropsOnPathTypes())
        {
            if (invalidateFixedFields)
            {
                ...
                {
                    InvalidateFixedFieldAt(this->GetTypePath()->GetPropertyIdUnchecked(index)->GetPropertyId(), index,
                                           scriptContext);
                }
            }
    ...
    void PathTypeHandlerBase::InvalidateFixedFieldAt(Js::PropertyId propertyId, Js::PropertyIndex index,
                                                     ScriptContext *scriptContext)
    {
        if (!FixPropsOnPathTypes())
        {
    ...
    void PathTypeHandlerBase::AddBlankFieldAt(Js::PropertyId propertyId, Js::PropertyIndex index,
                                              ScriptContext *scriptContext)
    {
        if (!FixPropsOnPathTypes())
        {
        ...
        else
        {
            ...
            if (index + 1 == this->GetTypePath()->GetMaxInitializedLength())
            {
                ...
                // on the type path.  By the time we reach the singleton instance, all fixed fields will have been
                // invalidated. Otherwise, some fields could remain fixed (or even uninitialized) and we would have to
                // spin off a loop here to invalidate any remaining fixed fields
                Assert(HasSingletonInstanceOnlyIfNeeded());
                this->GetTypePath()->ClearSingletonInstance();
            }
        }
    ...
    bool PathTypeHandlerBase::ProcessFixedFieldChange(DynamicObject *instance, PropertyId propertyId,
                                                      PropertyIndex slotIndex, Var value, bool isNonFixed,
                                                      const PropertyRecord *propertyRecord)
    {
        ...
        if (slotIndex >= newTypeHandler->GetTypePath()->GetMaxInitializedLength())
        {
            ...
            // added in order of indexes to be sure that we don't leave any uninitialized properties interspersed with
            // initialized ones, which could lead to incorrect behavior.  See comment in TypePath::Branch.
            AssertMsg(slotIndex == newTypeHandler->GetTypePath()->GetMaxInitializedLength(),
            ...
            if (ShouldFixAnyProperties() && CanBeSingletonInstance(instance))
            {
                bool markAsFixed = !isNonFixed && !IsInternalPropertyId(propertyId) &&
                    (VarIs<JavascriptFunction>(value)
                         ? ShouldFixMethodProperties() || ShouldFixAccessorProperties()
                         : (ShouldFixDataProperties() &&
                            CheckHeuristicsForFixedDataProps(instance, propertyRecord, propertyId, value)));

                // Mark the newly added field as fixed and prevent population of inline caches.

                newTypeHandler->GetTypePath()->AddSingletonInstanceFieldAt(instance, slotIndex, markAsFixed,
                                                                           newTypeHandler->GetPathLength());
            }
            else
            {
        ...
        // If we branched and this is the singleton instance, we need to remove it from this type handler.  The only
        // time this can happen is when another not fully initialized instance is ahead of this one on the current path.
        auto singletonWeakRef = this->GetTypePath()->GetSingletonInstance();
        if (newTypeHandler->GetTypePath() != this->GetTypePath() && singletonWeakRef != nullptr &&
            singletonWeakRef->Get() == instance)
        {
            // If this is the singleton instance, there shouldn't be any other initialized instance ahead of it on the
            // old path.
    ...
    bool PathTypeHandlerBase::TryUseFixedProperty(PropertyRecord const *propertyRecord, Var *pProperty,
                                                  FixedPropertyKind propertyType, ScriptContext *requestContext)
    {
        ...
        TraceUseFixedProperty(propertyRecord, pProperty, result, u"PathTypeHandler", requestContext);
        return result;
    }

    bool PathTypeHandlerBase::TryUseFixedAccessor(PropertyRecord const *propertyRecord, Var *pAccessor,
                                                  FixedPropertyKind propertyType, bool getter,
                                                  ScriptContext *requestContext)
    {
            ...
            PHASE_VERBOSE_TRACE1(Js::UseFixedDataPropsPhase) || PHASE_VERBOSE_TESTTRACE1(Js::UseFixedDataPropsPhase))
        {
            ...
            if (this->HasSingletonInstance() &&
                this->GetSingletonInstance()->Get()->GetScriptContext() != requestContext)
            {
    ...
    bool PathTypeHandlerBase::CheckFixedProperty(PropertyRecord const *propertyRecord, Var *pProperty,
                                                 ScriptContext *requestContext)
    {
        return TryGetFixedProperty<true, false>(propertyRecord, pProperty,
                                                (Js::FixedPropertyKind)(Js::FixedPropertyKind::FixedMethodProperty |
                                                                        Js::FixedPropertyKind::FixedDataProperty),
                                                requestContext);
    }
    ...
    bool PathTypeHandlerBase::TryGetFixedProperty(PropertyRecord const *propertyRecord, Var *pProperty,
                                                  Js::FixedPropertyKind propertyType, ScriptContext *requestContext)
    {
        if (!FixPropsOnPathTypes())
        {
            return false;
        ...
        if (value &&
            ((IsFixedMethodProperty(propertyType) && VarIs<JavascriptFunction>(value)) ||
             IsFixedDataProperty(propertyType)))
        {
            *pProperty = value;
            if (markAsUsed)
            {
    ...
    void PathTypeHandlerBase::DumpFixedFields() const
    {
        if (FixPropsOnPathTypes())
        {
            ...
            {
                Output::Print(u" %s %d%d%d,", GetTypePath()->GetPropertyId(i)->GetBuffer(),
        ...
        else
        {
            for (PropertyIndex i = 0; i < GetPathLength(); i++)
            {
                Output::Print(u" %s %d%d%d,", GetTypePath()->GetPropertyId(i)->GetBuffer(), 1, 0, 0);
            }
        }
    }
    ...
    void PathTypeHandlerBase::TraceFixedFieldsBeforeTypeHandlerChange(
        ...
        RecyclerWeakReference<DynamicObject> *oldSingletonInstanceBefore)
    {
        if (PHASE_VERBOSE_TRACE1(FixMethodPropsPhase))
        {
            Output::Print(u"FixedFields: %s 0x%p from %s to %s:\n", conversionName, instance, oldTypeHandlerName,
                          newTypeHandlerName);
            Output::Print(u"   before: type = 0x%p, type handler = 0x%p, old singleton = 0x%p(0x%p)\n", oldType,
                          oldTypeHandler, oldSingletonInstanceBefore,
                          oldSingletonInstanceBefore != nullptr ? oldSingletonInstanceBefore->Get() : nullptr);
        ...
        DynamicType *oldType, DynamicType *newType, RecyclerWeakReference<DynamicObject> *oldSingletonInstanceBefore)
    {
        if (PHASE_VERBOSE_TRACE1(FixMethodPropsPhase))
        {
            ...
            Output::Print(
                u"   after: type = 0x%p, type handler = 0x%p, old singleton = 0x%p(0x%p), new singleton = 0x%p(0x%p)\n",
                newType, newTypeHandler, oldSingletonInstanceAfter,
                oldSingletonInstanceAfter != nullptr ? oldSingletonInstanceAfter->Get() : nullptr,
                newSingletonInstanceAfter,
                newSingletonInstanceAfter != nullptr ? newSingletonInstanceAfter->Get() : nullptr);
            Output::Print(u"   fixed fields:");
        ...
        {
            Output::Print(
                u"   type %s, typeHandler %s, old singleton after %s null (%s), new singleton after %s null\n",
        ...
        DynamicType *oldType, DynamicType *newType, RecyclerWeakReference<DynamicObject> *oldSingletonInstanceBefore)
    {
        if (PHASE_VERBOSE_TRACE1(FixMethodPropsPhase))
        {
            ...
            Output::Print(
                u"   after: type = 0x%p, type handler = 0x%p, old singleton = 0x%p(0x%p), new singleton = 0x%p(0x%p)\n",
                instance->GetType(), newTypeHandler, oldSingletonInstanceAfter,
                oldSingletonInstanceAfter != nullptr ? oldSingletonInstanceAfter->Get() : nullptr,
                newSingletonInstanceAfter,
                newSingletonInstanceAfter != nullptr ? newSingletonInstanceAfter->Get() : nullptr);
            Output::Print(u"   fixed fields:");
        ...
        {
            Output::Print(
                u"   type %s, old singleton after %s null (%s)\n", oldType != newType ? u"changed" : u"unchanged",
                oldSingletonInstanceBefore == nullptr ? u"==" : u"!=",
                oldSingletonInstanceBefore != oldTypeHandler->GetSingletonInstance() ? u"changed" : u"unchanged");
            Output::Print(u"   fixed fields after:");
            newTypeHandler->DumpFixedFields();
    ...
    PathTypeHandlerNoAttr *PathTypeHandlerNoAttr::New(ScriptContext *scriptContext, TypePath *typePath,
                                                      uint16 pathLength, uint16 inlineSlotCapacity,
                                                      uint16 offsetOfInlineSlots, bool isLocked, bool isShared,
                                                      DynamicType *predecessorType)
    {
        return New(scriptContext, typePath, pathLength, max(pathLength, inlineSlotCapacity), inlineSlotCapacity,
                   offsetOfInlineSlots, isLocked, isShared, predecessorType);
    }

    PathTypeHandlerNoAttr *PathTypeHandlerNoAttr::New(ScriptContext *scriptContext, TypePath *typePath,
                                                      uint16 pathLength, const PropertyIndex slotCapacity,
...
#endif
        return RecyclerNew(scriptContext->GetRecycler(), PathTypeHandlerNoAttr, typePath, pathLength, slotCapacity,
                           inlineSlotCapacity, offsetOfInlineSlots, isLocked, isShared, predecessorType);
    }

    PathTypeHandlerNoAttr *PathTypeHandlerNoAttr::New(ScriptContext *scriptContext, PathTypeHandlerNoAttr *typeHandler,
                                                      bool isLocked, bool isShared)
    {
        Assert(typeHandler != nullptr);
        return RecyclerNew(scriptContext->GetRecycler(), PathTypeHandlerNoAttr, typeHandler->GetTypePath(),
                           typeHandler->GetPathLength(), static_cast<PropertyIndex>(typeHandler->GetSlotCapacity()),
                           typeHandler->GetInlineSlotCapacity(), typeHandler->GetOffsetOfInlineSlots(), isLocked,
                           isShared);
    }

    PathTypeHandlerNoAttr::PathTypeHandlerNoAttr(TypePath *typePath, uint16 pathLength,
                                                 const PropertyIndex slotCapacity, uint16 inlineSlotCapacity,
                                                 uint16 offsetOfInlineSlots, bool isLocked, bool isShared,
                                                 DynamicType *predecessorType) :
        PathTypeHandlerBase(typePath, pathLength, slotCapacity, inlineSlotCapacity, offsetOfInlineSlots, isLocked,
                            isShared, predecessorType)
    {
    }

    PathTypeHandlerWithAttr *PathTypeHandlerWithAttr::New(ScriptContext *scriptContext, TypePath *typePath,
                                                          ObjectSlotAttributes *attributes,
                                                          PathTypeSetterSlotIndex *setters,
                                                          PathTypeSetterSlotIndex setterCount, uint16 pathLength,
                                                          uint16 inlineSlotCapacity, uint16 offsetOfInlineSlots,
                                                          bool isLocked, bool isShared, DynamicType *predecessorType)
    {
        return New(scriptContext, typePath, attributes, setters, setterCount, pathLength,
                   max(pathLength, inlineSlotCapacity), inlineSlotCapacity, offsetOfInlineSlots, isLocked, isShared,
                   predecessorType);
    }

    PathTypeHandlerWithAttr *
    PathTypeHandlerWithAttr::New(ScriptContext *scriptContext, TypePath *typePath, ObjectSlotAttributes *attributes,
                                 PathTypeSetterSlotIndex *setters, PathTypeSetterSlotIndex setterCount,
                                 uint16 pathLength, const PropertyIndex slotCapacity, uint16 inlineSlotCapacity,
                                 uint16 offsetOfInlineSlots, bool isLocked, bool isShared, DynamicType *predecessorType)
    {
        Assert(typePath != nullptr);
#ifdef PROFILE_TYPES
        scriptContext->pathTypeHandlerCount++;
#endif
        return RecyclerNew(scriptContext->GetRecycler(), PathTypeHandlerWithAttr, typePath, attributes, setters,
                           setterCount, pathLength, slotCapacity, inlineSlotCapacity, offsetOfInlineSlots, isLocked,
                           isShared, predecessorType);
    }

    PathTypeHandlerWithAttr::PathTypeHandlerWithAttr(TypePath *typePath, ObjectSlotAttributes *attributes,
                                                     PathTypeSetterSlotIndex *setters,
                                                     PathTypeSetterSlotIndex setterCount, uint16 pathLength,
                                                     const PropertyIndex slotCapacity, uint16 inlineSlotCapacity,
                                                     uint16 offsetOfInlineSlots, bool isLocked, bool isShared,
                                                     DynamicType *predecessorType) :
        PathTypeHandlerNoAttr(typePath, pathLength, slotCapacity, inlineSlotCapacity, offsetOfInlineSlots, isLocked,
                              isShared, predecessorType),
        attributes(attributes), setters(setters), setterCount(setterCount)
    {
    }
    ...
    BOOL PathTypeHandlerWithAttr::SetConfigurable(DynamicObject *instance, PropertyId propertyId, BOOL value)
    {
        ...
        ObjectSlotAttributes attr =
            (ObjectSlotAttributes)(value ? (attributes[propertyIndex] | ObjectSlotAttr_Configurable)
                                         : (attributes[propertyIndex] & ~ObjectSlotAttr_Configurable));
    ...
    BOOL PathTypeHandlerWithAttr::SetEnumerable(DynamicObject *instance, PropertyId propertyId, BOOL value)
    {
        ...
        ObjectSlotAttributes attr =
            (ObjectSlotAttributes)(value ? (attributes[propertyIndex] | ObjectSlotAttr_Enumerable)
                                         : (attributes[propertyIndex] & ~ObjectSlotAttr_Enumerable));
        return SetAttributesHelper(instance, propertyId, propertyIndex, attributes, attr);
    }
    ...
    BOOL PathTypeHandlerWithAttr::SetWritable(DynamicObject *instance, PropertyId propertyId, BOOL value)
    {
        ...
        ObjectSlotAttributes attr =
            (ObjectSlotAttributes)(value ? (attributes[propertyIndex] | ObjectSlotAttr_Writable)
                                         : (attributes[propertyIndex] & ~ObjectSlotAttr_Writable));
        return SetAttributesHelper(instance, propertyId, propertyIndex, attributes, attr);
    }

    _Check_return_ _Success_(return) BOOL PathTypeHandlerWithAttr::GetAccessors(DynamicObject *instance,
                                                                                PropertyId propertyId,
                                                                                _Outptr_result_maybenull_ Var *getter,
                                                                                _Outptr_result_maybenull_ Var *setter)
    {
        PropertyIndex propertyIndex = GetTypePath()->LookupInline(propertyId, GetPathLength());
        if (propertyIndex == Constants::NoSlot)
        {
    ...
    BOOL PathTypeHandlerWithAttr::SetAccessors(DynamicObject *instance, PropertyId propertyId, Var getter, Var setter,
                                               PropertyOperationFlags flags)
    {
...
#if ENABLE_NATIVE_CODEGEN
    bool PathTypeHandlerWithAttr::IsObjTypeSpecEquivalent(const Type *type, const TypeEquivalenceRecord &record,
                                                          uint &failedPropertyIndex)
    {
    ...
    DescriptorFlags PathTypeHandlerWithAttr::GetSetter(DynamicObject *instance, PropertyId propertyId, Var *setterValue,
                                                       PropertyValueInfo *info, ScriptContext *requestContext)
    {
        ...
        if (attr & ObjectSlotAttr_Accessor)
        {
            ...
            *setterValue = instance->GetSlot(setterSlot);
            PropertyValueInfo::Set(info, instance, setterSlot, ObjectSlotAttributesToPropertyAttributes(attr),
    ...
    DescriptorFlags PathTypeHandlerWithAttr::GetSetter(DynamicObject *instance, JavascriptString *propertyNameString,
    ...
    BOOL PathTypeHandlerWithAttr::HasProperty(DynamicObject *instance, PropertyId propertyId, __out_opt bool *noRedecl,
                                              _Inout_opt_ PropertyValueInfo *info)
    {
        if (noRedecl != nullptr)
        {
    ...
    BOOL PathTypeHandlerWithAttr::GetProperty(DynamicObject *instance, Var originalInstance, PropertyId propertyId,
                                              Var *value, PropertyValueInfo *info, ScriptContext *requestContext)
    {
        PropertyIndex index = GetTypePath()->LookupInline(propertyId, GetPathLength());
        if (index == Constants::NoSlot)
        ...
        if (attr & ObjectSlotAttr_Accessor)
        {
            ...
            PropertyValueInfo::SetNoCache(info,
                                          instance); // we already cached getter, so we don't have to do it once more
    ...
    BOOL PathTypeHandlerWithAttr::GetProperty(DynamicObject *instance, Var originalInstance,
                                              JavascriptString *propertyNameString, Var *value, PropertyValueInfo *info,
                                              ScriptContext *requestContext)
    {
        // Consider: Implement actual string hash lookup
        ...
        return PathTypeHandlerWithAttr::GetProperty(instance, originalInstance, propertyRecord->GetPropertyId(), value,
                                                    info, requestContext);
    }

    BOOL PathTypeHandlerWithAttr::SetProperty(DynamicObject *instance, PropertyId propertyId, Var value,
                                              PropertyOperationFlags flags, PropertyValueInfo *info)
    {
        PropertyIndex index = PathTypeHandlerBase::GetPropertyIndex(propertyId);
        if (index != Constants::NoSlot)
        {
            if (attributes[index] & ObjectSlotAttr_Accessor)
            {
                ...
                if (typeHandler == this)
                {
                    ...
                    if (attributes[index] & ObjectSlotAttr_Accessor)
                    {
                        PropertyValueInfo::Set(info, instance, setters[index],
                                               ObjectSlotAttributesToPropertyAttributes(attributes[index]),
                                               InlineCacheSetterFlag);
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    DescriptorFlags descriptorFlags =
                        typeHandler->GetSetter(instance, propertyId, (Var *)&func, info, instance->GetScriptContext());
    ...
    BOOL PathTypeHandlerWithAttr::SetProperty(DynamicObject *instance, JavascriptString *propertyNameString, Var value,
                                              PropertyOperationFlags flags, PropertyValueInfo *info)
    {
        // Consider: Implement actual string hash lookup
        PropertyRecord const *propertyRecord;
    ...
    BOOL PathTypeHandlerWithAttr::GetAttributesWithPropertyIndex(DynamicObject *instance, PropertyId propertyId,
                                                                 BigPropertyIndex index, PropertyAttributes *attributes)
    {
        if (index < this->GetPathLength())
        {
