...
BigPropertyIndex DynamicTypeHandler::GetPropertyIndexFromInlineSlotIndex(uint inlineSlot)
{
    return inlineSlot - (offsetOfInlineSlots / sizeof(Var *));
}

BigPropertyIndex DynamicTypeHandler::GetPropertyIndexFromAuxSlotIndex(uint auxIndex)
{
...
PropertyIndex DynamicTypeHandler::RoundUpObjectHeaderInlinedInlineSlotCapacity(const PropertyIndex slotCapacity)
{
    ...
    return RoundUpInlineSlotCapacity(slotCapacity - objectHeaderInlinableSlotCapacity) +
        objectHeaderInlinableSlotCapacity;
}
...
int DynamicTypeHandler::RoundUpSlotCapacity(const int slotCapacity, const PropertyIndex inlineSlotCapacity)
{
    ...
    const int maxSlotCapacity = slotCapacity <= PropertyIndexRanges<PropertyIndex>::MaxValue
...
DynamicTypeHandler::DynamicTypeHandler(int slotCapacity, uint16 inlineSlotCapacity, uint16 offsetOfInlineSlots,
                                       uint8_t flags) :
    flags(flags), propertyTypes(PropertyTypesWritableDataOnly | PropertyTypesReserved),
    offsetOfInlineSlots(offsetOfInlineSlots), unusedBytes(Js::AtomTag), protoCachesWereInvalidated(false)
{
    ...
    this->inlineSlotCapacity = inlineSlotCapacity = IsObjectHeaderInlined(offsetOfInlineSlots)
...
{
    AssertMsg(index >= (int)(offsetOfInlineSlots / sizeof(Var)),
...
#if DBG
void DynamicTypeHandler::SetSlot(DynamicObject *instance, PropertyId propertyId, bool allowLetConst, int index,
                                 Var value)
#else
...
#if DBG
void DynamicTypeHandler::SetInlineSlot(DynamicObject *instance, PropertyId propertyId, bool allowLetConst, int index,
                                       Var value)
#else
...
#endif
{
    ...
    typename WriteBarrierFieldTypeTraits<Var>::Type *slots =
        reinterpret_cast<typename WriteBarrierFieldTypeTraits<Var>::Type *>(instance);
    slots[index] = value;
}

#if DBG
void DynamicTypeHandler::SetAuxSlot(DynamicObject *instance, PropertyId propertyId, bool allowLetConst, int index,
                                    Var value)
#else
...
void DynamicTypeHandler::SetInstanceTypeHandler(DynamicObject *instance, bool hasChanged)
{
...
uint16 DynamicTypeHandler::GetOffsetOfObjectHeaderInlineSlots() { return offsetof(DynamicObject, auxSlots); }

PropertyIndex DynamicTypeHandler::GetObjectHeaderInlinableSlotCapacity()
{
    const PropertyIndex maxAllowedSlotCapacity =
        (sizeof(DynamicObject) - DynamicTypeHandler::GetOffsetOfObjectHeaderInlineSlots()) / sizeof(Var);
    AssertMsg(maxAllowedSlotCapacity == 2,
              "Today we should be getting 2 with the math here. Change this Assert, if we are changing this logic in "
              "the future");
    return maxAllowedSlotCapacity;
}

void DynamicTypeHandler::SetInstanceTypeHandler(DynamicObject *instance, DynamicTypeHandler *typeHandler,
                                                bool hasChanged)
{
    instance->SetTypeHandler(typeHandler, hasChanged);
}

DynamicTypeHandler *DynamicTypeHandler::GetCurrentTypeHandler(DynamicObject *instance)
{
    return instance->GetTypeHandler();
}

void DynamicTypeHandler::ReplaceInstanceType(DynamicObject *instance, DynamicType *type)
{
    instance->ReplaceType(type);
}

void DynamicTypeHandler::ResetTypeHandler(DynamicObject *instance)
{
    // just reuse the current type handler.
    this->SetInstanceTypeHandler(instance);
}

BOOL DynamicTypeHandler::FindNextProperty(ScriptContext *scriptContext, BigPropertyIndex &index,
...
{
    // Type handlers that support big property indexes override this function, so if we're here then this type handler
    // does not support big property indexes. Forward the call to the small property index version.
    ...
    const BOOL found = FindNextProperty(scriptContext, smallIndex, propertyString, propertyId, attributes, type,
...
bool DynamicTypeHandler::InvalidateInlineCachesForAllProperties(ScriptContext *requestContext)
{
    ...
    if (count <
        128) // Invalidate a propertyId involves dictionary lookups. Only do this when the number is relatively small.
    {
        for (int i = 0; i < count; i++)
        {
            ...
            {
                isStoreField ? requestContext->InvalidateStoreFieldCaches(propertyId)
                             : requestContext->InvalidateProtoCaches(propertyId);
            }
        }
        return false;
    }
...
bool DynamicTypeHandler::TryUseFixedProperty(PropertyRecord const *propertyRecord, Var *pProperty,
...
bool DynamicTypeHandler::TryUseFixedAccessor(PropertyRecord const *propertyRecord, Var *pAccessor,
...
{
    return (fixedPropKind & Js::FixedPropertyKind::FixedAccessorProperty) ==
        Js::FixedPropertyKind::FixedAccessorProperty;
}

bool DynamicTypeHandler::CheckHeuristicsForFixedDataProps(DynamicObject *instance, const PropertyRecord *propertyRecord,
                                                          Var value)
{
    ...
    if (Js::TaggedInt::Is(value) &&
        ((instance->GetTypeId() == TypeIds_GlobalObject &&
          instance->GetScriptContext()->IsIntConstPropertyOnGlobalObject(propertyRecord->GetPropertyId())) ||
         (instance->GetTypeId() == TypeIds_Object &&
          instance->GetScriptContext()->IsIntConstPropertyOnGlobalUserObject(propertyRecord->GetPropertyId()))))
    {
        return true;
    }
    ...
    if (PHASE_ON1(Js::FixDataVarPropsPhase))
    {
        ...
        if (instance->GetType()->GetTypeId() == TypeIds_GlobalObject)
        {
            ...
            RootObjectInlineCache *cache =
                (static_cast<Js::RootObjectBase *>(instance))
                    ->GetRootInlineCache(propertyRecord, /*isLoadMethod*/ false, /*isStore*/ true);
...
bool DynamicTypeHandler::CheckHeuristicsForFixedDataProps(DynamicObject *instance, JavascriptString *propertyKey,
                                                          Var value)
{
    return false;
}
...
void DynamicTypeHandler::TraceUseFixedProperty(PropertyRecord const *propertyRecord, Var *pProperty, bool result,
                                               const char16_t *typeHandlerName, ScriptContext *requestContext)
{
    ...
    if (pProperty && *pProperty &&
        ((Js::VarIs<Js::JavascriptFunction>(*pProperty) &&
          (PHASE_VERBOSE_TRACE1(Js::FixedMethodsPhase) || PHASE_VERBOSE_TESTTRACE1(Js::FixedMethodsPhase))) ||
         ((PHASE_VERBOSE_TRACE1(Js::UseFixedDataPropsPhase) || PHASE_VERBOSE_TESTTRACE1(Js::UseFixedDataPropsPhase)))))
    {
...
BOOL DynamicTypeHandler::GetInternalProperty(DynamicObject *instance, Var originalInstance, PropertyId propertyId,
                                             Var *value)
{
...
BOOL DynamicTypeHandler::InitProperty(DynamicObject *instance, PropertyId propertyId, Var value,
...
BOOL DynamicTypeHandler::SetInternalProperty(DynamicObject *instance, PropertyId propertyId, Var value,
...
BOOL DynamicTypeHandler::GetItem(DynamicObject *instance, Var originalInstance, uint32_t index, Var *value,
                                 ScriptContext *requestContext)
{
    return instance->GetObjectArrayItem(originalInstance, index, value, requestContext);
}

DescriptorFlags DynamicTypeHandler::GetSetter(DynamicObject *instance, PropertyId propertyId, Var *setterValue,
...
DescriptorFlags DynamicTypeHandler::GetSetter(DynamicObject *instance, JavascriptString *propertyNameString,
...
DescriptorFlags DynamicTypeHandler::GetItemSetter(DynamicObject *instance, uint32_t index, Var *setterValue,
                                                  ScriptContext *requestContext)
{
...
//
BOOL DynamicTypeHandler::SetItemWithAttributes(DynamicObject *instance, uint32_t index, Var value,
...
void DynamicTypeHandler::SetPropertyUpdateSideEffect(DynamicObject *instance, PropertyId propertyId, Var value,
                                                     SideEffects possibleSideEffects)
{
    if (possibleSideEffects && propertyId < PropertyIds::_countJSOnlyProperty)
    {
        ...
        {
            scriptContext->optimizationOverrides.SetSideEffects(
                (SideEffects)(SideEffects_ValueOf & possibleSideEffects));
            scriptContext->optimizationOverrides.SetSideEffects(
                (SideEffects)(SideEffects_ToString & possibleSideEffects));
        }
        ...
        else if (propertyId == PropertyIds::Math)
        {
            ...
            {
                scriptContext->optimizationOverrides.SetSideEffects(
                    (SideEffects)(SideEffects_MathFunc & possibleSideEffects));
            }
        }
...
void DynamicTypeHandler::SetPropertyUpdateSideEffect(DynamicObject *instance,
                                                     JsUtil::CharacterBuffer<char16_t> const &propertyName, Var value,
                                                     SideEffects possibleSideEffects)
{
...
void DynamicTypeHandler::EnsureSlots(DynamicObject *instance, int oldCount, int newCount, ScriptContext *scriptContext,
...
void DynamicTypeHandler::AdjustSlots_Jit(DynamicObject *const object, const PropertyIndex newInlineSlotCapacity,
                                         const int newAuxSlotCapacity)
{
    JIT_HELPER_NOT_REENTRANT_NOLOCK_HEADER(AdjustSlots);
    Assert(object);

    // The JIT may call AdjustSlots multiple times on the same object, even after changing its type to the new type.
...
void DynamicTypeHandler::AdjustSlots(DynamicObject *const object, const PropertyIndex newInlineSlotCapacity,
                                     const int newAuxSlotCapacity)
{
    Assert(object);
    ...
    typename WriteBarrierFieldTypeTraits<Var>::Type *const newAuxSlots =
    ...
    Var *const oldInlineSlots = reinterpret_cast<Var *>(reinterpret_cast<uintptr_t>(object) +
    ...
    {
        // Move the remaining inline slots such that none are object header-inlined. Copy backwards, as the two buffers
        // may overlap, with the new inline slot array starting beyond the start of the old inline slot array.
        if (PHASE_TRACE1(Js::ObjectHeaderInliningPhase))
        {
        ...
        typename WriteBarrierFieldTypeTraits<Var>::Type *const newInlineSlots =
            reinterpret_cast<typename WriteBarrierFieldTypeTraits<Var>::Type *>(object + 1);
...
bool DynamicTypeHandler::CanBeSingletonInstance(DynamicObject *instance) { return !ThreadContext::IsOnStack(instance); }
...
BOOL DynamicTypeHandler::DeleteProperty(DynamicObject *instance, JavascriptString *propertyNameString,
                                        PropertyOperationFlags flags)
{
...
PropertyId DynamicTypeHandler::TMapKey_GetPropertyId(ScriptContext *scriptContext, const PropertyId key) { return key; }
...
void DynamicTypeHandler::Dump(unsigned indent) const
{
    ...
    if (this->propertyTypes & PropertyTypesReserved)
        Output::Print(u"PropertyTypesReserved ");
    if (this->propertyTypes & PropertyTypesWritableDataOnly)
        Output::Print(u"PropertyTypesWritableDataOnly ");
    if (this->propertyTypes & PropertyTypesHasSpecialProperties)
        Output::Print(u"PropertyTypesHasSpecialProperties ");
    if (this->propertyTypes & PropertyTypesWritableDataOnlyDetection)
        Output::Print(u"PropertyTypesWritableDataOnlyDetection ");
    if (this->propertyTypes & PropertyTypesInlineSlotCapacityLocked)
        Output::Print(u"PropertyTypesInlineSlotCapacityLocked ");
    Output::Print(u"\n");

    Output::Print(u"%*sflags: 0x%02x ", fieldIndent, padding, this->flags);
    if (this->flags & IsExtensibleFlag)
        Output::Print(u"IsExtensibleFlag ");
    if (this->flags & HasKnownSlot0Flag)
        Output::Print(u"HasKnownSlot0Flag ");
    if (this->flags & IsLockedFlag)
        Output::Print(u"IsLockedFlag ");
    if (this->flags & MayBecomeSharedFlag)
        Output::Print(u"MayBecomeSharedFlag ");
    if (this->flags & IsSharedFlag)
        Output::Print(u"IsSharedFlag ");
    if (this->flags & IsPrototypeFlag)
        Output::Print(u"IsPrototypeFlag ");
    if (this->flags & IsSealedOnceFlag)
        Output::Print(u"IsSealedOnceFlag ");
    if (this->flags & IsFrozenOnceFlag)
        Output::Print(u"IsFrozenOnceFlag ");
    Output::Print(u"\n");
