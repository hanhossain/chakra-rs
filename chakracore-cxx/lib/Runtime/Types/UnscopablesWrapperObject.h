...
//-------------------------------------------------------------------------------------------------------
//
// The UnscopablesWrapperObject is a psuedo-object that provides us with convenience. By wrapping a normal object in
// this class, we can intercept object operations requiring checks to @@unscopables. For all other operations, the
// object needs to be unwrapped. The caller is responsible for unwrapping, which must happen only once the @@unscopables
// check has happened. For example, a getter needs to have the propId checked against @@unscopables, but for the actual
// getter call the object passed needs to be unwrapped.
//
#pragma once

#define UNWRAP_FAILFAST()                                                                                              \
    AssertOrFailFastMsg(false,                                                                                         \
                        "This UnscopablesWrapperObject must be unwrapped by the caller handling the scope before "     \
...
namespace Js
{
    ...
    public:
        UnscopablesWrapperObject(RecyclableObject *wrappedObject, StaticType *type) :
            RecyclableObject(type), wrappedObject(wrappedObject)
        {
        }
        ...
        virtual BOOL GetEnumerator(JavascriptStaticEnumerator *enumerator, EnumeratorFlags flags,
                                   ScriptContext *requestContext, EnumeratorCache *enumeratorCache = nullptr) override
        {
            UNWRAP_FAILFAST();
            return FALSE;
        };
        ...
        {
            UNWRAP_FAILFAST();
            return RecyclableObject::GetTypeOfString(requestContext);
        };
    };

    template <>
    inline bool VarIsImpl<UnscopablesWrapperObject>(RecyclableObject *obj)
    {
