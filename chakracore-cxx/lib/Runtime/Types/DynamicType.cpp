...
namespace Js
{
    ...
    DynamicType::DynamicType(ScriptContext *scriptContext, TypeId typeId, RecyclableObject *prototype,
                             JavascriptMethod entryPoint, DynamicTypeHandler *typeHandler, bool isLocked,
                             bool isShared) :
        Type(scriptContext, typeId, prototype, entryPoint), typeHandler(typeHandler), isLocked(isLocked),
        isShared(isShared), hasNoEnumerableProperties(false)
#if DBG
        ,
        isCachedForChangePrototype(false)
#endif
    ...
    DynamicType *DynamicType::New(ScriptContext *scriptContext, TypeId typeId, RecyclableObject *prototype,
                                  JavascriptMethod entryPoint, DynamicTypeHandler *typeHandler, bool isLocked,
                                  bool isShared)
    {
        return RecyclerNew(scriptContext->GetRecycler(), DynamicType, scriptContext, typeId, prototype, entryPoint,
                           typeHandler, isLocked, isShared);
    }

    bool DynamicType::Is(TypeId typeId) { return !StaticType::Is(typeId); }
    ...
    Use this method sparingly for cases where the intention is to ONLY lock the type and NOT lock the type handler. When
    in doubt use the LockType() method instead of LockTypeOnly.

    Currently, whenever we lock the type the intention is to lock BOTH the type and the type handler. However, almost
    always when we check whether the type is locked we intend to check whether the type handler is locked. The only
    exception is when we add a type to the EquivalentTypeCache.

    When we add a type to the EquivalentTypeCache the intention is to lock the Type to ensure that when a cached type
    evolves the cache gets invalidated. For shared types, this also means we need to create a new type handler when the
    type evolves. However, for non-shared types, we can still reuse the type handler. This provides a significant
    performance benefit for cases where objects with large number of properties are created once during start up (e.g.
    page load) and used later, by avoiding the cost of re-creating the type hanlder and copying a large number of
    properties over to the new type handler. To get this benefit, we only lock the type while adding it to the
    equivalent type cache. If the type becomes shared; after it was added to the equivalent type cache; its type handler
    will then get locked as part of ShareType() and any further evolution of the type will result in creating a new type
    handler.
    */
    ...
    Currently, whenever we lock the type the intention is to lock BOTH the type and the type handler. However, almost
    always when we check whether the type is locked we intend to check whether the type handler is locked. The only
    exception is when we add a type to the EquivalentTypeCache. See LockTypeOnly() method for details on the
    EquivalentTypeCache case.
    ...
    To support snapshot enumeration we need to remember the type handler at the start of the enumeration in case the
    type evolves during enumeration. To achieve this we lock BOTH the type and the type handler in preparation for the
    enumeration.
    3. While setting prototype: PathTypeHandlerBase::SetPrototype()
    While setting prototype if we create a new type and put the old type to promoted type mapping in the
    TypeOfPrototypeObjectDictionary property we lock and share the new type.
    4. PreventExtensions, Seal and Freeze:
    Each of the operations PreventExtension, Seal and Freeze change the behavior of the type in ways that needs the type
    handler to be locked. The PreventExtensions operation prevents adding any new properties to the type so any future
    attempts to add new property to its type handler needs the type handler to be converted. For a shared type, this
    will happen by design, but if we end up creating a new non-shared type handler during PreventExtensions or Seal or
    Freeze then we explicitly need to lock the type handler to indicate that any future mutation should convert this
    newly created type handler. Seal and Freeze operations also need locking the type handler for similar reasons. In
    addition, Freeze also needs to invalidate the inline caches for the type, so it also needs to change the type during
    this operation.
    */
    ...
    If we find a successor for the PropertyRecord while promoting a type the successor's type should now be marked as
    Shared as a new object has reached the type.
    ...
    When a new object literal is created with a type from FunctionBody::GetObjectLiteralTypeRef() already allocated by
    the ByteCodeWriter in AllocateObjectLiteralTypeArray() its marked as Shared.
    4. While updating the constructor function cache: JavascriptOperators::UpdateNewScObjectCache
    When we cache constructor return values for a type that has a sharable type handler we share the type in cases where
    we can do the "this assignment optimization".
    ...
    While deleting last property on an object if we end up moving auxSlots to object header (inline, reoptimize) the
    object's type get changed to the predecessor type. As the predecessor's type is now shared we mark it as shared.
    ...
    */
    ...
    DynamicTypeHandler *DynamicType::DuplicateTypeHandler() { return GetTypeHandler()->Clone(this->GetRecycler()); }

    bool DynamicType::SetHasNoEnumerableProperties(bool value)
    {
        if (!value)
        {
        ...
        Assert(!this->GetTypeHandler()->FindNextProperty(this->GetScriptContext(), propertyIndex, &propertyString,
                                                         &propertyId, nullptr, this, this, EnumeratorFlags::None,
                                                         nullptr, &info));
#endif
    ...
    void DynamicObject::InitSlots(DynamicObject *instance, ScriptContext *scriptContext)
    {
        ...
        {
            instance->auxSlots = RecyclerNewArrayZ(recycler, typename WriteBarrierFieldTypeTraits<Var>::Type,
                                                   slotCapacity - inlineSlotCapacity);
        }
    }
    ...
    PropertyQueryFlags DynamicObject::HasPropertyQuery(PropertyId propertyId, _Inout_opt_ PropertyValueInfo *info)
    {
        ...
        return JavascriptConversion::BooleanToPropertyQueryFlags(
            GetTypeHandler()->HasProperty(this, propertyId, nullptr /*pNoRedecl*/, info));
    }

    // HasOwnProperty and HasProperty is the same for most objects except globalobject (moduleroot as well in legacy)
    // Note that in GlobalObject, HasProperty and HasRootProperty is not quite the same as it's handling let/const
    // global etc.
    ...
    PropertyQueryFlags DynamicObject::GetPropertyQuery(Var originalInstance, PropertyId propertyId, Var *value,
                                                       PropertyValueInfo *info, ScriptContext *requestContext)
    {
        Assert(!Js::IsInternalPropertyId(propertyId));
        return JavascriptConversion::BooleanToPropertyQueryFlags(
            GetTypeHandler()->GetProperty(this, originalInstance, propertyId, value, info, requestContext));
    }

    PropertyQueryFlags DynamicObject::GetPropertyQuery(Var originalInstance, JavascriptString *propertyNameString,
                                                       ...
                                                       ScriptContext *requestContext)
    {
        ...
        return JavascriptConversion::BooleanToPropertyQueryFlags(
            GetTypeHandler()->GetProperty(this, originalInstance, propertyNameString, value, info, requestContext));
    }

    BOOL DynamicObject::GetInternalProperty(Var originalInstance, PropertyId propertyId, Var *value,
                                            PropertyValueInfo *info, ScriptContext *requestContext)
    {
        Assert(Js::IsInternalPropertyId(propertyId));
    ...
    PropertyQueryFlags DynamicObject::GetPropertyReferenceQuery(Var originalInstance, PropertyId propertyId, Var *value,
                                                                PropertyValueInfo *info, ScriptContext *requestContext)
    {
        Assert(!Js::IsInternalPropertyId(propertyId));
        return JavascriptConversion::BooleanToPropertyQueryFlags(
    ...
    BOOL DynamicObject::SetProperty(PropertyId propertyId, Var value, PropertyOperationFlags flags,
                                    PropertyValueInfo *info)
    {
        Assert(!Js::IsInternalPropertyId(propertyId));
    ...
    BOOL DynamicObject::SetProperty(JavascriptString *propertyNameString, Var value, PropertyOperationFlags flags,
                                    PropertyValueInfo *info)
    {
        AssertMsg(
            !PropertyRecord::IsPropertyNameNumeric(propertyNameString->GetString(), propertyNameString->GetLength()),
    ...
    BOOL DynamicObject::SetInternalProperty(PropertyId propertyId, Var value, PropertyOperationFlags flags,
                                            PropertyValueInfo *info)
    {
        Assert(Js::IsInternalPropertyId(propertyId));
    ...
    DescriptorFlags DynamicObject::GetSetter(PropertyId propertyId, Var *setterValue, PropertyValueInfo *info,
                                             ScriptContext *requestContext)
    {
        Assert(!Js::IsInternalPropertyId(propertyId));
    ...
    DescriptorFlags DynamicObject::GetSetter(JavascriptString *propertyNameString, Var *setterValue,
                                             PropertyValueInfo *info, ScriptContext *requestContext)
    {
        AssertMsg(
            !PropertyRecord::IsPropertyNameNumeric(propertyNameString->GetString(), propertyNameString->GetLength()),
    ...
    BOOL DynamicObject::InitProperty(PropertyId propertyId, Var value, PropertyOperationFlags flags,
                                     PropertyValueInfo *info)
    {
        Assert(!Js::IsInternalPropertyId(propertyId));
    ...
    BOOL DynamicObject::HasOwnItem(uint32_t index) { return HasItem(index); }

    PropertyQueryFlags DynamicObject::GetItemQuery(Var originalInstance, uint32_t index, Var *value,
                                                   ScriptContext *requestContext)
    {
        return JavascriptConversion::BooleanToPropertyQueryFlags(
            GetTypeHandler()->GetItem(this, originalInstance, index, value, requestContext));
    }

    PropertyQueryFlags DynamicObject::GetItemReferenceQuery(Var originalInstance, uint32_t index, Var *value,
                                                            ScriptContext *requestContext)
    {
        return JavascriptConversion::BooleanToPropertyQueryFlags(
            GetTypeHandler()->GetItem(this, originalInstance, index, value, requestContext));
    ...
    BOOL DynamicObject::ToPrimitiveImpl(Var *result, ScriptContext *requestContext)
    {
        ...
        if (JavascriptOperators::CheckIfObjectAndProtoChainHasNoSpecialProperties(this))
        {
            if (this->GetPrototype() == requestContext->GetLibrary()->GetObjectPrototype() &&
                !this->IsCrossSiteObject() && !requestContext->GetLibrary()->GetObjectPrototype()->IsCrossSiteObject())
            {
        ...
        if (!aValue)
        {
            InlineCache *inlineCache = propertyId == PropertyIds::valueOf ? requestContext->GetValueOfInlineCache()
                                                                          : requestContext->GetToStringInlineCache();
            // Use per script context inline cache for valueOf and toString
            aValue = JavascriptOperators::PatchGetValueUsingSpecifiedInlineCache(inlineCache, this, this, propertyId,
                                                                                 requestContext);
        }
        ...
        else
        {
            ...
            {
                Assert(JavascriptConversion::IsCallable(aValue));
                // These typeIds should never be here (they override ToPrimitive or they don't derive to
                // DynamicObject::ToPrimitive) Otherwise, they may case implicit call in ToStringHelper
                Assert(this->GetTypeId() != TypeIds_HostDispatch && this->GetTypeId() != TypeIds_HostObject);
    ...
    BOOL DynamicObject::CallToPrimitiveFunction(Var toPrimitiveFunction, PropertyId propertyId, Var *result,
                                                ScriptContext *requestContext)
    {
        if (JavascriptConversion::IsCallable(toPrimitiveFunction))
        {
            ...
            Var aResult = threadContext->ExecuteImplicitCall(
                toStringFunction, ImplicitCall_ToPrimitive,
                [=, this]() -> Js::Var
                {
    ...
    BOOL DynamicObject::GetEnumeratorWithPrefix(JavascriptEnumerator *prefixEnumerator,
                                                JavascriptStaticEnumerator *enumerator, EnumeratorFlags flags,
    ...
    BOOL DynamicObject::GetEnumerator(JavascriptStaticEnumerator *enumerator, EnumeratorFlags flags,
                                      ScriptContext *requestContext, EnumeratorCache *enumeratorCache)
    {
    ...
    _Check_return_ _Success_(return) BOOL DynamicObject::GetAccessors(PropertyId propertyId,
    ...
    BOOL DynamicObject::PreventExtensions() { return GetTypeHandler()->PreventExtensions(this); }

    BOOL DynamicObject::Seal() { return GetTypeHandler()->Seal(this); }
    ...
    BOOL DynamicObject::IsSealed() { return GetTypeHandler()->IsSealed(this); }

    BOOL DynamicObject::IsFrozen() { return GetTypeHandler()->IsFrozen(this); }

    BOOL DynamicObject::IsWritable(PropertyId propertyId) { return GetTypeHandler()->IsWritable(this, propertyId); }
    ...
    BOOL DynamicObject::IsEnumerable(PropertyId propertyId) { return GetTypeHandler()->IsEnumerable(this, propertyId); }
    ...
    BOOL DynamicObject::SetPropertyWithAttributes(PropertyId propertyId, Var value, PropertyAttributes attributes,
                                                  PropertyValueInfo *info, PropertyOperationFlags flags,
                                                  SideEffects possibleSideEffects)
    {
        return GetTypeHandler()->SetPropertyWithAttributes(this, propertyId, value, attributes, info, flags,
                                                           possibleSideEffects);
    }
