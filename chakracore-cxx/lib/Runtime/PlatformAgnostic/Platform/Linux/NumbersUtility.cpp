...
#define BufSize 256
        ...
        {
            size_t length = 0;
            for (; length < max_length && (to[length] = static_cast<N>(from[length])) != 0; length++)
                ;
        ...
        size_t Utility::NumberToDefaultLocaleString(const char16_t *number_string, const size_t length,
                                                    char16_t *buffer, const size_t pre_allocated_buffer_size)
        {
            ...
            AssertMsg(expected_length < INT_MAX, "Length of the formatted string shouldn't be >= INT_MAX");
