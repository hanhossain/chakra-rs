...
#pragma prefast(suppress: 6235 6286, "(<non-zero constant> || <expression>) is always a non-zero constant. - This is wrong, DBG_DUMP is not set in some build variants")
    ...
    {
        std::fwrite(&data, sizeof(T), 1, file->bsdFilePtr);
    }
    ...
    void DynamicProfileInfo::WriteData<char16_t const *>(char16_t const * const& sz, PAL_FILE * file)
    {
        if (sz)
        {
            charcount_t len = static_cast<charcount_t>(std::u16string(sz).length());
            ...
            std::fwrite(&cbNeeded, sizeof(cbNeeded), 1, file->bsdFilePtr);
            std::fwrite(tempBuffer, sizeof(utf8char_t), cbNeeded, file->bsdFilePtr);
            HeapDeleteArray(len * 3, tempBuffer);
        }
        else
        {
            charcount_t len = 0;
            std::fwrite(&len, sizeof(len), 1, file->bsdFilePtr);
        }
    }
