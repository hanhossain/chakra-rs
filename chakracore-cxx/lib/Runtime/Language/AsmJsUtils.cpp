            ...
            else if (info->GetArgType(i).isInt64())
            {
                ...
                if (i < actualArgCount)
                {
                    ...
                    else if (DynamicObject::IsBaseDynamicObject(*origArgs))
                    {
                        ...
                        scriptContext->GetOrAddPropertyRecord(u"low", (int)std::u16string(u"low").length(), &lowPropRecord);
                        scriptContext->GetOrAddPropertyRecord(u"high", (int)std::u16string(u"high").length(), &highPropRecord);
