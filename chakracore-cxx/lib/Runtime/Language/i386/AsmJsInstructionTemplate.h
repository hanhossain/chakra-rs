...
#define Is128BitsReg(reg) Is64BitsReg(reg)
        ...
        struct AddressDefinition
        {
            ...
            void dump() const
            {
                ...
                {
                    if (offset < 0)
                        Output::Print(u"[%s-0x%X]", RegNamesW[regEffAddr], offset * -1);
                    else
                        Output::Print( u"[%s+0x%X]", RegNamesW[regEffAddr], offset );
                }
                else
                {
                    if (offset < 0)
                        Output::Print(u"[%s+%s*%d+0x%X]", RegNamesW[regEffAddr], RegNamesW[regEffAddr2], multiplier, offset);
                    else
                        Output::Print( u"[%s+%s*%d+0x%X]", RegNamesW[regEffAddr], RegNamesW[regEffAddr2], multiplier, offset );
                }
            }
        ...
        struct InstrParamsReg
        {
            ...
            void dump() const
            {
                Output::Print( u"%s", RegNamesW[reg] );
            }
#endif
        };
        ...
        struct InstrParamsPtr
        {
            ...
            void dump() const
            {
                Output::Print( u"ptr:0x%X", (int)addr );
            }
#endif
        };
        ...
        struct InstrParamsRegPtr
        {
            ...
            void dump() const
            {
                Output::Print( u"%s, ptr:0x%X", RegNamesW[reg], (int)addr);
            }
#endif
        };
        ...
        struct InstrParams2Reg
        {
            ...
            void dump() const
            {
                Output::Print( u"%s, %s", RegNamesW[reg], RegNamesW[reg2] );
            }
#endif
        };
        ...
        struct InstrParamsRegImm
        {
            ...
            {
                if (imm < 0)
                    Output::Print(u"%s, -0x%X", RegNamesW[reg], imm * -1);
                else
                    Output::Print(u"%s, 0x%X", RegNamesW[reg], imm);
            }
#endif
        };
...
#define DUMP_ASM_CODE_PADDING(size) ((DUMP_ASM_CODE_NB_BYTES-size%DUMP_ASM_CODE_NB_BYTES)%DUMP_ASM_CODE_NB_BYTES)*5+1
    ...
    template<typename T>
    void DumpAsmCode( const uint8_t* buffer, const int size, const char16_t* instructionName, T* params )
    {
#if DBG_DUMP
        if( PHASE_TRACE( AsmjsEncoderPhase, AsmJsJitTemplate::Globals::CurrentEncodingFunction ) )
        {
            ...
            for( int i = size; i > 0; --i, ++j )
            {
                ...
                Output::Print( u"0x%02X ", buffer[-i] );
            }
            Output::Print( u"%*c  %s ", DUMP_ASM_CODE_PADDING( size ), ' ', instructionName );
            if( params )
            {
                params->dump();
            }
...
#define InstructionMembers(name, supInstrSize, flags) \
    ...
    static const char16_t* InstructionName;\
    static const int Flags = flags;\
    static const char16_t* GetInstructionName() { return InstructionName; }

#define InstructionMemberInit(name)\
    const char16_t* name::InstructionName = _u(#name);
#else
#define InstructionMembers(name, supInstrSize, flags) \
    static const int SupportedInstrSize = supInstrSize;\
    static const int Flags = flags;\
    static const char16_t* GetInstructionName() { return u""; }
#define InstructionMemberInit(name)
#endif
...
#undef InstructionFormat
        ...
        int ApplyCustomTemplate( uint8_t*& buffer, const uint8_t* src, const int size )
        {
            ...
            buffer += size;
            DumpAsmCode<InstrParamsEmpty>( buffer, size, u"Custom",nullptr);
            return size;
        }
    };
}
