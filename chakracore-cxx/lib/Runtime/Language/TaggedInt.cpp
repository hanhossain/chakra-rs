...
namespace Js
{
    ...
    Var TaggedInt::OverflowHelper(int overflowValue, ScriptContext* scriptContext)
    {
    ...
    Var TaggedInt::DivideByZero(int nLeft, ScriptContext* scriptContext)
    {
        if (nLeft == 0)
        {
...
#error Unsupported processor type
    ...
    int TaggedInt::ToBuffer(Var aValue, __out_ecount_z(bufSize) char16_t * buffer, uint bufSize)
    {
        return ToBuffer(ToInt32(aValue), buffer, bufSize);
    }

    // fills the buffer from the end and returns the start index
    int TaggedInt::UnsignedToString(unsigned __int64 value, char16_t *buffer, int bufferSize)
    {
        ...
        buffer[bufferSize - 1] = char16_t(0);
    ...
    int TaggedInt::SignedToString(__int64 value, char16_t *buffer, int bufferSize)
    {
        bool neg = value < 0;
    ...
    int TaggedInt::ToBuffer(int value, __out_ecount_z(bufSize) char16_t * buffer, uint bufSize)
    {
        Assert(bufSize >= 22);
        return SignedToString(value, buffer, bufSize);
    }

    int TaggedInt::ToBuffer(uint value, __out_ecount_z(bufSize) char16_t * buffer, uint bufSize)
    {
        Assert(bufSize >= 22);
    ...
    // Explicitly marking noinline and stdcall since this is called from inline asm
    Var TaggedInt::IncrementOverflowHelper(ScriptContext* scriptContext)
    {
    ...
    // Explicitly marking noinline and stdcall since this is called from inline asm
    Var TaggedInt::DecrementUnderflowHelper(ScriptContext* scriptContext)
    {
