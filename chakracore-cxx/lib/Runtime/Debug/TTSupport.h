...
void __declspec(noreturn) TTDAbort_unrecoverable_error(const char* msg);
...
namespace TTD
{
    ...
    namespace UtilSupport
    {
        ...
        public:
            TTAutoString();
            TTAutoString(const char16_t* str);
    ...
    public:
        ...
        //clone a null terminated char16_t* string (or nullptr) into the allocator -- currently only used for wellknown tokens
        const char16_t* CopyRawNullTerminatedStringInto(const char16_t* str)
        {
            if (str == nullptr)
            {
                return nullptr;
            ...
            {
                size_t length = std::u16string(str).length() + 1;
        ...
        void CopyStringIntoWLength(const char16_t* str, uint32_t length, TTString& into)
        {
            ...
            js_memcpy_s(into.Contents, into.Length * sizeof(char16_t), str, length * sizeof(char16_t));
        ...
        void CopyNullTermStringInto(const char16_t* str, TTString& into)
        {
            if (str == nullptr)
            {
                into.Length = 0;
            ...
            {
                this->CopyStringIntoWLength(str, (uint32_t)std::u16string(str).length(), into);
            }
        }
