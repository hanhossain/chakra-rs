...
namespace Js
{
    ...
    {
        ResolvedObject() :
            propId(Js::Constants::NoProperty), scriptContext(nullptr), address(nullptr), objectDisplay(nullptr),
            obj(nullptr), originalObj(nullptr), isConst(false), name(nullptr)
        {
        }
    ...
    {
    public:
        virtual const char16_t *Name() = 0;
        virtual const char16_t *Type() = 0;
        virtual const char16_t *Value(int radix) = 0;
        ...
        virtual bool IsFake()
        {
            return (this->GetTypeAttribute() & DBGPROP_ATTRIB_VALUE_IS_FAKE) == DBGPROP_ATTRIB_VALUE_IS_FAKE;
        }
    ...
    // If someone has to change or provide the support for custom types/objects (such as PixelArray etc) be displayed on
    // the debugger, they need to aware of few things which are mentioned below.

    // <...>Display (eg RecyclableArrayDisplay), mentions how current variable is given to debugger, and tells what
    // walker (enumerator) to be chosen
    //              in order to walk to children of the current variable.
    // <...>Walker (eg RecyclableArrayWalker), mentions logic of walk thru content of the current variable (the object
    // generally acts like an enumerator). Let say for an array, it has logic to go thru each
    //              indices and populate values.
    // <...>Address (eg RecyclableArrayAddress), associated with each child and will be used to updating that item. The
    // object if this will be consumed by "<...>Walker"
    ...
    struct DebuggerPropertyDisplayInfo
    {
        ...
        DebuggerPropertyDisplayInfo(PropertyId _propId, Var _aVar, uint32_t _flags) :
            propId(_propId), aVar(_aVar), flags(_flags)
        {
        }
    ...
    public:
        VariableWalkerBase(DiagStackFrame *_pFrame, Var _instance, UIGroupType _groupType, bool allowLexicalThis,
                           bool allowSuperReference = false) :
            pFrame(_pFrame), instance(_instance), pMembersList(nullptr), groupType(_groupType),
            allowLexicalThis(allowLexicalThis), allowSuperReference(allowSuperReference)
        {
        }
        ...
        static void GetReturnedValueResolvedObject(ReturnedValue *returnValue, DiagStackFrame *frame,
        ...
        {
            return (groupType != UIGroupType::UIGroupType_None && groupType != UIGroupType::UIGroupType_Param &&
                    groupType != UIGroupType::UIGroupType_InnerScope);
        }
        bool IsWalkerForCurrentFrame() const
        {
            return groupType == UIGroupType::UIGroupType_None || groupType == UIGroupType_Param;
        }
        ...
        DebuggerPropertyDisplayInfo *AllocateNewPropertyDisplayInfo(PropertyId propertyId, Var value, bool isConst,
                                                                    bool isInDeadZone);

    protected:
        int GetMemberCount() { return pMembersList ? pMembersList->Count() : 0; }

        bool IsPropertyValid(PropertyId propertyId, RegSlot location, bool *isPropertyInDebuggerScope, bool *isConst,
                             bool *isInDeadZone) const;

    private:
        static const char16_t *ParseFunctionName(const char16_t *displayNameBuffer,
                                                 const charcount_t displayNameBufferLength,
                                                 ScriptContext *scriptContext);
    };
    ...
    public:
        RegSlotVariablesWalker(DiagStackFrame *_pFrame, DebuggerScope *_debuggerScope, UIGroupType _groupType,
                               bool allowSuperReference = false) :
            VariableWalkerBase(_pFrame, nullptr, _groupType, /* allowLexicalThis */ false, allowSuperReference),
            debuggerScope(_debuggerScope)
        {
        }
    ...
    {
    public:
        SlotArrayVariablesWalker(DiagStackFrame *_pFrame, Var _instance, UIGroupType _groupType, bool allowLexicalThis,
                                 bool allowSuperReference = false) :
            VariableWalkerBase(_pFrame, _instance, _groupType, allowLexicalThis, allowSuperReference)
        {
        }
        ...
        ScopeSlots GetSlotArray()
        {
            typename WriteBarrierFieldTypeTraits<Var>::Type *slotArray =
                (typename WriteBarrierFieldTypeTraits<Var>::Type *)instance;
    ...
    {
    public:
        ObjectVariablesWalker(DiagStackFrame *_pFrame, Var _instance, UIGroupType _groupType, bool allowLexicalThis,
                              bool allowSuperReference = false) :
            VariableWalkerBase(_pFrame, _instance, _groupType, allowLexicalThis, allowSuperReference)
        {
        }
    ...
    {
    public:
        RootObjectVariablesWalker(DiagStackFrame *_pFrame, Var _instance, UIGroupType _groupType) :
            ObjectVariablesWalker(_pFrame, _instance, _groupType, /* allowLexicalThis */ false)
        {
        }
    ...
    public:
        DiagScopeVariablesWalker(DiagStackFrame *_pFrame, Var _instance, bool _enumWithScopeAlso) :
            ...
            pDiagScopeObjects(nullptr), diagScopeVarCount(0), scopeIsInitialized(false),
            enumWithScopeAlso(_enumWithScopeAlso)
        {
        }
    ...
    class LocalsWalker : public IDiagObjectModelWalkerBase
    {
        ...
        JsUtil::List<VariableWalkerBase *, ArenaAllocator>
            *pVarWalkers; // This includes, current frame, all scopes and globals for a current frame
        uint totalLocalsCount;
        uint32_t frameWalkerFlags;

        // true, if user has not defined the 'arguments' in the script, this is used for displaying a fake arguments
        // object and display in the locals window.
    ...
    public:
        ...
        DynamicObject *CreateAndPopulateActivationObject(ScriptContext *scriptContext,
                                                         FnProcessResolvedObject processResolvedObjectFn)
        {
            ...
            if (count > 0)
            {
                ...
                for (uint32_t i = 0; i < count; i++)
                {
                    ...
                    if (this->Get(i, &resolveObject) && resolveObject.propId != Js::Constants::NoProperty)
                    {
                        ...
                        {
                            OUTPUT_TRACE(
                                Js::ConsoleScopePhase, u"Adding '%s' property to activeScopeObject\n",
                                resolveObject.scriptContext->GetPropertyName(resolveObject.propId)->GetBuffer());
                            if (resolveObject.IsInDeadZone())
                            {
                                PropertyOperationFlags flags = static_cast<PropertyOperationFlags>(
                                    PropertyOperation_SpecialValue | PropertyOperation_AllowUndecl);
                                PropertyAttributes attributes =
                                    resolveObject.isConst ? PropertyConstDefaults : PropertyLetDefaults;
                                resolveObject.obj = scriptContext->GetLibrary()->GetUndeclBlockVar();
                                activeScopeObject->SetPropertyWithAttributes(resolveObject.propId, resolveObject.obj,
                            ...
                            else
                            {
                                activeScopeObject->SetPropertyWithAttributes(
                                    ...
                                    JavascriptOperators::BoxStackInstance(
                                        resolveObject.obj, scriptContext, /* allowStackFunction */ false,
                                        /* deepCopy */ false), // The value escapes, box if necessary.
    ...
    public:
        ...
        virtual const char16_t *Name() override;
        virtual const char16_t *Type() override;
        virtual const char16_t *Value(int radix) override;
    ...
    private:
        void FetchValueAndAddress(DebuggerScopeProperty &scopeProperty, _Out_opt_ Var *pValue,
                                  _Out_opt_ IDiagObjectAddress **ppAddress);
    };
    ...
    protected:
        ScriptContext *scriptContext;
        Var instance;
        Var originalInstance; // Remember original instance for prototype walk, because evaluating getters in
                              // CallGetter() if __proto__ instance is passed does not work
        JsUtil::List<DebuggerPropertyDisplayInfo *, ArenaAllocator> *pMembersList;

        RecyclableArrayWalker *innerArrayObjectWalker; // Will be used for array indices on the object
        JsUtil::List<IDiagObjectModelWalkerBase *, ArenaAllocator>
            *fakeGroupObjectWalkerList; // such as [prototype], [Methods] etc.

        void InsertItem(Js::RecyclableObject *pOriginalObject, Js::RecyclableObject *pObject, PropertyId propertyId,
                        bool isConst, bool isUnscoped, Js::RecyclableMethodsGroupWalker **ppMethodsGrouptWalker,
                        bool shouldPinProperty = false);
        void InsertItem(PropertyId propertyId, bool isConst, bool isUnscoped, Var itemObj,
                        Js::RecyclableMethodsGroupWalker **ppMethodsGrouptWalker, bool shouldPinProperty = false);
    ...
    public:
        ...
        static Var GetObject(RecyclableObject *originalInstance, RecyclableObject *instance, PropertyId propertyId,
                             ScriptContext *scriptContext);
    };
    ...
    public:
        RecyclableObjectDisplay(ResolvedObject *resolvedObject,
        ...
        virtual BOOL HasChildren() override;
        virtual BOOL Set(Var updateObject) override;
        virtual BOOL SetDefaultTypeAttribute(DBGPROP_ATTRIB_FLAGS attributes) override
        {
        ...
        static BOOL GetPropertyWithScriptEnter(RecyclableObject *originalInstance, RecyclableObject *instance,
                                               PropertyId propertyId, Var *value, ScriptContext *scriptContext);
    ...
    public:
        ...
        virtual BOOL GetResolvedObject(Js::JavascriptArray *arrayObj, int index, ResolvedObject *pResolvedObject,
                                       uint32_t *pabsIndex);
    ...
    public:
        RecyclableCollectionObjectWalker(ScriptContext *scriptContext, Var instance) :
            scriptContext(scriptContext), instance(instance), propertyList(nullptr)
        {
        }
        virtual BOOL GetGroupObject(ResolvedObject *pResolvedObject) override;
        virtual BOOL Get(int i, ResolvedObject *pResolvedObject) override;
    ...
    public:
        RecyclableCollectionObjectDisplay(ScriptContext *scriptContext, const char16_t *name,
                                          RecyclableCollectionObjectWalker<TData> *walker) :
            scriptContext(scriptContext), name(name), walker(walker)
        {
        }

        virtual const char16_t *Name() override { return name; }
        virtual const char16_t *Type() override { return u""; }
        virtual const char16_t *Value(int radix) override;
        ...
        virtual DBGPROP_ATTRIB_FLAGS GetTypeAttribute() override
        {
            return DBGPROP_ATTRIB_VALUE_READONLY | DBGPROP_ATTRIB_VALUE_IS_FAKE |
                (HasChildren() ? DBGPROP_ATTRIB_VALUE_IS_EXPANDABLE : 0);
        }
        virtual WeakArenaReference<IDiagObjectModelWalkerBase> *CreateWalker() override;
        ...
        virtual DiagObjectModelDisplayType GetType()
        {
            return DiagObjectModelDisplayType_RecyclableCollectionObjectDisplay;
        }
        virtual bool IsLiteralProperty() const { return false; }
    };
    ...
    public:
        RecyclableKeyValueDisplay(ScriptContext *scriptContext, Var key, Var value, const char16_t *name) :
            scriptContext(scriptContext), key(key), value(value), name(name)
        {
        }
        ...
        virtual DBGPROP_ATTRIB_FLAGS GetTypeAttribute() override
        {
            return DBGPROP_ATTRIB_VALUE_IS_EXPANDABLE | DBGPROP_ATTRIB_VALUE_IS_FAKE | DBGPROP_ATTRIB_VALUE_READONLY;
        }
        virtual WeakArenaReference<IDiagObjectModelWalkerBase> *CreateWalker() override;
    ...
    public:
        RecyclableKeyValueWalker(ScriptContext *scriptContext, Var key, Var value) :
            scriptContext(scriptContext), key(key), value(value)
        {
        }
    ...
    public:
        ...
        virtual const char16_t *Value(int radix) override { return u""; }
        virtual BOOL HasChildren() override { return TRUE; }
        virtual WeakArenaReference<IDiagObjectModelWalkerBase> *CreateWalker() override;
    };
