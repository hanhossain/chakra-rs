...
namespace TTD
{
    ...
    void SnapshotExtractor::ExtractSlotArrayIfNeeded(Js::ScriptContext* ctx, typename WriteBarrierFieldTypeTraits<Js::Var>::Type* scope)
    {
    ...
    void SnapshotExtractor::ExtractScopeIfNeeded(Js::ScriptContext* ctx, Js::FrameDisplay* environment)
    {
        if(this->m_marks.IsMarked(environment))
        {
            ...
            for(uint16 i = 0; i < funcScopeInfo->ScopeCount; ++i)
            {
                ...
                {
                    this->ExtractSlotArrayIfNeeded(ctx, (typename WriteBarrierFieldTypeTraits<Js::Var>::Type*)scope);
    ...
    void SnapshotExtractor::EvacuateMarkedIntoSnapshot(ThreadContext* threadContext, JsUtil::BaseHashSet<Js::FunctionBody*, HeapAllocator>& liveTopLevelBodies)
    {
        ...
        JsUtil::BaseDictionary<Js::HashedCharacterBuffer<char16_t>*, const Js::PropertyRecord*, Recycler, PowerOf2SizePolicy, Js::PropertyRecordStringHashComparer>* tcSymbolRegistrationMap = threadContext->GetSymbolRegistrationMap_TTD();
