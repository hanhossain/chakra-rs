...
namespace Js
{
    ...
    public:
        ...
        Js::Var GetThisFromFrame(Js::IDiagObjectAddress **ppOutAddress,
                                 Js::IDiagObjectModelWalkerBase *localsWalker = nullptr);
        // This function will try to populate obj and address field of the ResolvedObject.
        void TryFetchValueAndAddress(const char16_t *source, int sourceLength, Js::ResolvedObject *pObj);
        Js::ScriptFunction *TryGetFunctionForEval(Js::ScriptContext *scriptContext, const char16_t *source,
                                                  int sourceLength, BOOL isLibraryCode = FALSE);
        void EvaluateImmediate(const char16_t *source, int sourceLength, BOOL isLibraryCode, Js::ResolvedObject *pObj);
