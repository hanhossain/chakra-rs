...
namespace TTD
{
    ...
    namespace NSSnapValues
    {
        ...
        {
            TTDComparePath::PathEntry next{ (uint32_t)pid, nullptr };
        ...
        Field(Js::Var)* InflateSlotArrayInfo(const SlotArrayInfo* slotInfo, InflateMap* inflator)
        {
            ...
            if(slotInfo->isFunctionBodyMetaData)
            {
                ...
                for(uint32_t j = 0; j < slotInfo->SlotCount; j++)
                {
                    ...
                    for(uint32_t i = 0; i < fbody->GetScopeSlotArraySize(); i++)
                    {
            ...
            else
            {
                ...
                {
                    Js::PropertyId trgtPid = slotInfo->PIDArray[j];
                    for(uint32_t i = 0; i < slotInfo->SlotCount; i++)
                    {
        ...
        void AssertSnapEquiv(const SlotArrayInfo* sai1, const SlotArrayInfo* sai2, TTDCompareMap& compareMap)
        {
            ...
            for(uint32_t i = 0; i < sai1->SlotCount; ++i)
            {
                Js::PropertyId id1 = sai1->PIDArray[i];
                bool found = false;
                for(uint32_t j = 0; j < sai1->SlotCount; ++j)
                {
        ...
        void EmitScriptFunctionScopeInfo(const ScriptFunctionScopeInfo* funcScopeInfo, FileWriter* writer, NSTokens::Separator separator)
        {
            ...
            for(uint32_t i = 0; i < funcScopeInfo->ScopeCount; ++i)
            {
        ...
        void AssertSnapEquiv(const ScriptFunctionScopeInfo* funcScopeInfo1, const ScriptFunctionScopeInfo* funcScopeInfo2, TTDCompareMap& compareMap)
        {
            ...
            for(uint32_t i = 0; i < funcScopeInfo1->ScopeCount; ++i)
            {
        ...
        void ExtractSnapFunctionBodyScopeChain(bool isWellKnownFunction, SnapFunctionBodyScopeChain& scopeChain, Js::FunctionBody* fb, SlabAllocator& alloc)
        {
            ...
            if(!isWellKnownFunction && fb->GetScopeObjectChain() != nullptr)
            {
                ...
                scopeChain.ScopeCount = (uint32_t)scChain->pScopeChain->Count();
        ...
        void EmitSnapFunctionBodyScopeChain(const SnapFunctionBodyScopeChain& scopeChain, FileWriter* writer)
        {
            ...
            writer->WriteSequenceStart_DefaultKey(NSTokens::Separator::CommaSeparator);
            for(uint32_t i = 0; i < scopeChain.ScopeCount; ++i)
            {
        ...
        void EmitTopLevelCommonBodyResolveInfo(const TopLevelCommonBodyResolveInfo* fbInfo, bool emitInline, ThreadContext* threadContext, FileWriter* writer, NSTokens::Separator separator)
        {
            ...
            {
                TTDAssert(!fbInfo->IsUtf8, "Should only emit char16_t encoded data in inline mode.");

                writer->WriteInlineCode((char16_t*)fbInfo->SourceBuffer, fbInfo->ByteLength / sizeof(char16_t), NSTokens::Separator::CommaSeparator);
            }
            else
            {
        ...
        void ParseTopLevelCommonBodyResolveInfo(TopLevelCommonBodyResolveInfo* fbInfo, bool readSeparator, bool parseInline, ThreadContext* threadContext, FileReader* reader, SlabAllocator& alloc)
        {
            ...
            if(parseInline || IsNullPtrTTString(fbInfo->SourceUri))
            {
                ...
                reader->ReadInlineCode((char16_t*)fbInfo->SourceBuffer, fbInfo->ByteLength / sizeof(char16_t), true);
            }
            else
            {
        ...
        void AssertSnapEquiv(const TopLevelCommonBodyResolveInfo* fbInfo1, const TopLevelCommonBodyResolveInfo* fbInfo2, TTDCompareMap& compareMap)
        {
            ...
            for(uint32_t i = 0; i < fbInfo1->ByteLength; ++i)
            {
        ...
        Js::FunctionBody* InflateTopLevelLoadedFunctionBodyInfo(const TopLevelScriptLoadFunctionBodyResolveInfo* fbInfo, Js::ScriptContext* ctx)
        {
            ...
            uint32_t scriptLength = fbInfo->TopLevelBase.ByteLength;
            ...
            uint32_t srcUriLength = fbInfo->TopLevelBase.SourceUri.Length;
            ...
            SRCINFO si = {
                ...
                /* lnMinHost           */ 0,
                /* ichMinHost          */ 0,
                /* ichLimHost          */ static_cast<uint32_t>(scriptLength / chsize), // OK to truncate since this is used to limit sourceText in debugDocument/compilation errors.
                /* ulCharOffset        */ 0,
        ...
        {
            NSSnapValues::ExtractTopLevelCommonBodyResolveInfo(&fbInfo->TopLevelBase, fb, topLevelCtr, moduleId, 0, false, (const byte*)source, sourceLen * sizeof(char16_t), alloc);
        }
        ...
        Js::FunctionBody* InflateTopLevelEvalFunctionBodyInfo(const TopLevelEvalFunctionBodyResolveInfo* fbInfo, Js::ScriptContext* ctx)
        {
            ...
            Js::FastEvalMapString key(nullptr /*owningVar*/, source, sourceLen, fbInfo->TopLevelBase.ModuleId, fbInfo->IsStrictMode, false);
        ...
        void InflateFunctionBody(const FunctionBodyResolveInfo* fbInfo, InflateMap* inflator, const TTDIdentifierDictionary<TTD_PTR_ID, FunctionBodyResolveInfo*>& idToFbResolveMap)
        {
            ...
            else
            {
                ...
                if(resfb == nullptr)
                {
                    ...
                    //
                    uint32_t blength = parentBody->GetNestedCount();
            ...
            {
                const char16_t* fname = resfb->GetDisplayName();
                for(uint32_t i = 0; i < fbInfo->FunctionName.Length; ++i)
                {
            ...
            const TTDIdentifierDictionary<unsigned long, TopLevelEvalFunctionBodyResolveInfo*>& topLevelEvalScriptMap)
        {
            ...
            for(uint32_t i = 0; i < snpCtx->LoadedTopLevelScriptCount; ++i)
            {
            ...
            {
                for(uint32_t i = 0; i < snpCtx->NewFunctionTopLevelScriptCount; ++i)
                {
                ...
                for(uint32_t i = 0; i < snpCtx->EvalTopLevelScriptCount; ++i)
                {
        ...
        void EmitSnapContext(const SnapContext* snapCtx, FileWriter* writer, NSTokens::Separator separator)
        {
            ...
            writer->WriteSequenceStart_DefaultKey(NSTokens::Separator::CommaSeparator);
            for(uint32_t i = 0; i < snapCtx->LoadedTopLevelScriptCount; ++i)
            {
            ...
            writer->WriteSequenceStart_DefaultKey(NSTokens::Separator::CommaSeparator);
            for(uint32_t i = 0; i < snapCtx->NewFunctionTopLevelScriptCount; ++i)
            {
            ...
            writer->WriteSequenceStart_DefaultKey(NSTokens::Separator::CommaSeparator);
            for(uint32_t i = 0; i < snapCtx->EvalTopLevelScriptCount; ++i)
            {
            ...
            writer->WriteSequenceStart_DefaultKey(NSTokens::Separator::CommaSeparator);
            for(uint32_t i = 0; i < snapCtx->PendingAsyncModCount; ++i)
            {
                NSTokens::Separator sep = (i != 0) ? NSTokens::Separator::CommaSeparator : NSTokens::Separator::NoSeparator;
                writer->WriteRecordStart(sep);
        ...
        void ParseSnapContext(SnapContext* intoCtx, bool readSeparator, FileReader* reader, SlabAllocator& alloc)
        {
            ...
            reader->ReadSequenceStart_WDefaultKey(true);
            for(uint32_t i = 0; i < intoCtx->LoadedTopLevelScriptCount; ++i)
            {
            ...
            reader->ReadSequenceStart_WDefaultKey(true);
            for(uint32_t i = 0; i < intoCtx->NewFunctionTopLevelScriptCount; ++i)
            {
            ...
            reader->ReadSequenceStart_WDefaultKey(true);
            for(uint32_t i = 0; i < intoCtx->EvalTopLevelScriptCount; ++i)
            {
            ...
            reader->ReadSequenceStart_WDefaultKey(true);
            for(uint32_t i = 0; i < intoCtx->PendingAsyncModCount; ++i)
            {
                reader->ReadRecordStart(i != 0);
        ...
        void AssertSnapEquiv(const SnapContext* snapCtx1, const SnapContext* snapCtx2, const JsUtil::BaseDictionary<TTD_LOG_PTR_ID, NSSnapValues::SnapRootInfoEntry*, HeapAllocator>& allRootMap1, const JsUtil::BaseDictionary<TTD_LOG_PTR_ID, NSSnapValues::SnapRootInfoEntry*, HeapAllocator>& allRootMap2, TTDCompareMap& compareMap)
        {
            ...
            for(uint32_t i = 0; i < snapCtx1->PendingAsyncModCount; ++i)
            {
