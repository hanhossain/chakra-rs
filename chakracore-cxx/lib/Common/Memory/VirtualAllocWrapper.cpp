...
PreReservedVirtualAllocWrapper::~PreReservedVirtualAllocWrapper()
{
    if (IsPreReservedRegionPresent())
    {
        ...
        PreReservedHeapTrace(u"MEM_RELEASE the PreReservedSegment. Start Address: 0x%p, Size: 0x%x * 0x%x bytes", preReservedStartAddress, PreReservedAllocationSegmentCount,
...
void * PreReservedVirtualAllocWrapper::EnsurePreReservedRegionInternal()
{
    ...
    if (PHASE_FORCE1(Js::PreReservedHeapAllocPhase))
    {
        ...
        PreReservedHeapTrace(u"Reserving PreReservedSegment For the first time(CFG Non-Enabled). Address: 0x%p\n", preReservedStartAddress);
...

    {
        ...
        if (lpAddress == nullptr)
        {
            ...
            do
            {
                ...
                {
                    PreReservedHeapTrace(u"No more space to commit in PreReserved Memory region.\n");
                    return nullptr;
                }
...
#pragma warning(suppress: 28160) // Calling VirtualFreeEx without the MEM_RELEASE flag frees memory but not address descriptors (VADs)
        ...
        if (success)
        {
            PreReservedHeapTrace(u"MEM_DECOMMIT: Address: 0x%p of size: 0x%x bytes\n", lpAddress, dwSize);
        }
