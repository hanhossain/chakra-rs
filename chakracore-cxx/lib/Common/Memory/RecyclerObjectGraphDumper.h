...
public:
    struct Param
    {
        bool (*dumpReferenceFunc)(char16_t const *, void *objectAddress, void *referenceAddress);
        bool dumpRootOnly;
        bool skipStack;
    ...
    void BeginDumpObject(char16_t const * name);
    void BeginDumpObject(char16_t const * name, void* objectAddress);
    void EndDumpObject();
...
#define DUMP_IMPLICIT_ROOT(recycler, address) BEGIN_DUMP_OBJECT(recycler, u"Implicit Root"); DUMP_OBJECT_REFERENCE(recycler, address); END_DUMP_OBJECT(recycler);
#else
