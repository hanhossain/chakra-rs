...
public:
    ...
    {
        Output::Print(u"[ 0x%08x { 0x%08x, 0x%08x }]", node, node->strongRef, mode->next);
    }

    void Dump()
    {
        RecyclerWeakReferenceBase *current;
        Output::Print(u"HashTable with %d buckets and %d nodes\n", this->size, this->count);

        for (uint i=0;i<size;i++) {
            Output::Print(u"Bucket %d (0x%08x) ==> ", i, &buckets[i]);
...
#ifdef RECYCLER_TRACE_WEAKREF
                Output::Print(u"Remove 0x%08x to bucket %d, count is %d\n", current, val, count);
#endif
                break;
            }
            pprev = &current->next;
...
#ifdef RECYCLER_TRACE_WEAKREF
        Output::Print(u"Add WeakRef 0x%08x for StrongRef %p to bucket %d\n", entry, strongReference, targetBucket);
#endif
