...
namespace JsUtil
{
    ...
    public:
        ...
        static bool StaticEquals(__in_z T const * s1, __in_z T const* s2, charcount_t length);

        static hash_t StaticGetHashCode(__in_z T const * s, charcount_t length)
        {
            return InternalGetHashCode<false>(s, length);
        }

        template <bool fastHash>
        static hash_t InternalGetHashCode(__in_z T const * s, charcount_t length)
        {
    ...
    template<>
    inline bool
    CharacterBuffer<char16_t>::StaticEquals(__in_z char16_t const * s1, __in_z char16_t const * s2, charcount_t length)
    {
        return (s1 == s2) || PAL_wmemcmp(s1, s2, length) == 0;
    }

    template<>
    inline bool
    CharacterBuffer<unsigned char>::StaticEquals(__in_z unsigned char const * s1, __in_z unsigned char const *s2, charcount_t length)
    {
