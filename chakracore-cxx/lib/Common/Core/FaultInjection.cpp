...
#define CaptureStack(FramesToSkip, FramesToCapture, BackTrace, BackTraceHash) \
    ...
    bool FaultInjection::InitializeSym()
    {
        ...
        if (hDbgHelp == NULL)
        {
            hDbgHelp = LoadLibraryEx(u"dbghelp.dll", 0, 0);
        }
        if (hDbgHelp == NULL)
        {
...
#pragma endregion helpers
    ...
    const char16_t* crashStackStart = u"=====Callstack for this exception=======\n";
    const char16_t* crashStackEnd = u"=====End of callstack for this exception=======\n";
    const char16_t* injectionStackStart = u"=====Fault injecting record=====\n";
...
#define FAULT_TYPE(x) _u(#x),\
    ...
    const char16_t *FaultInjection::FaultTypeNames[] =
    {
#include "FaultTypes.h"
    };
#undef FAULT_TYPE
    static_assert(sizeof(FaultInjection::FaultTypeNames) == FaultInjection::FaultType::FaultTypeCount*sizeof(char16_t*),
    ...
    bool FaultInjection::FaultInjectionTypes::IsEnabled(const char16_t* name)
    {
    ...
    FaultInjection::~FaultInjection()
    {
        ...
        {
            Output::Print(u"Available Fault Types:\n");
    ...
    bool FaultInjection::IsFaultEnabled(FaultType faultType)
    {
        if (!faultInjectionTypes)
        {
            ...
            if ((const char16_t*)globalFlags.FaultInjectionType == nullptr)
            {
    ...
    void FaultInjection::ParseFaultTypes(const char16_t* szFaultTypes)
    {
        ...
        const char16_t* delims = u",";
        char16_t *nextTok = nullptr;
    ...
    static void SmashLambda(_Inout_z_ char16_t* str)
    {
    ...
    bool FaultInjection::EnsureStackMatchInfraInitialized()
    {
        ...
        else if (stackMatchInitialized == Uninitialized)
        {
            ...
            char16_t buffer[MAX_SYM_NAME]; // assume the file is normal
            ...
            while (PAL_fgetws(buffer, MAX_SYM_NAME, fp))
            {
                ...
                const char16_t jscript9test[] = u"jscript9test!";
                const char16_t jscript9[] = u"jscript9!";
                char16_t* symbolStart = stristr(buffer, jscript9test);
                ...
                {
                    auto tmp = (char16_t*)realloc(baselineStack[baselineFrameCount], (len + 1)*sizeof(char16_t));
    ...
    void FaultInjection::InstallExceptionFilters()
    {
        ...
        if (GetEnvironmentVariable(u"FAULTINJECTION_DEBUG", nullptr, 0) != 0)
        {
    ...
    void FaultInjection::FaultInjectionAnalyzeException(_EXCEPTION_POINTERS *ep)
    {
        ...
        auto printFrame = [&](void * addr)
        {
            ...
            if (pfnSymFromAddrW(hProcess, (unsigned long)addr, &dwSymDisplacement, &sip.si))
            {
                ...
                PAL_fwprintf(PAL_get_stderr(), u"%s!%s+0x%llx\n", mi.ModuleName, sip.si.Name, (unsigned long)dwSymDisplacement);
            }
            else
            {
                PAL_fwprintf(PAL_get_stderr(), u"0x%p\n", addr);
            }
        };
        ...
        PAL_fwprintf(PAL_get_stderr(), crashStackStart);
        ...
        PAL_fwprintf(PAL_get_stderr(), crashStackEnd);
        ...
        while (record)
        {
            ...
            {
                PAL_fwprintf(PAL_get_stderr(), injectionStackStart);
                for (int i = 0; i < record->FrameCount; i++)
                {
                    printFrame(backTrace[i]);
                }
                PAL_fwprintf(PAL_get_stderr(), injectionStackEnd);
            }
            record = record->next;
        }
        ...
        PAL_fwprintf(PAL_get_stderr(), u"***FI: Exception: %08x, module: %s, offset: 0x%p\n",
        ...
        if (needDump)
        {
            thread_local static char16_t dumpName[MAX_PATH + 1];
            wcscpy_s(filename, globalFlags.Filename);
            char16_t* jsFile = filename;
            char16_t *pch = jsFile;
            ...
            while (true)
            {
                _snwprintf_s(dumpName, _TRUNCATE, u"%s_%s_M%d_T%s_C%d_%llx_%llx_%d.dmp",
                    mainModule, jsFile,
            ...
            {
                PAL_fwprintf(PAL_get_stderr(), u"CreateFile <%s> failed. gle=0x%08x\n", dumpName, GetLastError());
            }
            else
            {
                ...
                thread_local static char16_t dbgTip[1024];
                if (InjectionFirstRecord == nullptr)
                {
                    wcsncpy_s(dbgTip,
                        u"\n"
                        u"************************************************************\n"
                        u"* The dump is made by FaultInjection framework, however, the fault is not actually injected yet.\n"
                        u"************************************************************\n", _TRUNCATE);
                }
                else
                {
                ...
                if (rv)
                {
                    PAL_fwprintf(PAL_get_stderr(), u"Minidump created: %s\n", dumpName);
                }
                else
                {
                    PAL_fwprintf(PAL_get_stderr(), u"MiniDumpWriteDump failed. gle=0x%08x\n", GetLastError());
                }
                CloseHandle(hFile);
            }
        }
