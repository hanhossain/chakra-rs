...
void BailOutRecord::DumpValue(int offset, bool isFloat64)
{
    if (offset < 0)
    {
        Output::Print(u"Stack offset %6d",  offset);
    }
    else if (offset > 0)
    {
        if ((uint)offset <= GetBailOutRegisterSaveSlotCount())
        {
            ...
            {
#ifdef _M_ARM
                Output::Print(u"Register %-4S  %4d", RegNames[(offset - RegD0) / 2  + RegD0], offset);
#else
                Output::Print(u"Register %-4S  %4d", RegNames[offset], offset);
#endif
            }
            else
            {
        ...
        else
        {
            ...
            Output::Print(u"Constant index %4d value:0x%p (Var)", constantIndex, this->constants[constantIndex]);
            Assert(!isFloat64);
        }
    }
    else
    ...
    *innerMostInlinee = nullptr;
    ...
    if (entryPointInfo->HasInlinees())
    {
        ...
        if (inlineeFrameRecord)
        {
            ...
            InlinedFrameLayout* outerMostFrame = (InlinedFrameLayout *)(((uint8_t *)Js::JavascriptCallStackLayout::ToFramePointer(layout)) - entryPointInfo->GetFrameHeight());
    ...
    uint32_t bailOutOffset, IR::BailOutKind bailOutKind, Js::Var branchValue, Js::Var *registerSaves, BailOutReturnValue * bailOutReturnValue)
{
    ...
    BAILOUT_TESTTRACE(executeFunction, bailOutKind, u"BailOut: function %s, Loop: %d Opcode: %s", executeFunction->GetDisplayName(),
    ...
    BailOutRecord const * bailOutRecord, IR::BailOutKind bailOutKind, uint32_t actualBailOutOffset, Js::ImplicitCallFlags savedImplicitCallFlags, void * returnAddress)
{
    ...
    uint8_t callsCount = entryPointInfo->callsCount > 255 ? 255 : static_cast<uint8_t>(entryPointInfo->callsCount);
        ...
        PHASE_FORCE(Js::ReJITPhase, executeFunction))
    {
        ...
        else switch(bailOutKind)
        {
            ...
            case IR::BailOutOnNoProfile:
                ...
                else if (executeFunction->IncrementBailOnMisingProfileRejitCount() >  BailOnNoProfileRejitLimit)
                {
            ...
            case IR::BailOutFailedFixedFieldTypeCheck:
            {
                ...
                else
                {
                    ...
                    if (PHASE_TRACE(Js::ObjTypeSpecPhase, executeFunction))
                    {
                        char16_t debugStringBuffer[MAX_FUNCTION_BODY_DEBUG_STRING_SIZE];
                        Output::Print(
                            u"Objtypespec (%s): States on bailout: Saved cache: %d, Live cache: %d\n",
    ...
    REJIT_KIND_TESTTRACE(bailOutKind, u"Bailout from function: function: %s, bailOutKindName: (%S), bailOutCount: %d, callCount: %d, reJitReason: %S, reThunk: %s\r\n",
    ...
    else if (rejitReason != RejitReason::None)
    {
        ...
        if(PHASE_TRACE(Js::ReJITPhase, executeFunction))
        {
            char16_t debugStringBuffer[MAX_FUNCTION_BODY_DEBUG_STRING_SIZE];
            Output::Print(
                u"Rejit: function: %s (%s), bailOutCount: %hu",
                executeFunction->GetDisplayName(),
                executeFunction->GetDebugNumberSet(debugStringBuffer),
                bailOutRecord->bailOutCount);

            Output::Print(u" callCount: %u", callsCount);
            Output::Print(u" reason: %S", GetRejitReasonName(rejitReason));
            if(bailOutKind != IR::BailOutInvalid)
            {
...
void BailOutRecord::ScheduleLoopBodyCodeGen(Js::ScriptFunction * function, Js::ScriptFunction * innerMostInlinee, BailOutRecord const * bailOutRecord, IR::BailOutKind bailOutKind)
{
    ...
    uint8_t totalJittedLoopIterations = entryPointInfo->totalJittedLoopIterations > 255 ? 255 : static_cast<uint8_t>(entryPointInfo->totalJittedLoopIterations);
        ...
        else switch(bailOutKind)
        {
            ...
            case IR::BailOutFailedTypeCheck:
                ...
                if(executeFunction->GetByteCodeInLoopCount() > (uint)CONFIG_FLAG(LoopBodySizeThresholdToDisableOpts))
                {
                    ...
                    {
                        Output::Print(u"Disabled obj type spec in jit loop body for loop %d in %s (%d)\n",
    ...
    REJIT_KIND_TESTTRACE(bailOutKind, u"Bailout from loop: function: %s, loopNumber: %d, bailOutKindName: (%S), reJitReason: %S\r\n",
    ...
    if (rejitReason != RejitReason::None)
    {
        ...
        if(PHASE_TRACE(Js::ReJITPhase, executeFunction))
        {
            char16_t debugStringBuffer[MAX_FUNCTION_BODY_DEBUG_STRING_SIZE];
            Output::Print(
                u"Rejit(loop): function: %s (%s) loop: %u bailOutCount: %hu reason: %S",
                executeFunction->GetDisplayName(),
                executeFunction->GetDebugNumberSet(debugStringBuffer),
