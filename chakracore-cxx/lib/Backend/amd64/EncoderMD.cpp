...
static const uint8_t OpEncoding[] =
{
...
EncoderMD::GetRegEncode(RegNum reg)
{
    ...
    return (uint8_t)(OpEncoding[1 + reg - RegRAX]);
}
...
EncoderMD::EmitModRM(IR::Instr * instr, IR::Opnd *opnd, uint8_t reg1)
{
    ...
    uint8_t regBase;
    uint8_t regIndex;
    uint8_t baseRegEncode;
    uint8_t rexEncoding = 0;
    ...
    case IR::OpndKindIndir:
        ...
        else
        {
            ...
            else if (baseOpnd->GetReg() == RegR12 || baseOpnd->GetReg() == RegRSP)
            {
                ...
                *(m_pc++) = (uint8_t)(((regBase & 7) << 3) | (regBase & 7));
...
EncoderMD::Encode(IR::Instr *instr, uint8_t *pc, uint8_t* beginCodeAddress)
{
    ...
    for (;; opcodeTemplate++, form++)
    {
        ...
        uint8_t opcodeByte = *opcodeTemplate;
        uint8_t rexByte = REXOVERRIDE | REXW;
...
void
EncoderMD::EmitRexByte(uint8_t * prexByte, uint8_t rexByte, bool skipRexByte, bool reservedRexByte)
{
    if (skipRexByte)
    {
            ...
            if (reservedRexByte)
            {
                ...
                if (m_relocList != nullptr)
                {
                    ...
                    if (lastRelocEntry.m_ptr > prexByte && lastRelocEntry.m_ptr < m_pc)
                    {
                        ...
                        lastRelocEntry.m_ptr = (uint8_t*)lastRelocEntry.m_ptr - 1;
                        lastRelocEntry.m_origPtr = (uint8_t*)lastRelocEntry.m_origPtr - 1;
                    }
                }
            }
            return;
        ...
        case RelocTypeBranch:
            {
                ...
                if (reloc->isShortBr() )
                {
                    ...
                    {
                        Encoder::EnsureRelocEntryIntegrity(codeBufferAddress_, codeSize, (size_t)m_encoder->m_encodeBuffer, (size_t)relocAddress, sizeof(uint8_t), (ptrdiff_t)labelInstr->GetPC() - ((ptrdiff_t)reloc->m_ptr + 1));
                    }
                }
                else
                {
...
uint8_t
EncoderMD::GetRexByte(uint8_t rexCode, IR::Opnd * opnd)
{
    return this->GetRexByte(rexCode, opnd->AsRegOpnd()->GetReg());
}

uint8_t
EncoderMD::GetRexByte(uint8_t rexCode, RegNum reg)
{
