...
namespace Wasm
{
    ...
    {
        Assert(m_numElem > 0);
        m_elems = AnewArray(m_alloc, uint32_t, m_numElem);
        memset(m_elems, Js::Constants::UninitializedValue, m_numElem * sizeof(uint32_t));
    }

    void WasmElementSegment::AddElement(const uint32_t funcIndex)
    {
        if (m_elems == nullptr)
        {
            Init();
    ...
    uint32_t WasmElementSegment::GetElement(const uint32_t tableIndex) const
    {
