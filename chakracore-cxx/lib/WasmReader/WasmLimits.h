...
namespace Wasm
{
    ...
    public:
        ...
        static uint32_t GetMaxElementSegments()
        {
            return CONFIG_FLAG(WasmIgnoreLimits) ? UINT32_MAX : MaxElementSegments;
        }
        static uint32_t GetMaxTableSize()
        {
            return CONFIG_FLAG(WasmIgnoreLimits) ? UINT32_MAX : CONFIG_FLAG(WasmMaxTableSize);
        }
        static uint32_t GetMaxStringSize() { return CONFIG_FLAG(WasmIgnoreLimits) ? UINT32_MAX : MaxStringSize; }
        static uint32_t GetMaxFunctionLocals()
        {
            return CONFIG_FLAG(WasmIgnoreLimits) ? UINT32_MAX : MaxFunctionLocals;
        }
        static uint32_t GetMaxFunctionParams()
        {
            return CONFIG_FLAG(WasmIgnoreLimits) ? UINT32_MAX : MaxFunctionParams;
        }
        ...
        static unsigned long GetMaxBrTableElems()
        {
            return CONFIG_FLAG(WasmIgnoreLimits) ? UINT32_MAX : MaxBrTableElems;
        }
        static uint32_t GetMaxMemoryInitialPages()
        {
            return CONFIG_FLAG(WasmIgnoreLimits) ? UINT32_MAX : MaxMemoryInitialPages;
        }
        static uint32_t GetMaxMemoryMaximumPages()
        {
            return CONFIG_FLAG(WasmIgnoreLimits) ? UINT32_MAX : MaxMemoryMaximumPages;
        }
