...
namespace Wasm
{
    ...
    {
        EmitInfo(Js::RegSlot location_, const WasmTypes::WasmType &type_) : WAsmJs::EmitInfoBase(location_), type(type_)
        {
        }
    ...
    public:
        ...
        PolymorphicEmitInfo(EmitInfo info) { Init(info); }
    ...
    struct WasmLocal
    {
        WasmLocal() : location(Js::Constants::NoRegister), type(WasmTypes::Limit) {}
        WasmLocal(Js::RegSlot loc, WasmTypes::WasmType tp) : location(loc), type(tp) {}
        Js::RegSlot location;
        WasmTypes::WasmType type;
    };
    ...
    public:
        ...
        ~WasmCompilationException() { SysFreeString(errorMsg); }
    ...
    struct BlockInfo
    {
        ...
        bool HasYield() const { return yieldInfo.Count() > 0; }
    ...
    private:
        ...
        EmitInfo EmitMemAccess(WasmOp wasmOp, const WasmTypes::WasmType *signature,
                               Js::ArrayBufferView::ViewType viewType);
        EmitInfo EmitSimdMemAccess(Js::OpCodeAsmJs op, const WasmTypes::WasmType *signature,
                                   Js::ArrayBufferView::ViewType viewType, uint8_t dataWidth, bool isStore);
        ...
        BlockInfo *PushLabel(WasmBlock blockData, Js::ByteCodeLabel label, bool addBlockYieldInfo = true,
                             bool checkInParams = true);
        ...
        PolymorphicEmitInfo PopStackPolymorphic(PolymorphicEmitInfo expectedTypes,
        ...
        EmitInfo PopEvalStack(WasmTypes::WasmType expectedType = WasmTypes::Any,
                              const char16_t *mismatchMessage = nullptr);
